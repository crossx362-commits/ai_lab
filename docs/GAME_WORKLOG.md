...
## ⛔ 다음 세션 먼저 읽기 — 루프가 멈춰 있다 (2026-08-20 01:06)

> **✅ 갱신(2026-08-20, 클로드): 아래 결함은 수리 완료, 루프 재기동됨.**
> `loop.sh` 폴백을 ①프로세스 한정(loop/agent에 영구 기록 안 함, `AGENT_BASE` 복귀)
> ②그록 402 감지 시 `loop/grok_dead` 표식(TTL 12h) + 표식 신선하면 폴백 제외로 고쳤다.
> `loop/agent`는 claude로 복구. 원장 기록: HARNESS_GUARDRAILS_LEDGER.md 끝줄.

**루프를 그냥 켜지 마라. 켜면 몇 초 만에 같은 자리에 다시 빠진다.**
...
### 결함 — 폴백 대상이 죽어 있는데 확인을 안 한다 (미수정)
`loop.sh:281-288`은 한도에 걸리면 **그록 CLI가 있기만 하면**(`grok_bin`) 전환하고,
그 값을 `loop/agent`에 **영구히 써 버린다**. 그래서 지금 `loop/agent = grok`이다 —
**클로드 한도가 풀려도 재기동하면 곧장 죽은 그록으로 간다.** 되돌리는 코드는 없다.
