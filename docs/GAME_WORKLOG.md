...
| 개발 직렬 | 잡몹 피해 닫음 | 잡몹 속도 `8c89e69b` · 잡몹 HP `6f1e1226` · 잡몹 피해 `fc2499d8`(MobDmg·QA_NO). `W3Party` 금지 |
...
| UI 폴리싱 다음 | 직전=코드(잡몹 피해) | 다음은 UI 폴리싱 1건. 허브 본문-내비는 `0fc2eb14`로 닫힘 |
...
(원장 2~4% = 25~50대에 사망)가 authored인데 grep 소비처 0곳. 형제 체력배율은 MobHp가
읽는데 피해만 죽어 있던 함정. `MobDmg`가 읽고 속성 탭·던전 부제가 소비.
`QA_NO_MOB_DMG`면 옛 0.03·줄 없음. 표시 전용 — W3Party 무접촉.

샷 `mob_dmg_shots/qa_go_Character.png` — 속성 탭 `잡몹 피해 3%(§18-11)` (잡몹 HP 아래).
네거티브 `qa_negctrl_no_dmg.png` — `QA_NO_MOB_DMG=1`이면 그 줄이 사라지고 HP 다음이 PvP 회복.
코드 `fc2499d8`. 다음은 UI 폴리싱 1건(직전=코드).

### 검수
- `game_compile_check.py` PASS (350소스 0).
- `unity_meas` batch `MobDmgSelfCheck` PASS 21/21 (`results/mob_dmg_selfcheck.log`).
...
직전 트랙 코드(티어배율) → 필드 사냥 시작 편성 한 결함. 스타트/취소가
본문 yMax=640에 붙어 내비 플레이트(636)와 겹쳤다. `HuntPickHud.Content`·
`Actions`가 NavPlateTop-12. `QA_NO_HUNT_PICK_NAV`면 옛 640 겹침.
표시 전용 — W3Party 무접촉.

샷 `hunt_pick_nav_shots/after.png` — 스타트/취소 금테가 내비 위.
네거티브 `neg.png` — QA_NO면 금테가 내비 타일에 먹힘.
코드 `0a66661f`. 다음은 소비처0 재스캔(직전=폴리싱).

### 검수
- `game_compile_check.py` PASS (330소스 0).
- `unity_meas` batch `HuntPickHudSelfCheck` PASS (`results/hunt_pick_hud_selfcheck.log` — 아랫변 624 · 간격 12 · 차단 아랫변 640).
- 샷 쌍 플레이모드 육안 확인. 블렌더 꺼짐(3D 없음).

## 완료 (2026-08-25) — §18-1 티어배율 소비처 (Grok)

원장 §18-1 앵커 ② 티어당 ×1.6이 BalanceConfig.티어배율로 authored돼
있으면서도 grep 소비처 0곳이었다. 형제 티어1시간당골드(GhAnchor)는 읽히는데
Economy.TierRevenueMultiplier만 ×1.6 거듭제곱 표로 죽어 있던 함정.
`TierMul.Table`이 읽고 필드 골드·행위 비용이 따른다. `QA_NO_TIER_MUL`면 옛 표·줄 없음.
표시 전용 줄 + 표 소비 — W3Party 무접촉.

샷 `tier_mul_shots/qa_go:Character.png` — 부제 `티어당 ×1.6(§18-1)`.
네거티브 `qa_negctrl_no_mul.png` — `QA_NO_TIER_MUL=1`이면 기본 부제만.
코드 `9de769f8`. 다음은 UI 폴리싱 1건(직전=코드).

### 검수
- `game_compile_check.py` PASS (328소스 0).
- `unity_meas` batch `TierMulSelfCheck` PASS (`results/tier_mul_selfcheck.log`).
- 샷 쌍 플레이어 육안 확인. 블렌더 꺼짐(3D 없음). MCP execute_code는 도메인 리로드로 끊김.

## 완료 (2026-08-25) — 파티 편성 탭 안내줄-내비 (Grok)

직전 트랙 폴리싱(스타일 선택 바) → 파티 편성 탭 한 결함. 하단 Info가
본문 yMax=640에 붙어 내비 플레이트(636)와 겹쳤다. `PartyFormHud.Content`·`Hint`가
NavPlateTop-12. `QA_NO_PARTY_FORM_NAV`면 옛 640 겹침. 던전은 하단바가 없어
내비 결함이 아니다. 표시 전용 — W3Party 무접촉.

샷 `party_form_nav_shots/after.png` — 안내줄 금테가 내비 위.
네거티브 `neg.png` — QA_NO면 금테가 내비 타일에 먹힘.
코드 `99f4f2aa`. 다음은 소비처0 재스캔.

### 검수
- `game_compile_check.py` PASS (324소스 0).
- `PartyFormHudSelfCheck` PASS (아랫변 624 · 간격 12 · 차단 아랫변 640).
- 샷 쌍 플레이모드 육안 확인. 블렌더 꺼짐(3D 없음).
