...
문서 전 영지·EstateYard 금지. 폴리싱 다음은 필드.
...
### 3) 영지 — 건물 위치·신규 아트·건설/업그레이드 창 — ⏳ 남음(다음 작업)
오너 지시 원문: "영지에 바닥 모양 때문에 건물이 위치 안 맞는 것 같아 해결하고
건물 새로 만들어줘 어울리게 바닥도 고려해서, 클래시 오브 클랜처럼 건설도 가능하고
업그레이드도 건물 선택해서 바로 창 떠서 보게".
현재 상태: 바닥 폭 100% 달성(`FitHead` 0.05), 바닥 명도 0.19로 건물(0.15)과 맞춤.
`BuildingBox`는 크기표(`prop_scale.json`)를 읽도록 이미 고쳤으나 **바닥
다이아몬드와의 정렬은 미확인**. `EstateHud.PaletteTiles`(건설 팔레트)는 이미 존재.
