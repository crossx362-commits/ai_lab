...
## 1차 6종 실전 계측 하네스 — 코드 완료·Unity 실행 차단 (2026-08-16)
...
- 실행 블로커: `unity_meas` 빌드가 `com.unity.modules.physicscore2d@1.0.0 cannot be found`로
  중단. 원본 useHub 에디터와 다른 세션의 manifest는 건드리지 않음.
- 남은 판정: 6종 각각 두 수치 `>0`, 직업별 PNG 6장, `QA_ADV_NO_SLOT=2`에서 슬롯2=0/FAIL.
