...
## 지금 (2026-08-19) — 탑 대보스 마릿수 60/30/10

직전 트랙 UI → 코드 칸. §10-7 탑 대보스만 항상 1체였다(던전 Plan은 이미 굴림).
`BossCount.Begin`을 `BattleScreen`이 읽는다. 10층 Force 2. `QA_NO`면 옛 1.
샷 `boss_count_shots/qa_go:Tower.png` — `대보스 2체(§10-7)`.
코드 `9a84944d`. 다음은 UI 한 결함(탑 레이드 5층 단위 도크 부제). 이펙트는 FX 미커밋.
