...
- 미구현 큐: 헬리콥터, 궁극기 §49, 네트워크. 화면 방해 아이템 4종·각도/파워 고정탄·회오리도 보류(§2-9-10 맨 아래에 이유). ⚠️ **셋 다 명세에 내용이 정의돼 있지 않다** — 아이템·헬리콥터·궁극기는 기획서 원문에도 규칙이 없어, 만들려면 오너에게 규칙을 받거나 원작(포트리스2)을 조사해 출처를 코드 머리말에 남겨야 한다(내가 발명한 시스템은 과거에 오너가 되돌렸다).
