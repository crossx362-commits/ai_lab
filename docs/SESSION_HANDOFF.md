...
- 미구현 큐: 아이템/헬리콥터, 궁극기 §49, 네트워크. ⚠️ **셋 다 명세에 내용이 정의돼 있지 않다** — 아이템·헬리콥터·궁극기는 기획서 원문에도 규칙이 없어, 만들려면 오너에게 규칙을 받거나 원작(포트리스2)을 조사해 출처를 코드 머리말에 남겨야 한다(내가 발명한 시스템은 과거에 오너가 되돌렸다).
