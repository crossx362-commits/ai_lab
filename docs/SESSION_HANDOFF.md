...
- 🚨 **연출 커밋(`bb292005`)의 검증 상태 — «절반만» 닫혔다**(2026-09-20).
  ...
  **왜 무효인가**: 「점화중/꺼진뒤」 두 장을 픽셀로 쟀는데, **내 코드가 아무것도 안 하는 기종**
  (추진 0 · 연기만: 캐터펄트·캐롯·듀크·마인랜더·포세이돈)도 **400~1144px 변했다** — 입자가 난수다.
  **바닥 1144 와 신호 686~1514 가 겹친다.**
  ⇒ **다음 사람이 할 일**: 픽셀 말고 **숫자를 찍어라** — 갤러리가 두 시각에서 각 기종의
  **방출률·링 크기·불꽃 속도**를 로그로 출력하게 하면 **결정적**이다(난수가 안 낀다).
