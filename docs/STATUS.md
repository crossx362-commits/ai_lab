...
| # | 항목 | 통과 기준 | 네거티브 컨트롤 |
|---|---|---|---|
| 1 | **몹 AI 4계열 실루엣 재생성** ⭐ **2/4 완료(chaser✅ charger✅) — ranged·swarmer 남음** | §0-A 픽셀아트 화풍, **4 AI 실루엣** × 22프레임씩. 무채색 회색-백(색은 런타임 `MobDef.색조`). **charger 반입·Read 육안검증 완료**(코뿔소 브루트·wind-up 텔레그래프), 인게임 qa_hunt만 GUI세션 대기. 남은 2계열: spec 준비됨, **1계열씩** 아래 파이프라인대로 생성 | 옛 아트로 되돌리면 캐릭터와 톤이 갈림(옛 charger=매끈 3D톤 양 vs 신규=셀셰이딩 코뿔소, 자명) |
