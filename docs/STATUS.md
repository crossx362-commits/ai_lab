...
최종 갱신: 2026-08-18 · 침략 진입 면 선택(§13-3)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **기획서 ✅ · 소비처 0곳** — 원장 `GAME_DESIGN_ASHES_TO_STARS.md`를 훑어 ✅인데 grep 소비처가 0인 칸 **하나만** 닫는다. §13-3 진입 면 선택은 닫음. 시각 UI「다른 게임만큼」·V2 손맛·V4 70%는 사람 관문이라 닫지 않는다. `W3Party`·오너 Unity는 건드리지 않는다. 4직업·몹 실루엣 재생성 금지. 통과 기준: 소비처 ≥1 + SelfCheck + 네거티브. 증거 없는 완료 금지.
