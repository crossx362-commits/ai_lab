...
최종 갱신: 2026-08-26 (바퀴) · **필드 정예 처치 훅 완료**(`03035f8f`) — W3Party.KillMob이 정예(종류 3·4) 처치 1회씩 `EliteDrop.NoteFieldKill()`로 넘김(GameMode·`!DungeonRun.Active` 한정 — 던전은 노드 Complete 기존 지급 유지·이중 지급 차단). §10-1 「처치 시 추가 드랍」을 노드와 같은 보상(강화석+일반 장비 §10-8)으로 지급 + `EliteDrop.FieldKills` 누적 카운트(다음 보정 바퀴 소비처). QA_NO_ELITE_DROP 네거티브로 기존 동작 유지. EliteDropSelfCheck 44/44 PASS(훅 단언 11건+W3Party 소스 계약 3건 신설)·GameSweep 전수 64/64 PASS exit 0(unity_meas 배치) + 오너 에디터 리프레시 도메인 리로드 후 콘솔 에러 0. 직전 바퀴 GameFullCheck 전수 정리(`c31479be`)·ORDERS①(`bf979fc9`)·③(`f5e6b1c3`)·②(`51c3e862`). 루프 가동 중(opencode).
...
1. **§18-14 소환수 재소환(0.5G/h + 쿨다운 30초)** — 승인(오너 위임, 원장 §18-14 수치 그대로). 신규 시스템 — 기존 소비처0 해소 패턴(Reader+속성 탭+QA_NO 네거티브) 준용. 선행 필드 정예 훅은 닫음(`03035f8f`).
