...
최종 갱신: 2026-08-20 · 폴리싱칸(필드) — 전투HUD 좌측 요약 패널 두 줄 세로 잘림(안 읽힘) 수정
마지막 트랙: 폴리싱
...
다음 코드칸 후보 (이번 이터에 발견, 화면 노출 정직 인계):
- **§5 이동기 프로필 행이 캐릭터 속성 패널에서 안 보인다** (기존 밀도 한계, 이번 배선이 유발 아님).
  속성 패널 `DrawAttributes`는 `Info`가 `panel.yMax > r.yMax`면 행을 건너뛴다(GameScreen.cs:485) —
  건강한 1차 캐릭터는 header(0)·목숨(1)·HP스탯(2)·전직(3)·직업특성ConceptLine(4)·보유스킬SkillLine(5)로
  6행(0~5)이 꽉 차고, 프로필 행(형태·리스크·자동사냥·거리·무적·쿨)은 **행 6이라 항상 넘쳐 안 보인다**.
  SkillLine(d120ee06, 형제 무적·쿨 5cc0a8ee보다 먼저)이 들어오며 프로필이 5행→6행으로 밀린 밀도 regression.
  즉 §5 이동기 표시(형태·거리·무적·쿨) 전부가 현재 캐릭터 화면에서 죽어 있다(코드는 맞고 화면은 없다).
  **통과 기준**: QA_JOB_TRAIT=1 캐릭터 속성 탭에 「거리 3초분 · 무적 0.3초 · 쿨 6초」(또는 형태 포함 §5 줄)이
  잘리지 않고 렌더. **네거티브 컨트롤**: 그 줄을 다시 행 6(패널 밖)으로 되돌리면 화면에서 사라진다.
  주의: 레이아웃 변경이라 우선순위 재배치(§5 이동기 spec을 StatLine 같은 상시노출 우선존으로)로
  ConceptLine/SkillLine 중 하나가 밀릴 수 있다 — 어느 걸 희생할지 §5(무적=「조작 핵심 기술」) 우선순위로 판단.
  ⚠️ 다음 세션 주의: 이번 이터 시점에 다른 세션이 텍스처 아트/임포트를 진행 중이었다 —
  `fx_*.png`·`bg_*.png` 미커밋 수정, `TextureImportRules.cs` +14(영지 바닥 타일용), `ui/icons`·`ui/chrome`
  metas 40여 개 untracked, 에셋 임포트 워커 가동. 전투HUD 화면에 뜬 `fx_death` 흰 종이 배경·스킬
  슬롯 흰 테두리 등은 그 in-flight 아트 하류라 **건드리지 않았다**(클로버 금지). 그 아트가 커밋된 뒤
  전투HUD를 다시 폴리싱하면 깨끗한 상태에서 판정 가능.

코드칸(§5 이동기거리) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱(영지)이라
