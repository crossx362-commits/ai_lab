...
최종 갱신: 2026-08-16 · 루프(파티 편성 명부 크롬)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)

1. **V4 외부 테스터 70% 사람 판정** — 자동검사로 통과를 선언하지 마라. 패배→삭제→재건 경계는 이미 닫힘.

> **이번 이터 결과(코드/실행): 파티 편성 명부에 이미 있던 초상 프레임·역할·목숨을 연결.**
...
| 항목 | 근거 | 커밋 |
|---|---|---|
| **파티 편성 명부 크롬(초상·역할·목숨)** | 캐릭터만 쓰던 프레임/하트/역할을 편성이 소비. SelfCheck PASS. PNG 643294B: 5인 초상+서로 다른 역할뱃지+하트3, 옛 회색글자·`목숨 3/3`과 갈림. API 제거 시 컴파일 RED 6건 | 이 커밋 |
