...
최종 갱신: 2026-08-20 · 코드칸(직업 스킬 배선) 성공 — JobDef.스킬(SkillDef[]) 소비처 0곳 배선 + 속성 탭 빈 행 갭 제거(커밋 PENDING_JOBSKILL)
마지막 트랙: 코드
폴리싱 다음: **전투HUD**. 이어 타이틀 → 영지 → 필드 → 캐릭터 → 파티 → 월드맵 → 던전 → 결과.
  (영지 폴리싱은 `docs/GAME_SPEC_ESTATE_BUILD.md` 문서 있으므로 허용 — 문서 전 금지 풀림, INBOX 처리됨 참조)

코드칸(직업 스킬 목록) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20, 커밋 `PENDING_JOBSKILL`): 직전 트랙
