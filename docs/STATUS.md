...
최종 갱신: 2026-08-16 · 이터레이션(코드 — §3 2차 각성 초필살기 전투 배선)
...
| 8 | **§3 전직 시스템** — 기본/구저장(`ab20a005`) + Lv20 선택(`76c6a80b`) + 재료/시험(`93b9fe39`) + 2→4스킬(`42315c7b`) + 6종 고유 분기(`8128fbf7`) + 6종 실전 하네스(`ab5fe3d0`) + Lv50 각성(`4a0b21d3`) + **2차 초필 실전(`31a50057`)**. 다음은 이전에 코드만 있고 패키지로 막혔던 **1차 6종 슬롯1/2 실행 증거 종결** | 6종이 검사 폴백 없이 슬롯별 효과 `>0`, 직업별 PNG 6장 | `QA_ADV_NO_SLOT=1|2`로 해당 슬롯을 끄면 계측 0·FAIL. 현재 사본 Unity 실행 가능 |
