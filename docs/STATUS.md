...
최종 갱신: 2026-08-17 · 루프(인간 전직 재료 +15%)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **UI 퀄리티 남은 것**(INBOX 16:46 · 21:45) — 하단 도크·격자 8×8·시작 2명·침략 보호막·수비대 회복·인간 PvE 18h·경매 수수료 7%·영지생산·드랍률·전직재료배율은 닫음. 허브 카드 여백은 다른 세션이 작업 중일 수 있다. 전체 「다른 게임만큼」은 사람 육안.
2. **기획서 ✅ · 소비처 0곳** — 대기하지 말고 원장을 훑어 다음 구멍을 큐에 올린다. `RaceDef` 런타임 칸은 전직재료까지 닫힘. 이속·체력은 W3Party가 이미 읽고, 방어배율·불굴·야성 감각은 W3Party라 대화 세션. 다음 비전투 구멍: **수인 약탈량 +20%**(§18-9). `InvasionState.LootCopper`가 기준값만 쓰고 종족을 안 본다. 통과 기준: 수인 약탈이 같은 티어 인간/엘프/드워프의 120%. `QA_NO_RACE_LOOT=1`이면 수인도 100. 네거티브: `LootCopper`에서 종족 배율을 빼면 수인=인간. 드워프 강화 +10%p는 `Equipment.SuccessPercent`가 이미 읽음. 16×16 부지·배치 프리셋·경로 전투 시뮬·무료 영입 3회·일과표는 💡라 안 연다. 동일 상대 24h -80%·인간 PvP 9h는 보호막과 어긋나 안 넣음.

V4 외부 테스터 70% → 넘김. 사람 70% 계속·24h 재실행은 측정하지 않았다. 테스터 통과가 아니다.

> **이번 이터 결과(코드/실행): 인간 전직 재료 +15% — RaceDef.전직재료배율을 읽는다.**
