...
최종 갱신: 2026-08-16 · 이터레이션(V3 — 파티 공격→보스 HP 연결, 실행 QA 대기)
...
| 2b | **보스 나머지 통합(HP·장판·힐체크)** — #2에서 갈라져 나옴. **HP 코드 `e48aee09`, 실행 QA 대기** | 파티가 W3 보스 타깃에 낸 피해만 보스별 HP·페이즈·처치에 전달. SelfCheck 계약 `9000→4500→0`, 페이즈1·처치1. ⚠️ Unity 실행/PNG 미확인. **장판(FloorAOE)은 파티 피해 0**이고 위험 슬롯을 영영 문다. **힐체크 `ReportDamageToActive` 호출부 0곳** → 요구회복 0으로 항상 통과 | `BOSS_NO_DPS=1`이면 Boss/W3 타깃 모두 `9000→9000`. 장판·힐체크는 각 기믹 비활성 시 효과 소멸 |
