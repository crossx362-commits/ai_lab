...
| — | 2026-08-26 20:05 | **루프 생존성 수리(INBOX STOP 「루프가 안 돈다」 19:25)** — 원인 체인 실측 확정: ①19:29 바퀴(캐릭터창 폴리싱)가 Unity MCP `Endpoint is unavailable`로 code=1 중간사망→STATUS 미갱신(`lap-20260826-192914` 로그 말미) ②미갱신 3회 「정상 종료(exit 0)」에 launchd KeepAlive(`SuccessfulExit=false`) 재실행 불가 ③재기동 감시자 loop_watch가 pgrep 의존+침묵 설계라 회복 실패 — **12:11~19:27 약 7시간 공백**(watch 로그 파일 부재 = log() 한 번도 미실행). macOS pgrep -f가 launchd 자식 일부 procargs를 못 읽는 신규 발견(PID 12651 loop.sh 살아있는데 rc=1 · speed_lane은 정상 매칭) → 감시 축을 lap 로그 mtime으로 교체(fresh ≤90분 통과·stale kickstart→bootstrap 폴백·매 실행 판정 근거 로그). PROMPT.md엔 「유니티 MCP 불능 시 그 검증만 건너뛰고 배치 SelfCheck 대체 검증 후 커밋·STATUS 완주」 규칙 추가. 게임 런타임·W3Party 무접촉 · 이미지 생성·블렌더 해당 없음 | `bash -n` PASS · 샌드박스 격리 실측 4시나리오 전부 PASS(lap 부재→재기동 · stale 2h→재기동 · fresh→통과 · STOP→건너뜀 — `DRY_RUN`+`LOOP_WATCH_ROOT` 임시루트) · 실환경 스모크 rc=0·launchctl 호출 0·메인 PID 12651 무사 · PROMPT.md 배포본 bootout 없이 cp 동기화(diff 일치) · 커밋 가드 temp-index 분리 PASS(2건 일치) · `d2410dcf` |
