...
최종 갱신: 2026-08-20 · 코드칸(캐릭터 속성 패널 밀도) — 76px 그리드에 넘쳐 죽어 있던 ✅ 소비처 3줄(보유 스킬·종족 특성·종족 정체성)을 컴팩트 피치로 전부 화면 노출
마지막 트랙: 코드
폴리싱 다음: **파티**. 이어 월드맵 → 던전 → 결과 → 전투HUD → 타이틀 → 영지 → 필드 → 캐릭터.
  (영지 폴리싱은 `docs/GAME_SPEC_ESTATE_BUILD.md` 문서 있으므로 허용 — 문서 전 금지 풀림, INBOX 처리됨 참조)

코드칸(캐릭터 속성 패널 밀도 상한) 이번 이터 결과 — 화면 노출 완료 (루프, 2026-08-20): 직전 트랙 폴리싱(캐릭터)이라
코드칸. Explore 전수 grep(전투 디렉터리 `Assets/Scripts/W3Party.cs` 포함)으로 진짜 소비처 0곳 후보를 받았으나
1위 `RaceDef.전투당발동`은 표시가 이미 고유메커니즘 산문(「불굴 …(전투당 1회)」 ProjectSetup:127·129)에 중복돼 있고
실기능은 전투 게이트(전투 AI 통합)라 코드칸 범위 밖으로 판정, 나머지 2후보(정예우선타겟·소모품자동사용)는 💡라 제외.
대신 STATUS 큐 맨 위 「캐릭터 속성 패널 밀도 상한」을 닫았다. **원인**: 속성 탭(`DrawAttributes`)은 표제·목숨·기본스탯
(§3 StatLine)·이동기(§5)·전직·직업 특성(§4 ConceptLine)·보유 스킬(§3 SkillLine)·종족 특성(§18-9 MechanicLine)·종족
정체성(§18-9 IdentityLine)까지 **이미 배선된 ✅ 소비처가 9~10행** 쌓이는데, 공유 행 그리드가 76px(RowH 64+RowGap 12)
고정이라 1차+ 캐릭터에서 하위 3줄(보유 스킬·종족 특성·종족 정체성)이 `r.yMax`를 넘겨 조용히 사라졌다(Info는 넘치면
건너뜀 — GameScreen.cs:485). 배선은 맞고 밀도만 죽어 있던 regression. **수리**: 개별 커스텀 버튼은 `DrawAtlasButton`이
private라 못 만들므로, 공유 헬퍼(`GameScreen`)에 행 피치/높이 필드 `RowPitch`/`RowHt`를 추가(**기본값 76/64라 타 화면
전부 무영향**)해 `RowButtonRect`·`RowDescRect`·`Info`·`Row`/`Locked` 설명칸이 읽게 하고, `DrawAttributes`만 시작에서
46/40으로 낮췄다가 끝에서 되돌린다(같은 화면 index 그리드 경로 DrawAdvancement가 기본 76 기대 — 안 되돌리면 다음
프레임에 눌림). 하트 오프셋(하드코딩 RowH+RowGap)도 RowPitch 기준으로 고쳤다. **표시 전용 — 전투·밸런스·아트·온라인·
영지·W3Party 무접촉. `CharacterScreen.cs`(피치 설정·하트)·`GameScreen.cs`(공유 필드·헬퍼) 두 파일.** **통과 기준·증거**:
`game_compile_check.py` PASS(274소스 CS 0). unity_meas 배치 빌드 프레임 220(QA_JOB_TRAIT=1 수호기사 1차, 속성 탭) —
표제·목숨·HP스탯·이동기·2차각성(잠김)·「직업 특성 — 전투 방패탱」·「보유 스킬 — 도발의 함성·성채 방패·최후의 보루」·
「종족 특성 — 불굴 …(전투당 1회)(발동 25%)」·「종족 정체성 — 안 죽고 버티며 쌓아올리는 생산가」 **9행 전부 패널 안에
렌더**(`panel_density_shots/qa_go:Character.png`). **네거티브 컨트롤(실측 스크린샷)**: meas 사본에서 `RowPitch/RowHt`를
...
폴리싱칸(캐릭터) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「캐릭터」라 `go:Character`를 `unity_meas`(오너가 `unity/` 6000.5.6f1 `-useHub`+임포트워커 PID
25198/25231/25249 점유 중 → `sync_meas.sh` 후 배치 빌드) 프레임 200 캡처. **결함**: 장비(장비 탭) 정보 칸의 첫 두 줄
(Lv·경험 + xp 막대)이 패널 금테 **밖**으로 삐져나가 장비/속성 탭 옆에 떠 렌더됐고, 목숨 하트는 정보 칸 중간(무기·없음
줄)에 라벨 없이 떠 겹쳤다(육안 확인, `qa_negctrl_pullup_spill.png`). **원인**: 직전 캐릭터 폴리싱(정보 칸 바닥을 금테
안쪽 `chrome.yMax`로 당김)이 창 높이 유지를 위해 top도 같은 만큼 끌어올리는 pull-up을 썼는데, 그 pull-up이 top을 패널
위(`stage.y`)보다 높게 밀어 첫 두 줄이 프레임 밖으로 나갔다. 게다가 「panel」 스프라이트의 ContentRect(`chrome`)는 바깥
스크롤워크 장식까지 여백으로 세 위·아래로 ≈50px씩 과하게 안쪽을 잡아, 정보 14줄이 chrome 높이(≈9줄)를 못 넘겨 아래
장비 슬롯·가방(§11)이 잘리는 밀도 문제도 있었다. **수리**: 정보 칸을 실제 **안쪽 얇은 금테 사각형**(≈`stage`와 `chrome`의
중간)에 맞춰 `Mathf.Lerp(stage,chrome, top 0.62·bottom 0.45)`로 잡고, 줄 간격을 22→20f로 좁혀 14줄(Lv·xp·전투력·상태·
장착 헤더·장비 6슬롯·가방§11)을 넘침 없이 담았다. 목숨 하트는 정보 칸 Lv 줄 오른쪽(`info.xMax-76`)으로 내려 헤더로
읽히게 했다. **표시 전용 — 전투·밸런스·아트·온라인·영지·W3Party 무접촉, `CharacterScreen.cs` 한 파일(정보 칸 지오메트리·
줄 간격·하트 위치만).** **통과 기준·증거**: `game_compile_check.py` PASS(274소스 CS 0). 재캡처 프레임 200에서 Lv·경험·
...
코드칸(§5 이동기 프로필 화면 노출) 이번 이터 결과 — 화면 노출 완료 (루프, 2026-08-20): 직전 트랙 폴리싱(필드)이라
코드칸. 큐 맨 위 「다음 코드칸 후보」(§5 이동기 프로필 행이 캐릭터 속성 패널에서 안 보임)를 닫았다. **원인**: `Info`는
넘치면 조용히 건너뛰는데(GameScreen.cs:485 `panel.yMax > r.yMax`), 건강한 1차 캐릭터 속성 패널이 header(0)·목숨(1)·
StatLine(2)·전직(3)·ConceptLine(4)·SkillLine(5)로 6행이 꽉 차 §5 이동기 프로필(형태·거리·무적·쿨)은 행 6이라 **항상
넘쳐 안 보였다** — 배선(d8393369)은 맞고 화면만 죽어 있던 밀도 regression. **수리**: 프로필 행을 ConceptLine·SkillLine
**앞**, StatLine 다음 우선존으로 끌어올렸다(무적=원장 379 「조작의 핵심 기술」이라 직업특성·보유스킬보다 우선). 여유가
없으면 대신 하위 우선순위(SkillLine·종족 줄)가 밀린다. 프로필 한 행 안에서도 §5 두 조각(형태·거리/무적/쿨)을 맨 앞으로
몰아, LabelClip(우측 잘림)이 걸려도 §5는 온전하고 리스크(§4)·자동사냥(§6)만 잘리게 순서를 바꿨다. **표시 전용 — 전투·
밸런스·아트·W3Party 무접촉. `CharacterScreen.cs` 한 파일(+26 −28).** **통과 기준·증거**: `game_compile_check.py` PASS
(274소스 CS 0). `AshesToStars.MobilityDistanceSelfCheck.Run` 배치 **PASS**(배선 회귀 없음, `mobility_visible_selfcheck.log`).
시각 QA `mobility_visible_shots/qa_go:Character.png`(QA_JOB_TRAIT=1 수호기사, 프레임 220) — 속성 패널 3행째에 「이동기 —
방패 돌진 · 거리 3초분 · 무적 0.3초 · 쿨 6초 · 사망 리스크 — 최저 · 자동사냥」이 **잘리지 않고**(§5 전부 온전, §6만 우측
클립) 렌더. **네거티브 컨트롤(실측 스크린샷)**: 프로필 Info 인덱스를 `statusMax+1`→`9`(패널 밖)으로 되돌리고 재빌드하면
...
`폴리싱 다음`이 「필드」. 에디터가 원본 unity/를 6000.5.6f1 `-useHub`+임포트워커(PID 25198/25231/25249)로
점유 중 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`, `GAME_START=hunt` 프레임 300 캡처.
**결함**: 전투HUD 좌측 상단 요약 패널(웨이브·편성 파티 / 처치·경과초) 두 줄이 세로로 잘려 뭉개져 안 읽혔다 —
패널이 `Rect(14,14,218,50)`인데 `UiAtlas.ContentRect`가 짧은 축(50)의 24%≈14px를 위아래로 먹어 본문이 22px만
남고, 그걸 두 줄로 반 나누면 **각 줄 11px**가 돼 16px Bold 글자의 위아래가 잘렸다(같은 `_cmdLabel` 스타일의
단줄 우측 「스킬 자동」은 22px 칸이라 선명 — 지렛대는 줄 높이였다). **수리**: 패널 높이 50→**84px**로 키워
각 줄이 ≈20px가 되게 함(`DrawCombatSummary` 한 줄, `W3Party.cs:3446`). 바탕·금테·본문이 모두 이 `left` rect에서
파생해 함께 커지므로 글씨가 어두운 바탕 안에 온전히 들어간다. **전투HUD 크롬(글씨·여백)만 — 전투 수치·AI·경제·
아트·W3Party 전투 로직 무접촉.** **통과 기준·증거**: 재캡처에서 「12-12 · 편성 파티」·「처치 1 · 0s」 두 줄이
잘리지 않고 또렷이 렌더(전에는 세로 잘림으로 뭉갬). **네거티브 컨트롤(코드에 명시)**: 높이를 84f→50f로 되돌리면
두 줄이 다시 11px로 잘려 뭉갠다. 증거 `output/qa/ashes-to-stars/field_polish_shots/{qa_hunt.png(after 전체),
toplabel_after_legible.png(after 확대),toplabel_before_clipped.png(before 확대)}`. 파일 `Scripts/W3Party.cs` 한 파일.
**⚠️ 손대지 않은 것(정직 인계)**: 우측 보상 칩(골드/경험·연속·분당) 소자·반투명은 그대로, 화면 우중앙 회색
반투명 상자(`fx_death` 흰종이 계열, 다른 세션 미커밋 아트 하류)·중앙 콜아웃 겹침(직전 이터 104c73bd에서 계단
분리 방금 손봄)은 한 이터 한 결함 원칙 + 아트 오염/최근수정 회귀 위험으로 **건드리지 않았다**.
**다음 이터는 코드칸(마지막 트랙=폴리싱) — 폴리싱 다음이 「캐릭터」.**

다음 코드칸 후보 (정직 인계):
- **캐릭터 속성 패널 밀도 상한** — 이번 이터로 §5 프로필은 우선존 승격으로 살렸지만, 패널이 여전히 6행뿐이라
  하위 우선순위(SkillLine 보유 스킬·종족 특성·종족 정체성)는 1차+ 캐릭터에서 넘쳐 안 보인다(패널 세로 확장 또는
  스크롤·탭 분할이 근본 수리). 화면 노출 필요 시 큐로 올릴 것. **통과 기준**: 1차 캐릭터 속성 탭에서 SkillLine·
  종족 줄이 모두 렌더. **네거티브 컨트롤**: 패널 높이/스크롤을 되돌리면 다시 넘쳐 사라진다.
- grep 소비처 0곳 재탐색(정의만 있고 부르는 곳 없는 ✅ 항목) — 전투 디렉터리(`Assets/Scripts/W3Party.cs`)까지 반드시
  훑을 것(2026-08-20 실측: Explore가 전투 디렉터리를 빼먹어 이미 소비 중인 `RaceDef.이속배율`을 오진했다).

코드칸(§5 이동기거리) 이전 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱(영지)이라
코드칸(기획서 ✅ · 소비처 0곳). Explore 서브에이전트 전수 grep으로 후보 3종을 받되 **1차 후보(RaceDef.이속배율)는
검증에서 오진을 잡아 기각** — Explore가 `Assets/Scripts/`(전투)를 안 봐 "소비처 0곳"이라 했지만 `W3Party.cs:750`이
`_bSpd *= def.이속배율`로 이미 소비 중이었다(교훈: 소비처 grep은 전투 디렉터리까지 반드시 훑는다). 후보 C(MobDef
원거리 유지거리·발사간격·탄속)는 전투 AI 통합이라 코드칸 범위 밖. **채택**: `JobDef.이동기거리`(정의 JobDef.cs:29
default 3f)가 진짜 grep 소비처 0곳 — §5 이동기 기본 사양 표(원장 374 「이동 거리 | 캐릭터 기본 이동 3초분」,
헤더 369 ✅ 오너 결정 2026-08-13) 1행인데, 형제 무적·쿨(5cc0a8ee)·형태(MovementLine)는 배선됐어도 이 거리만
죽어 있었다. **수리**: `JobInfo.MobilityStatLine`이 거리를 무적·쿨 **앞**에 붙여 「거리 3초분 · 무적 0.3초 · 쿨 6초」
(표 순서 거리→무적→쿨, 형제 substring 보존). **표시 전용 — 전투·밸런스·아트·W3Party 무접촉.** **통과 기준·증거**:
`game_compile_check.py` PASS(274소스 CS 0). unity_meas 배치 `MobilityDistanceSelfCheck.Run` **전항 PASS** — 11직업
전부 MobilityStatLine이 `이동기거리` 필드값을 읽어 「거리 {값}초분」 렌더·거리가 무적 앞·형제 무적/쿨 조각 보존·
모르는직업 빈문자열·CharacterScreen이 MobilityStatLine 소비. **네거티브 컨트롤(실측)**: meas 사본에서 거리 append
줄을 지우면 **FAIL 22건 exit 1**, 문자열이 「무적 0.3초 · 쿨 6초」로 회귀(`mobility_distance_negctrl.log`).
증거 `output/qa/ashes-to-stars/shots/mobility_distance_{selfcheck,negctrl}.log`. 파일 `JobInfo.cs`·신규
`Editor/MobilityDistanceSelfCheck.cs`(+meta guid 34383f23…). **⚠️ 화면 노출 정직 인계**: 시각 QA
`mobility_distance_shots/qa_go:Character.png`(QA_JOB_TRAIT=1 수호기사, 프레임 220)에서 프로필 행이 **안 보였다** —
속성 패널이 6행(0~5)만 담아 프로필(행 6)이 넘친 **기존 밀도 한계**(형제 무적/쿨·형태도 같이 죽어 있음, 이번 배선이
유발 아님). 배선 자체는 SelfCheck로 결정적 검증됨. §5 이동기 표시행 화면 노출은 위 「다음 코드칸 후보」에 통과
기준·네거티브 컨트롤과 함께 큐로 올렸다. **아트 오염 주의**: fx/bg png·ui metas·`TextureImportRules.cs`·`import_p22_bw.py`는
다른 세션 미커밋 텍스처 작업 — 건드리지 않았다. **다음 이터는 폴리싱 칸(마지막 트랙=코드) — 폴리싱 다음이 「필드」.**

폴리싱(영지) 이전 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「영지」(GAME_SPEC_ESTATE_BUILD.md 있어 허용). `unity/`는 오너가 6000.5.6f1
`-useHub`+임포트워커(PID 25198/25231/25249)로 점유 중 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·
`UNITY_BIN=6000.5.6f1`, `go:Estate` 프레임 260 캡처(방어 탭). **결함**: 방어 팔레트 4카드(화살탑·마법탑·
성벽·함정)의 이름 라벨이 안 읽혔다 — `DrawVillagePalette`가 라벨을 14px 띠(`inner.yMax-14,h14`)에 넣어
`LabelFit`이 3글자 이름을 폭 68·높이 14 안에 맞추려 12px 밑으로 줄였고, 반투명 카드+어두운 배경에서
「원 사자/미 브버」처럼 뭉갰다(오너 반복 지적 「안 읽히는 글씨」 계열). **수리**: 아이콘 상한을 22→16px로
줄이고(`inner.height-20`) 라벨 띠를 14→18px로 넓혀 LabelFit이 ~15px로 렌더. **표시 좌표만 — 전투·경제·
AI·아트·W3Party 무접촉**(EstateHud PaletteTiles·PaletteH·TileW 불변, 로컬 리터럴만). **통과 기준·증거**:
qa_shot 배치 빌드 `error CS` 0(빌드 가드 ✅ 1032670 bytes), 재캡처에서 「화살탑 1·마법탑 1·성벽 1·함정 1」이
또렷이 렌더. **네거티브 컨트롤**: 라벨 띠를 `inner.yMax-14,h14`로 되돌리면 LabelFit이 다시 ~12px로 줄여
이름이 뭉갠다(`qa_go:Estate_before.png` 상태 — 이번 이터 편집 전 빌드로 실측). 증거 `output/qa/ashes-to-stars/
estate_polish_shots/{qa_go:Estate.png(after),qa_go:Estate_before.png}`. `EstateScreen.cs` 한 파일.
**아트 오염 주의**: fx/bg png·ui metas·`TextureImportRules.cs`는 다른 세션 미커밋 텍스처 작업 — 건드리지 않았다.
**다음 이터는 코드칸(마지막 트랙=폴리싱) — 폴리싱 다음이 「필드」.**

코드칸(§2 시작 직업 역할 설명) 이전 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱(타이틀)이라
...
바로 아래에 붙어(`copy.y`), 좌측 열(폭 0.54) 아래로 ~500px가 통째로 비어 화면이 위로 쏠렸다 — 오른쪽
3카드는 열 전체 높이를 채우는데 왼쪽만 상단 몰림(오너 반복 지적 「넓은 화면 빈 여백」 계열, 첫 캡처
`qa_go:Title_before.png` 육안). **수리**: `TitleScreen.Body`에서 소개 블록(titleH 36+gap 8+descH 90=134)을
좌측 열 세로 중앙(`by = copy.y + (copy.height-134)/2`)에 놓아 오른쪽 중앙 카드와 높이를 맞췄다. 헤더·카드·
StarterPick 경로 무변경, **표시 좌표만**(전투·AI·W3Party·아트 무접촉). **통과 기준·증거**: qa_shot 배치 빌드
`error CS` 0(빌드 가드 통과, ✅ 486636 bytes), 재캡처에서 소개 copy가 열 세로 중앙(이어하기 카드 높이대)에
렌더되고 상단 void 사라짐. **네거티브 컨트롤**: `by`를 `copy.y`로 되돌리면 copy가 다시 헤더 아래로 붙고
좌측 아래가 통째로 빔(`qa_go:Title_before.png` 상태). 증거 `output/qa/ashes-to-stars/title_polish_shots/
{qa_go:Title.png(after),qa_go:Title_before.png}`. `TitleScreen.cs` 한 파일. **아트 오염 주의**: fx/bg png·ui metas·
`TextureImportRules.cs`는 다른 세션 미커밋 텍스처 작업 — 건드리지 않았다. **다음 이터는 코드칸(마지막
트랙=폴리싱) — 폴리싱 다음이 「영지」(GAME_SPEC_ESTATE_BUILD.md 있어 허용).**

코드칸(정예 유형 거울 역할) 이전 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱(전투HUD)이라
코드칸(기획서 ✅ · 소비처 0곳). Explore 서브에이전트 전수 grep — 후보 3종(`MobDef.정예유형`·`MobDef.속도배율`·
`BalanceConfig.사망상한`). `MobDef.정예유형`은 asset이 런타임에 아예 로드되지 않아(W3Party가 하드코딩 트윈)
읽으려면 로드 경로 신설이 필요 → 범위 초과. 대신 그 필드가 대표하는 **§522 ✅(오너 결정 2026-08-13)
「정예 = 직업 역할의 거울」 표**의 대응직업·대응법이 코드 소비처 0곳이었다 — 던전 정예 노드 카드가 이름만
(「수호자 · 주술사」) 보여 정작 ✅ 핵심인 「무엇을 먼저 죽일 것인가」가 안 읽혔다. **수리**: `EliteKinds.RoleTip(EliteKind)`
= §522 표 6종 「대응직업 · 대응법」(수호자=탱·오라 밖 유인 / 처형자=딜·진로 차단 / 주술사=힐·최우선 처치 /
군단장=버퍼·2순위 처치 / 저주술사=디버퍼·사거리 밖 이탈 / 소환술사=소환사·본체 끊기) + `FormatWithRole`가
「수호자(탱 · 오라 밖 유인)」로 렌더, `EliteKinds.Caption`이 `Format`(맨이름) 대신 이걸 읽음. **표시 전용 —
전투 수치·AI·W3Party·아트 무접촉.** `Format`은 테스트가 정확 문자열을 핀하므로 바이트 불변, 거울은 별도 메서드로 추가.
**통과 기준·증거**: `game_compile_check.py` PASS(273소스 CS 0, --self-test 주입오류 탐지=예). meas 배치 빌드
성공 + `qa_dungeon.png` 던전 지도 렌더(헤더 「수호자 · 주술사(§10-2)」 시드 확인 — 정예 노드는 입구 인접이
아니라 카드 자체는 이 프레임에 안 뜸). Unity batchmode `EliteKindsSelfCheck.Run` **전항 PASS** — 카드 문자열
실측 「정예 아레나 · 수호자(탱 · 오라 밖 유인) · 주술사(힐 · 최우선 처치)(§10-2)」(시드 카드·더미 카드 동일).
...
폴리싱(전투HUD) 이전 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「전투HUD」라 `hunt`를 `unity_meas`(오너가 `unity/` 6000.5.6f1 `-useHub`+임포트워커 PID
25231/25249 점유 중 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`), 프레임 300 캡처.
**결함**: 여러 파티원이 같은 프레임에 스킬을 쓰면 `DrawCallouts`가 각 콜아웃을 시전자 월드 좌표(중앙
밀집)에만 놓아, 「도발의 함성/화염 폭풍/치유/교양」 4종이 같은 한 줄에 통째로 포개져 안 읽혔다(첫 캡처
육안 확인). **수리**: `W3Party.DrawCallouts`에 프레임별 `_calloutPlaced` rect 목록을 두고, 새 콜아웃이
이미 놓인 rect와 `Overlaps`면 `pr.yMax+2f`로 아래로 밀어(최대 12회, 밴드 하한 clamp) 세로 계단으로
흩는다. 클램프 밴드(상단 요약·하단 지휘 바 침범 금지)는 그대로. **전투 수치·AI 무변경**(콜아웃 표시
좌표만 — W3Party 크롬 규칙 준수). **통과 기준·증거**: meas 배치 빌드 `error CS` 0(exit 0), 재캡처
프레임 300에서 콜아웃 4종이 각자 한 줄씩 분리돼 모두 읽힘. **네거티브 컨트롤**: `_calloutPlaced` 겹침
검사를 빼면 4종이 다시 같은 좌표에 포개진다(첫 캡처 상태). 증거
`output/qa/ashes-to-stars/combat_hud_shots/qa_hunt_callout_stagger.png`(after). `W3Party.cs` 한 파일 —
전투 수치·AI·온라인·영지·아트 무접촉, 콜아웃 배치 좌표만. 코드 `104c73bd`.
**아트 오염 주의**: 전투HUD 화면의 `fx_death` 흰 종이 배경·스킬 슬롯 흰 테두리는 다른 세션의 미커밋
텍스처 작업(fx/bg png, ui metas 40여, 임포트 워커 가동) 하류라 **건드리지 않았다** — 위 헤더 ⚠️ 참조.
**다음 이터는 코드칸(마지막 트랙=폴리싱) — 폴리싱 다음이 「타이틀」.**

코드칸(§3·§4 직업 기본 스탯) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱(결과)이라
코드칸(기획서 ✅ · 소비처 0곳). Explore 서브에이전트 전수 grep — `JobDef` `[Header("기본 스탯")]` 블록
(최대체력·공격력·사거리·공격간격)이 「authored·committed인데 읽는 코드 0곳」 최적 후보. ProjectSetup이
11직업 전부 실값 authored하고(수호기사 320/10/1.3/0.7, 궁수 115/24/8/0.4, 검사 130/26/5.5/0.4 등 직업별로
전부 다름) Resources/jobs/*.asset에 committed까지 됐는데 grep 결과 참조처는 ProjectSetup 할당 라인
1곳뿐 — 전투 W3Party는 자체 상수표를 쓰고 asset 필드를 안 읽는다(W3Party.cs 사거리 히트는 전부 주석·
LifeSystemSelfCheck의 하드코딩 계약 배열). 컨셉·스킬·사망리스크·이동기·자동사냥적합도가 겪은 「정의만
있고 부르는 곳 0」 함정과 동일 계열. **수리**: `JobInfo.StatLine(jobName)` 추가(최대체력·공격력·사거리·
공격간격 읽어 「HP 320 · 공격력 10 · 사거리 1.3 · 공격 0.7초」, 매칭 직업 없으면 빈 문자열 — MobilityStatLine과
동일 패턴). CharacterScreen **속성** 탭에 표시. **배치 위치**: 이동기 프로필·종족 특성보다 **앞**(목숨 status
바로 뒤, 전직 블록 앞)에 둠 — 속성 탭의 표제 데이터라 우선순위가 높아, 관례(새 조각 맨 뒤)와 달리 패널이
꽉 차도 항상 보이게 했다. 빈 값이면 statusMax를 안 밀어 종족 줄 여유를 안 뺏는다. **밸런스·전투 수치
무변경**(표시 전용, committed 값 그대로 읽음). **통과 기준·증거**: `unity_meas`(오너 `unity/` 6000.5.6f1
`-useHub`+임포트워커 PID 25198 점유 중 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`)
배치 빌드 `error CS` 0(grep=0, exit 0), `GAME_START=go:Character QA_JOB_TRAIT=1`(수호기사 1차·속성 탭)
프레임 200 캡처 — 목숨 줄 바로 아래에 「HP 320 · 공격력 10 · 사거리 1.3 · 공격 0.7초」가 잘림·겹침 없이
렌더, 아래 직업 특성·보유 스킬 줄 무회귀. **네거티브 컨트롤**: CharacterScreen의 `stats` Info 호출을 빼면
그 줄이 사라져 `JobDef.최대체력·공격력·사거리·공격간격`이 다시 소비처 0곳(dead field). 증거
`output/qa/ashes-to-stars/jobstat_shots/qa_go:Character.png`. `JobInfo.cs`·`CharacterScreen.cs` 두 파일 —
전투 수치·AI·온라인·영지·W3Party 무접촉, 표시 문자열만. **남은 관찰(다음 코드칸 후보)**: JobDef.이동기거리
(=3)는 authoring조차 없어 default로만 남고 표시 의미 모호 — 후순위 유지. **다음 이터는 폴리싱 칸(마지막
트랙=코드) — 폴리싱 다음이 「전투HUD」.**

폴리싱(결과) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「결과」라 `go:Result`를 `QA_GEAR_DROP=1 QA_HUNT_GOLD=1`로 meas 배치 빌드(오너가 `unity/`
6000.5.6f1 `-useHub`+임포트워커 PID 25198 점유 중 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·
`UNITY_BIN=6000.5.6f1`), 프레임 200 캡처. **결함**: 보상 줄(RewardInfo)의 왼쪽 아이콘이 48px(`r.x+4`~
`r.x+52`)인데 글씨 자리를 「공백 7칸」으로만 비워, 공백폭(폰트당 ~7px×7≈49px)이 아이콘 폭보다 좁아
첫 글자 「획」이 아이콘에 먹혀 「…득 골드」·「…득: 고급 가죽 흉갑」으로 읽혔다(육안·크롭 확인). **수리**:
공백 프리픽스에 기대지 말고 `GameScreen.InfoAt`에 `absTextLeft` 오버로드를 추가해 글씨 시작 x를 절대
좌표로 고정 — `RewardInfo`가 아이콘 있을 때 `r.x+60`(아이콘 오른쪽 끝 `r.x+52`보다 8px 여유)부터 그린다.
아이콘 없는 줄은 `absTextLeft=0`이라 기존 content 칸 그대로(폭 0 회귀 없음). `Info`의 하단 가드
(`panel.yMax>r.yMax`)는 `RewardInfo`가 직접 재현해 선택 밴드 침범 방지 유지. **밸런스·전투 수치 무변경**
(표시 좌표만). **통과 기준·증거**: meas 배치 빌드 `error CS` 0(exit 0), 재캡처 프레임 200에서 「획득 골드:
1골드」·「획득: 고급 가죽 흉갑(§10-8)」이 아이콘에 안 먹히고 첫 글자부터 온전히 읽힘, 보상 줄이 「계속/영지로」
카드와 겹치지 않음. **네거티브 컨트롤**: `RewardInfo`의 `textLeft`를 `0f`로(또는 옛 `"       "+text`로)
되돌리면 아이콘이 다시 「획」을 덮어 「…득」으로 잘린다. 증거
`output/qa/ashes-to-stars/result_polish_shots/qa_go:Result.png`(after). `GameScreen.cs`·`ResultScreen.cs`
두 파일 — 전투 수치·AI·온라인·영지·W3Party 무접촉, 보상 줄 글씨 좌표만. **다음 이터는 코드칸(마지막 트랙=
폴리싱) — 폴리싱 다음이 「전투HUD」.**

코드칸(§5 이동기 무적·쿨) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱(던전)이라
코드칸(기획서 ✅ · 소비처 0곳). Explore 서브에이전트 전수 grep — ProjectSetup이 authored하는 필드는
이제 전부 소비처≥1(RaceInfo/JobInfo/StyleScreen 배선으로 닫힘). 남은 죽은 필드는 「직렬화되나
ProjectSetup이 안 채워 기본값으로만 committed되고 읽는 코드 0곳」 부류. 후보 3: ①JobDef 이동기 수치
(무적시간·이동기쿨·이동기거리) ②RaceDef.전투당발동 ③CombatStyleDef 토글 2종. **②는 제외**: Race_드워프.asset
고유메커니즘 문자열이 이미 「…생존 (전투당 1회)」를 품고 있어(디코드 확인) MechanicLine에 전투당발동을
붙이면 중복. **③도 제외**: 정예우선타겟은 W3Party 전투 AI를 건드려야 실작동(§10-2)이라 위험, 소모품자동사용은
§4가 항상 false로 못박음. **①선택**: `무적시간=0.3·이동기쿨=6`은 Resources/jobs/*.asset 전 직업 동일 committed
(디코드 확인, §5는 직업별로 이동기 *형태*만 변주·수치 공통)인데 읽는 코드 0곳이었다 — 형제 필드 이동기형태는
MovementLine에서 읽는데 두 수치만 죽어 있었다. §5 스펙 표(원장 376·377 ✅ 오너 결정 2026-08-13)의 무적 프레임
0.3초(원장 379 「이 게임 조작의 핵심 기술」)·쿨다운 6초가 화면엔 이동기 *형태*만 있고 정작 안 보였다. **수리**:
`JobInfo.MobilityStatLine(job)` 추가(무적시간·이동기쿨 읽어 「무적 0.3초 · 쿨 6초」, 매칭 직업 없으면 빈 문자열).
CharacterScreen 속성 탭 프로필 행(이동기 형태·사망 리스크·자동사냥과 같은 한 행)에 **맨 뒤**로 append.
**맨 뒤에 둔 이유**: InfoAt은 LabelClip(좌측 정렬·우측 잘림, GameScreen.cs:500)이라 행이 넘치면 뒤 조각부터
잘린다 — 무적·쿨을 뒤에 두면 여유 없을 때 이것만 잘리고 이미 보이던 형태·리스크·자동사냥 조각은 안 밀려난다
(정체성이 MechanicLine 뒤인 것과 같은 우선순위 원칙). **밸런스·전투 수치 무변경**(표시 전용, committed 값 그대로).
**통과 기준·증거**: `unity_meas`(오너 `unity/` 6000.5.6f1 `-useHub`+임포트워커 PID 25198 점유 중 → `sync_meas.sh`
후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`) 배치 빌드 `error CS` 0(`build_game.log` grep=0, exit 0),
`GAME_START=go:Character QA_JOB_TRAIT=1`(수호기사 1차·속성 탭) 프레임 200 캡처 — 프로필 행이
「이동기 — 방패 돌진 · 사망 리스크 — 최저 · 자동사냥 하 · **무적 0.3초 · 쿨 6초**」로 잘림·겹침 없이 렌더,
앞 조각(형태·리스크·자동사냥) 무회귀. **네거티브 컨트롤**: CharacterScreen의 `mobility` append를 빼면
「무적 0.3초 · 쿨 6초」가 사라져 `JobDef.무적시간·이동기쿨`이 다시 소비처 0곳(dead field). 증거
`output/qa/ashes-to-stars/mobility_shots/qa_go:Character.png`. `JobInfo.cs`·`CharacterScreen.cs` 두 파일 —
전투 수치·AI·온라인·영지·W3Party 무접촉, 표시 문자열만. **남은 관찰(다음 코드칸 후보)**: JobDef.이동기거리
(=3, 「이동 3초분」)는 여전히 소비처 0곳이나 표시 의미가 모호(전투 조작에 안 드러남) — 후순위.
**다음 이터는 폴리싱 칸(마지막 트랙=코드) — 폴리싱 다음이 「결과」.**

폴리싱(던전) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「던전」이라 `dungeon` 모드(DungeonRun.Begin→GameFlow.Dungeon 노드 맵)를 `unity_meas`
(오너가 `unity/` 6000.5.6f1 `-useHub`+임포트워커 점유 중 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·
`UNITY_BIN=6000.5.6f1`) 배치 빌드, 프레임 200 캡처. **결함**: 다음 노드가 2장(전투·던전포기)뿐일 때
노드 그리드 밴드가 `r.height-80`(본문 전 높이 ~560px)라 카드가 본문 전체로 늘어나, `CardLayout`의 tall
분기(아이콘+제목+부제 묶음 세로 중앙)만 카드 한가운데 뜨고 카드 속 나머지가 텅 빈 화면이 됐다(육안
확인, 던전 벽 배경이 통째로 가려짐). **수리**: `DungeonScreen.Body`의 노드 그리드 밴드 높이를 행당 상한
300px(`rows*300 + (rows-1)*12`)로 잡고 본문 세로 가운데 정렬(`band = Rect(r.x, r.y+80 + (availH-bandH)*0.5,
r.width, bandH)`). 2장이면 카드가 ~630×288 → `IsWideCard`(aspect 2.19>1.45) 가로형으로 서서 아이콘 왼쪽·
글씨 오른쪽으로 게임 나머지 허브 카드와 같은 비율로 읽히고, 위·아래로 던전 배경이 보인다. **강화 3택
(HuntBoon.PickBand)·「이 방향 끝」·「던전 나가기」 경로는 무변경** — 이번 결함(노드 맵 2장 케이스)만.
**통과 기준·증거**: meas 배치 빌드 `error CS` 0(qa_shot의 `build_game.log` grep 게이트 통과, exit 0),
재캡처 프레임 200에서 두 카드가 ~288px 가로형으로 본문 가운데 서고 던전 벽 배경이 카드 위·아래로 보인다.
**네거티브 컨트롤**: 밴드를 `new Rect(r.x, r.y+80f, r.width, r.height-80f)` 전폭으로 되돌리면 카드가 다시
본문 전 높이(~560px)로 늘어나 아이콘 묶음만 가운데 뜨고 속이 텅 빈다. 증거
`output/qa/ashes-to-stars/polish_dungeon_shots/qa_dungeon.png`. `DungeonScreen.cs` 한 파일 — 전투 수치·AI·
온라인·영지·W3Party 무접촉, 노드 맵 밴드 지오메트리만. **다음 이터는 코드칸(마지막 트랙=폴리싱) — 폴리싱은
그 다음 「결과」.**

코드칸(§18-9 종족 정체성) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱(월드맵)이라
코드칸(기획서 ✅ · 소비처 0곳). Explore 서브에이전트가 4개 Def SO의 필드를 전수 grep — `RaceDef.정체성`이
유일한 「authored인데 읽기 0곳」 최적 후보(형제 필드 고유메커니즘·고유발동확률·회복시간·배율 12종은 전부
살아있는 소비처 있음). **정체성**은 ProjectSetup.cs:134가 종족마다 authored(인간 "빨리 크고 빨리 회복하는
범용형"·엘프 "죽지 않으려 움직이는 유리대포"·드워프 "안 죽고 버티며 쌓아올리는 생산가"·수인 "먼저 덮치고
먼저 빠지는 사냥꾼")하고 Race_*.asset 4개에 committed(엘프 asset python 디코드 확인)돼 있으면서도
읽는 코드 0곳(dead field, grep 히트는 정의+ProjectSetup 쓰기+「게임의 정체성」주석뿐). **수리**: 형제
고유메커니즘의 유일 소비처인 `RaceInfo`에 `IdentityLine(RaceId)` 추가(정체성 비면 빈 문자열, 지어내지
않음) — CharacterScreen 속성 탭에서 **MechanicLine 뒤에** 별도 한 줄로 소비(라인 404 다음). **뒤에 둔 이유**:
Info는 새 행이 `r.yMax`를 넘치면 조용히 건너뛰는데(GameScreen.cs:485), 정체성을 종족 특성 줄 앞에 두면
이미 잘 그려지던 MechanicLine을 한 칸 밀어 잃을 수 있다(회귀). 뒤에 두면 여유 없을 때 정체성만 빠지고
MechanicLine은 안전 — 정체성이 더 낮은 우선순위(계정 기본직 캐릭터는 위 직업 줄이 비어 항상 보임).
긴 메커니즘 설명 뒤에 이어붙이지 않고 별도 줄로 낸 이유는 LabelClip 가로 잘림 회피(정체성이 안 보이게 됨).
**밸런스·전투 수치 무변경**(표시 전용). **통과 기준·증거**: `unity_meas`(오너 `unity/` 6000.5.6f1
`-useHub`+임포트워커 점유 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`) 배치 빌드
`error CS` 0(`build_game.log` grep=0, exit 0), `GAME_START=go:Character QA_RACE_TRAIT=1`(계정 종족 드워프·
기본직) 프레임 200 캡처 — 「종족 특성 — 불굴 … (발동 25%)」줄이 **여전히** 렌더되고 그 아래 「종족 정체성 —
안 죽고 버티며 쌓아올리는 생산가」가 잘림·겹침 없이 추가로 렌더(드워프 authored 정체성과 일치). MechanicLine
회귀 없음. **네거티브 컨트롤**: `IdentityLine` 소비(`if (!string.IsNullOrEmpty(raceIdent))`)를 빼면
「종족 정체성 …」줄이 사라져 `RaceDef.정체성`이 다시 소비처 0곳(dead field). 증거
`output/qa/ashes-to-stars/race_ident_shots/qa_go:Character.png`. `RaceInfo.cs`·`CharacterScreen.cs`
두 파일 — 전투 수치·AI·온라인·영지·W3Party 무접촉, 표시 문자열만. **남은 관찰(다음 코드칸 후보)**: JobDef
기본 스탯(최대체력·공격력·사거리)이 authored인데 소비처 0곳이나 W3Party가 소유한 실전투 수치와 중복이라
표시하면 모순 위험 — 배선 시 W3Party 실값과 대조 필요(2순위, 신중). MobDef는 SO 자체가 절차 스폰이라
사실상 미사용(MobInfo 경로 없음).
**다음 이터는 폴리싱 칸(마지막 트랙=코드) — 폴리싱 다음이 「던전」.**

폴리싱(월드맵) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「월드맵」이라 `go:WorldMap`을 `unity_meas`(에디터가 `unity/` 6000.5.6f1 `-useHub`+임포트워커
점유 중 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`) 배치 빌드, 프레임 200 캡처.
**결함**: 별 상태줄 「내 별 · 1층 · 별 ×1.02 · 영공 1.1」이 화면에 **두 번** 렌더 — ①헤더 부제(`Subtitle`,
line 27 `"내 별 " + SizeLabel`)와 ②그 바로 아래 배너 플레이트(`Body`, line 100 `starCap = "내 별 · " + SizeLabel
+ " — 층을 오를수록 커진다(§14)"`)가 같은 SizeLabel을 각각 붙여, 헤더 밑에 동일 문구가 세로로 겹쳐 보였다
(오너 지적 「중복」, 육안 확인 crop). **수리**: 배너 `starCap`을 SizeLabel 없는 순수 §14 설명
「층을 오를수록 내 별이 커진다(§14)」로 축약 — 수치(별 크기·영공)는 헤더 부제에 위임하고, 배너는 옆의
별 아이콘(층마다 커짐)의 규칙 설명만 맡는다. 헤더 부제·`SizeLabel`·`SizeLine`은 **무변경**(엘프 인식
+20% 접두는 유지). **통과 기준·증거**: meas 배치 빌드 `error CS` 0(`build_game.log` grep, exit 0), 재캡처
프레임 200에서 배너가 「층을 오를수록 내 별이 커진다(§14)」만 렌더하고 헤더는 여전히 「내 별 1층 · 별 ×1.02 ·
영공 1.1 · 드워프 골드 소모 −20%(§18-9) · 침략은 탑 30층(§14·§15)」 전량을 보여 — 별 상태줄이 한 번만 나온다.
**네거티브 컨트롤**: `starCap`을 `"내 별 · " + WorldStar.SizeLabel(...) + " — 층을 오를수록 커진다(§14)"`로
되돌리면 헤더 부제와 배너에 「내 별 · 1층 · 별 ×1.02 · 영공 1.1」이 다시 두 번 겹친다. 증거
`output/qa/ashes-to-stars/polish_worldmap_shots/qa_go:WorldMap.png`. `WorldMapScreen.cs` 한 파일 — 전투 수치·AI·
온라인·영지·W3Party 무접촉, 표시 문자열만. 코드 `56d9b6f1`. **남은 관찰**: 배너 플레이트는 여전히 body 전폭인데 내용이 왼쪽
아이콘+한 줄뿐이라 오른쪽 절반이 빈 여백 — 이번 결함 아님(중복 우선), 다음 월드맵 폴리싱 후보(전폭→내용폭 축소).
**다음 이터는 코드칸(마지막 트랙=폴리싱) — 폴리싱은 그 다음 「던전」.**

코드칸(§6 자동사냥적합도) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱이라
코드칸(기획서 · 소비처 0곳). 직전 이터가 남긴 후보 `JobDef.자동사냥적합도`를 잡음 — grep 유일 히트가
JobDef.cs 정의(11종 authored: 소환사 0.95·마법사 0.90·검사 0.70·광전사 0.65·궁수 0.50·정령사 0.45·
드루이드 0.40·수호기사 0.35·주술사 0.30·음유시인 0.25·사제 0.20) + ProjectSetup 쓰기뿐, **읽기 0곳**
(dead field). **수리**: 형제 필드들(사망리스크·이동기형태)과 같은 표시 전용 패턴 — `JobInfo.AutoHuntLine`이
자동사냥적합도(float 0~1)를 정성 밴드 「최상/상/중/하」로 반환, CharacterScreen 속성 탭 profile 행에
**별도 행 없이** 덧붙임(새 행은 종족 특성 줄을 r.yMax 밖으로 밀어 잃게 함, line 382 주석). 밴드 임계값은
원장 line 204 💡의 명시 등급(소환사·마법사 최상 / 검사·광전사 상 / 궁수 중 / 힐·버퍼 하)에 authored 11종이
전부 맞도록: ≥0.85 최상·≥0.60 상·≥0.50 중·그 아래 하 — 지어낸 게 아니라 문서 등급을 값에 되짚음.
**밸런스 수치 무변경**(표시 전용, 자동사냥 골드/AI 무접촉). **통과 기준·증거**: `unity_meas`(오너 `unity/`
6000.5.6f1 `-useHub`+임포트워커 점유 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·UNITY_BIN 6000.5.6f1)
배치 빌드 error CS 0(exit 0), `GAME_START=go:Character QA_JOB_TRAIT=1`(수호기사 1차 전직·속성 탭) 프레임 200
캡처 — profile 행이 「이동기 — 방패 돌진 · 사망 리스크 — 최저 · 자동사냥 하」로 잘림·겹침 없이 렌더,
수호기사 authored 0.35(밴드 하)와 일치. 종족 줄 밀림 회귀 없음. **네거티브 컨트롤**: profile 덧붙임
(`if (!string.IsNullOrEmpty(hunt))`)을 빼면 「· 자동사냥 하」 조각이 사라져 필드가 다시 소비처 0곳(dead field).
증거 `output/qa/ashes-to-stars/auto_hunt_shots/qa_go:Character.png`. `JobInfo.cs`·`CharacterScreen.cs`
두 파일 — 전투 수치·AI·온라인·영지·W3Party 무접촉, 표시 문자열만.
**남은 관찰(다음 코드칸 후보)**: 없음 확인 필요 — 다음 코드칸은 grep으로 소비처 0곳 재탐색.
**다음 이터는 폴리싱 칸(마지막 트랙=코드) — 폴리싱 다음이 「월드맵」.**

폴리싱칸(파티) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「파티」라 `go:Party`를 `unity_meas`(오너가 `unity/` 6000.5.6f1 `-useHub`+임포트워커
점유 중 → `sync_meas.sh` 후 배치 빌드) 프레임 200 캡처. **결함**: 파티 카드 5장 전부 역할 뱃지가
초상 프레임 우하단 모서리 **밖으로 ~10px 흘러내려** 프레임에 매달린 것처럼 보였다 — `PartyScreen.DrawPartyCard`가
뱃지를 `faceR.xMax-8, faceR.yMax-8`부터 20px로 그렸는데 프레임 바깥 테두리는 `faceR`+2px라 우/하로
넘쳤다(육안 확인, 카드 크롭 5장). **수리**: 뱃지 Rect를 `faceR.xMax-22, faceR.yMax-22, 20, 20`로 당겨
프레임 안쪽 우하단 모서리에 앉힘(초상 위 표준 코너 뱃지). **통과 기준·증거**: 배치 빌드 error CS 0(exit 0),
재캡처 프레임 200에서 탱커 방패·물리딜러 교차검·마법딜러·힐러·서포터 뱃지가 전부 프레임 안에 들어가 렌더
(`party_card1`·`party_card2` 크롭 확인). **네거티브 컨트롤**: 첫 캡처에서 5장 전부 뱃지가 프레임 밖으로
매달린 것을 육안 확인 → 뱃지 Rect를 `faceR.xMax-8, faceR.yMax-8`로 되돌리면 다시 프레임 밖으로 흘러내린다.
증거 `output/qa/ashes-to-stars/polish_party_shots/qa_go:Party.png`. `PartyScreen.cs` 한 파일 — 전투 수치·AI·
온라인·영지·W3Party 무접촉, 뱃지 지오메트리만. **남은 관찰**: 형제 `UiAtlas.DrawRosterMarks`(line 454)도
같은 `faceR.xMax-8` 공식이라 다른 화면 로스터 행에서 같은 흘러내림 가능 — 이번 결함(파티) 아님, 다음 폴리싱 후보.
**다음 이터는 코드칸(마지막 트랙=폴리싱).**
...
점유 중 → `sync_meas.sh` 후 배치 빌드) 프레임 200 캡처. **결함**: 장비(장비 탭) 정보 칸의 마지막 줄
「가방 0/60(§11)」이 패널 「panel」 금테(9-slice border ≈18~20px) 위에 올라타 겹쳤다 — 정보 Rect의 고정
하단 여백이 12px뿐이라 금테보다 얇았다(육안 확인, `char_bottom` 크롭). **수리**: `DrawEquipStudio`에서
정보 Rect 바닥을 `chrome.yMax`(=`ContentRect(stage,"panel")` 금테 안쪽)로 당기고, 당긴 만큼 위(`infoTop`)로
올려 **창 높이는 그대로** 유지 — 줄 수가 줄지 않아 `Line()` 넘침 가드(y+20>yMax)에 가방 줄이 걸려
사라지는 회귀가 없다(§11 소비처 보존). **통과 기준·증거**: 배치 빌드 error CS 0(exit 0), 재캡처 프레임 200에서
「가방 2/60(§11)」이 금테와 겹치지 않고 패널 안에 여백을 두고 렌더(`char_after` 크롭 확인). **네거티브 컨트롤**:
수정 전 첫 캡처에서 「가방 0/60(§11)」이 실제로 하단 금테 위에 올라탄 것을 육안 확인(before/after 두 샷 존재) →
바닥을 `stage.yMax-12`로 되돌리면 다시 금테에 겹친다. 증거
`output/qa/ashes-to-stars/polish_char_shots/qa_go:Character.png`. `CharacterScreen.cs` 한 파일 — 전투 수치·AI·
온라인·영지·W3Party 무접촉, 정보 칸 지오메트리만. **다음 이터는 코드칸(마지막 트랙=폴리싱).**

코드칸(§5 이동기형태) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱이라
코드칸(기획서 ✅ · 소비처 0곳). 서브에이전트 딥서치로 후보 3종(JobDef.이동기형태 / JobDef.자동사냥적합도 /
RaceDef.고유발동확률)을 조사 — 가장 깨끗한 갭 = **`JobDef.이동기형태`**: §5 이동기(원장 369「✅ 오너 결정
2026-08-13」·381 계열별 형태 표)의 확정 컬럼인데 ProjectSetup이 직업 11종 개별 authored(수호기사 방패 돌진·
궁수 백스텝·마법사 점멸·소환사 위치 교체 …)하고도 **읽는 코드 0곳**(grep 유일 히트가 JobDef.cs 정의 +
ProjectSetup 쓰기 2줄, 읽기 0). 행동설명이 겪은 이중 갭(에셋에 authored 값 미저장)은 **여기엔 없음** —
`Job_*.asset` 11개 전부 `이동기형태` 필드가 유니코드 이스케이프로 이미 저장·ProjectSetup과 일치(python
디코드로 11종 확인). **수리**: ① `JobInfo.MovementLine(jobName)` 추가 — RiskLine과 동일 패턴, 같은
`Resources.LoadAll<JobDef>("jobs")` 에셋에서 `d.이동기형태`를 읽어 「이동기 — {형태}」, 빈 값이면 미표시
(지어내지 않음). ② CharacterScreen 속성 탭에서 **이동기와 사망 리스크를 한 행에 합침** — Info 행이 76px로
커 패널이 ≈6행뿐이라, 이동기를 별도 행으로 두니 첫 빌드에서 기존 ✅ 소비처 `사망 리스크` 줄이 r.yMax
밖으로 밀려 **조용히 사라졌다**(Info는 넘치면 건너뜀, 실측 확인). 둘 다 한 토큰짜리 짧은 §4·§5 직업
프로필이라 「이동기 — 방패 돌진 · 사망 리스크 — 최저」로 합쳐 둘 다 보이고 종족 특성 줄 여유도 생김.
**통과 기준·증거**: `unity_meas`(오너 `unity/` 6000.5.6f1 `-useHub`+임포트워커 점유 → `sync_meas.sh` 후
`GAME_PROJ=unity_meas`·UNITY_BIN 6000.5.6f1) 배치 빌드 error CS 0(qa_shot 컴파일 오류 시 exit 1인데 exit 0),
`GAME_START=go:Character QA_JOB_TRAIT=1`(수호기사 1차 시드) 프레임 200 캡처 — 속성 탭 마지막 행이
「이동기 — 방패 돌진 · 사망 리스크 — 최저」로 잘림·겹침 없이 렌더, Job_guardian.asset authored(방패 돌진·최저)와
일치. **네거티브 컨트롤**: 별도 행(합치기 전) 첫 빌드에서 사망 리스크가 실제로 사라짐을 육안 확인(합치기
전/후 두 샷 존재) → 합쳐야 둘 다 보임. MovementLine append를 빼면 「이동기 —」 조각이 사라져 필드가 다시
소비처 0곳(dead field). 증거 `output/qa/ashes-to-stars/job_movement_shots/qa_go:Character.png`.
`JobInfo.cs`·`CharacterScreen.cs` 두 파일 — 전투 수치·AI·온라인·영지 무접촉, 표시 문자열·행 배치만.
**남은 관찰(이번 결함 아님)**: `JobDef.자동사냥적합도`(§6 자동사냥, 11종 authored, 읽기 0곳)·`RaceDef.고유발동확률`
(4종 authored, 읽기 0곳, 종족 고유메커니즘 줄에 `(발동 25%)`로 합칠 후보)이 다음 코드칸 후보.
**다음 이터는 폴리싱 칸(마지막 트랙=코드) — 폴리싱 다음이 「캐릭터」.**

폴리싱(필드) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「필드」라 `go:Field`를 `unity_meas`(에디터가 `unity/` 6000.5.6f1 `-useHub` 점유 중 →
`sync_meas.sh` 후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`) 배치 빌드, 프레임 240 캡처.
**결함**: 필드 허브 2×3 카드 중 「일정 사냥」(cards[4]) 아이콘이 `FieldScreen.cs:199`에서
`HuntSchedule.Running ? "field" : "heart"`라, **일정 미가동(기본 상태)일 때 빨간 하트**를 표시.
바로 옆 「저체력 귀환 켜짐」(cards[3])도 하트라 **같은 행에 동일한 빨간 하트 2개**가 나란히 떠
읽는 사람이 두 카드를 혼동했다(하트=HP/목숨인데 일정 사냥은 HP와 무관 — 저체력 카드의
heart/heart_broken on/off 패턴을 복붙한 플레이스홀더 잔재). **수리**: 아이콘을 상태와 무관하게
`"field"`로 고정 — 가동 상태가 이미 쓰던 사냥 아이콘이고, 「사냥 시작」(cards[0])과 같은 계열이라
일관됨. 가동/정지 구분은 제목(`CardTitle()`)·부제가 이미 전달한다(atlas에 시계/일정 아이콘 없음 확인).
**통과 기준·증거**: meas 배치 빌드 `error CS` 0(qa_shot이 컴파일 오류 시 exit 1인데 ✅ 빌드 완료·exit 0).
after 샷 `output/qa/ashes-to-stars/shots/qa_go:Field.png` = 「일정 사냥」이 금색 사냥 엠블럼(「사냥 시작」과
동일), 하트는 「저체력 귀환」 카드 한 장에만 남아 의미 일치, 나란한 이중 하트 소멸. **네거티브 컨트롤**:
`"field"`를 `HuntSchedule.Running ? "field" : "heart"`로 되돌리면 정지 상태에서 하트가 다시 떠
이중 하트 재발. `FieldScreen.cs` 한 파일 — 전투 수치·AI·온라인·영지 무접촉, 카드 아이콘 키만.
**다음 이터는 코드칸(마지막 트랙=폴리싱) — 폴리싱은 그 다음 「캐릭터」.**

코드칸(§3 전투 스타일 행동설명) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20, 커밋 `ef4b1026`):
직전 트랙 폴리싱이라 코드칸(기획서 ✅ · 소비처 0곳). 서브에이전트 딥서치로 후보 3종(CombatStyleDef.
행동설명 / RaceDef.정체성 / JobDef.무적시간)을 조사 — 정체성은 종족 정보 표시 화면이 아예 없어 표시
지점을 새로 놓아야 하고, 무적시간은 ⚠️ 사망 카운트 직접 개입(사망인접 보류)이라 탈락. **가장 깨끗한
갭 = `CombatStyleDef.행동설명`**: 원장 §3 스타일 표 「행동」열(GAME_DESIGN 212~217)의 ✅ 확정인데
ProjectSetup이 스타일마다 authored(공격형=회피 시도 안 함 …)하고도 **읽는 코드 0곳**(grep 유일 히트가
정의 + ProjectSetup 쓰기, 읽기 0). 게다가 커밋된 `Style_*.asset` 4개는 필드는 있으나 **값이 비어**
있어(authored 값이 asset에 저장된 적 없음) 배선만으론 화면에 안 나오는 이중 갭. **수리**: ① `StyleScreen.
StatLine`이 이미 `Resources.LoadAll<CombatStyleDef>("styles")`로 로드하는 같은 에셋에서 `d.행동설명`을
수치 아래 한 줄로 붙임(값 비면 지어내지 않고 미표시 — StyleScreen 독트린) ② `Style_{공격형,균형형,방어형,
생존형}.asset`에 ProjectSetup `r.Item6` authored 값 4개를 Unity \u 이스케이프로 채움(하드코딩 아님) ③
스타일 2×2 그리드 높이 `-240`→`-300` — 예전엔 2행 카드 바닥 50px이 하단 DrawChoice(이펙트 테스트·돌아
가기, r.yMax-168) 선택 바에 깔려, 수치 한 줄만 있을 땐 안 보이던 겹침이 행동설명 둘째 줄을 잘랐다.
**통과 기준·증거**: `unity_meas`(오너 `unity/` 6000.5.6f1 `-useHub`+임포트워커 점유 → `sync_meas.sh` 후
`GAME_PROJ=unity_meas`·UNITY_BIN 6000.5.6f1) 배치 빌드 error CS 0(qa_shot 컴파일 오류 시 exit 1인데 exit 0),
`GAME_START=style` 프레임 220 캡처 — 4개 스타일 카드 전부 「회피 시도 안 함, 적극 교전」·「위험 패턴만
회피」·「안전거리 유지, HP 30%에 후퇴」·「HP 50%에 즉시 이탈·귀환」이 수치 아래 잘림·겹침 없이 렌더,
ProjectSetup authored와 일치(지어내기 아님). **네거티브 컨트롤**: StatLine 행동설명 append를 빼거나
asset 필드를 비우면 줄이 사라져 필드가 다시 소비처 0곳(dead field). 증거 `output/qa/ashes-to-stars/
style_behavior_shots/qa_style.png`. `StyleScreen.cs` 한 파일 + `Style_*.asset` 4개 — 전투 수치·AI·온라인·
영지 무접촉, 표시 문자열·카드 치수만. **남은 관찰(이번 결함 아님)**: `RaceDef.정체성`(§종족 ✅, 읽기
0곳)은 종족 정보 화면이 없어 표시 지점 신설 필요 — 다음 코드칸 후보. `CombatStyleDef.정예우선타겟`은
💡 제안 + W3Party 타겟팅이라 화면 네거티브 컨트롤 불가, 대기. **다음 이터는 폴리싱 칸(마지막 트랙=코드)
— 폴리싱 다음이 「필드」.**

폴리싱(영지) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20, 커밋 `e912ffb8`): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「영지」라 `go:Estate`를 `unity_meas`(오너가 `unity/` 6000.5.6f1 `-useHub`+임포트워커
점유 중 → `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`) 배치 빌드, 프레임 240 캡처.
**결함**: 방어 배치 팔레트(마을 탭 하단 도크)의 타일 4개 중 **3글자 이름(화살탑·마법탑)이 잘려**
「화살」·「마법」만 보이고 `탑`과 개수(`4`)가 사라졌다(2글자 성벽·함정 4는 정상). 원인:
`EstateHud.PaletteTiles`의 타일 폭이 `Min(TileW, Max(36, r.height+8))`로 팔레트 높이(44)에 묶여
**52px로 눌려** 있었고, 라벨은 `Hint`→`UiPages.LabelFit`(_h2 wordWrap=true)이라 폭 초과 시 두 줄로
접혔는데 14px 라벨 칸이 둘째 줄을 잘라 `탑·개수`가 소실됐다(minFont 11 전에 12px에서 멈춰 폭이 여전히
초과). **수리**: 타일 폭을 높이에서 분리 — `TileW` 56→68, `tw = TileW`(고정)로 바꿔 3글자+개수가
한 줄에 들어가게 함(`EstateHud.cs` 한 파일). **통과 기준·증거**: meas 배치 빌드 `error CS` 0(qa_shot이
컴파일 오류 시 exit 1인데 ✅ 빌드 완료·exit 0). after 샷 `output/qa/ashes-to-stars/shots/qa_go:Estate.png`
= 타일 4개 전부 `화살탑 4·마법탑 4·성벽 4·함정 4` 완전 표시(잘림 없음), 도크·우측 배너와 겹침 없음,
가운데 정렬 유지. `EstateHudSelfCheck` 제약 수식 손계산 통과: `slim[0].width 68 ≤ TileW 68`,
`used 290 < 400` 및 `< bar.width(1208)×0.40=483`, `x0 495 > bar.x+200`. **네거티브 컨트롤**:
`tw`를 다시 `Min(TileW, Max(36, r.height+8))`(52px)로 되돌리면 화살탑/마법탑이 다시 `탑·개수`를 잃는다.
`EstateHud.cs` 한 파일 — 전투/AI/온라인/영지 배치 로직 무접촉, HUD 타일 치수만.
**다음 이터는 코드칸(마지막 트랙=폴리싱) — 폴리싱은 그 다음 「필드」.**

코드칸(직업 사망 리스크) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20): 직전 트랙 폴리싱이라
이번은 코드칸(기획서 ✅ · 소비처 0곳). 서브에이전트 딥서치로 후보 3종(JobDef.이동기쿨/거리, RaceDef.
고유발동확률/전투당발동, CombatStyleDef.소모품자동사용)을 조사한 뒤 **더 깨끗한 갭**으로 방향 수정:
이동기쿨/거리는 `MakeJobs`가 값을 안 실어 전 직업 기본 6/3이고 직업별 변주는 이미 `DashSpec`
하드코딩으로 구현돼 있어 **갭이 아니라 회귀 위험**(배선 시 §5 변주 평탄화·값 균일이라 네거티브 컨트롤
불가)이라 버림. 종족 3종은 전부 사망/위험 처리 인접(불굴·야성감각·소모품자동사용→LifeSystem)이라 보류.
대신 `.사망리스크`·`.자동사냥적합도`·`.이동기형태`가 전부 소비처 0곳(사망 무관)임을 grep으로 확인하고,
그중 **원장 §4 전직 표의 ✅ 확정 컬럼 「사망 리스크」**(GAME_DESIGN 169·176·185·192줄)에 대응하는
`JobDef.사망리스크`를 골랐다(자동사냥적합도는 204줄 💡 제안이라 탈락). 이 필드는 ProjectSetup이
직업마다 authored(수호기사 최저·광전사 높음·검사 중상…)하고도 **읽는 코드 0곳**(`grep -rniI 사망리스크`
유일 히트가 JobDef 정의 + ProjectSetup 쓰기, 읽기 0 — 컨셉·스킬이 겪은 「정의만 있고 부르는 곳 없다」
재발). **수리**: ① `JobInfo.RiskLine(jobName)` 추가 — `d.사망리스크`를 「사망 리스크 — {값}」으로 읽음
(못 읽으면 "" = StyleScreen 지어내기 금지 독트린), ② `CharacterScreen.cs:381` 스킬 줄 바로 뒤에
소비처 한 줄(속성 탭 DrawAttributes, 컨셉·스킬과 동일 자리). **통과 기준·증거**: `game_compile_check`
PASS(error CS 0). `unity_meas`(오너 `unity/` 6000.5.6f1 `-useHub`+임포트워커 점유 → `sync_meas.sh` 후
`GAME_PROJ=unity_meas`·UNITY_BIN 6000.5.6f1) 배치 빌드 error CS 0, `GAME_START=go:Character QA_JOB_TRAIT=1`
프레임 220 캡처 — 속성 탭에 `직업 특성 — 정통 방패탱…` · `보유 스킬 — 도발의 함성…` · **`사망 리스크 — 최저`**
3줄이 잘림·겹침 없이 렌더. 「최저」는 JobDef 수호기사 authored값과 일치(하드코딩 아님). **네거티브 컨트롤**:
QA_JOB_TRAIT 없는 기본 로스터 샷은 기본직 「탱커·탱」이 선택돼 `RiskLine("탱")=""`이라 사망 리스크 줄이
아예 안 뜬다(매칭 JobDef 없음). CharacterScreen 소비처 두 줄을 제거하면 줄이 사라져 필드가 다시
소비처 0곳(dead field)으로 복귀. 증거 `output/qa/ashes-to-stars/job_risk_shots/qa_go:Character.png`.
`JobInfo.cs`·`CharacterScreen.cs` 두 파일 — 전투 수치·AI·온라인·영지 무접촉, 표시 문자열만.
**남은 관찰(이번 결함 아님)**: `자동사냥적합도`(§6 자동사냥, 204줄 💡)·`이동기형태`도 소비처 0곳이나
전자는 💡 제안이라 확정 대기, 후자는 이미 W3Party 대시 라벨(방패 돌진/점멸)로 부분 표시됨. RaceDef
고유발동확률·전투당발동, JobDef 이동기쿨/거리는 각각 사망 인접·이미 구현이라 신중. **다음 이터는
폴리싱 칸(마지막 트랙=코드) — 폴리싱 다음이 「영지」(`docs/GAME_SPEC_ESTATE_BUILD.md` 참조).**


폴리싱(타이틀) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「타이틀」이라 `go:Title`을 `unity_meas`(오너가 `unity/` 에디터 6000.5.6f1 `-useHub`+
임포트워커 점유 중 — `sync_meas.sh` 후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1` 우회) 배치 빌드로
프레임 180에 캡처. **결함**: 헤더 부제(`Ashes to Stars — 죽으면 캐릭터가 진짜 사라진다`, TitleScreen.cs:24)와
본문 헤드라인 빈 상태 폴백(`죽으면 캐릭터가 진짜 사라진다`, :45)이 **같은 문장이라 타이틀 화면에 같은
문구가 두 번** 떴다. 본문 헤드라인 줄은 원래 획득 칭호(TowerEnding.TitleName·SoloRaidClear.LastTitle)를
보여주는 자리인데 빈 상태 폴백만 부제를 그대로 복제. **수리**: 폴백을 칭호 없음 빈 상태 문구
`아직 정상에 오른 파티가 없다`로 교체(칭호 획득 분기는 그대로). **통과 기준·증거**: meas 배치 빌드
`error CS` 0(qa_shot이 컴파일 오류 시 exit 1인데 ✅ 빌드 완료). before 샷은 좌측 헤드라인이 상단 부제와
동일 문장, after 샷 `output/qa/ashes-to-stars/shots/qa_go:Title.png`는 좌측이 `아직 정상에 오른 파티가 없다`로
부제와 갈라짐 — 잘림·겹침·흰종이·낡은텍스처 없음, 레이아웃 불변. **네거티브 컨트롤**: :45를
`죽으면 캐릭터가 진짜 사라진다`로 되돌리면 같은 문장이 다시 두 번 뜬다(중복 복귀). `TitleScreen.cs` 한 파일 —
전투/AI/온라인/영지/W3Party 무접촉. **다음 이터는 코드칸(마지막 트랙=폴리싱).**

코드칸(종족 방어배율) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20, 커밋 `739f89e8`): 직전 트랙
폴리싱이라 이번은 코드칸(기획서 ✅ · 소비처 0곳). 서브에이전트 딥서치로 **새 0곳 갭**: `RaceDef.방어배율`
(RaceDef.cs:23, §2/§18-9 ✅ 4종 특성 「엘프 방어 -20%」)이 `ProjectSetup.cs:136`에서 에셋에 authored되고도
**어떤 런타임 코드도 안 읽음** — 형제 필드 체력배율·이속배율은 `W3Party.cs:745`에서 읽어 파티 배율에
곱하는데 방어배율만 죽어 있었다(`grep -rniI 방어배율` 유일 히트가 RaceDef 정의 + ProjectSetup 쓰기 둘뿐,
읽기 0. CombatStyleDef·prop_scale·JobDef.스킬이 겪은 「정의만 있고 부르는 곳 없다」 재발). 수리:
① `_raceDmgTakenMul` 필드 추가, ② `ApplyRaceModifiers`에서 방어배율→받는 피해 배율 환산
(`Mathf.Clamp(2 - 방어배율, 0.1, 2)` — 엘프 0.80→1.20 = 「방어 -20% ⟺ 받는 피해 +20%」, §18-9. `1/방어배율`은
+25%가 되어 라인 160 「10~20% 기울기·곱연산 폭주 금지」 위반이라 안 씀), ③ 파티 전체 피해 진입점 하나인
`Damage()`(W3Party.cs:3130)에 `dmg *= _raceDmgTakenMul` 한 줄(수호가 오라와 같은 자리, 받는 피해 배율 그룹).
**통과 기준·증거**: `game_compile_check` PASS(error CS 0). meas 배치 빌드(오너 `unity/` 6000.5.6f1 `-useHub`+
임포트워커 점유 → `sync_meas.sh` 후 `unity_meas`·UNITY_BIN 6000.5.6f1)로 `GAME_ARGS=--race` 실행, `[W3]` 종족
로그 캡처 — 엘프 `방어배율0.80→받는피해×1.20`, 인간 `방어배율1.00→받는피해×1.00`. **네거티브 컨트롤**:
`--race 인간`(기본 측정 종족)은 ×1.00으로 배선 전과 완전히 동일 → 회귀 없음, 효과가 종족별로만 갈림을 증명.
`dmg *= _raceDmgTakenMul` 줄을 제거하면 방어배율이 다시 소비처 0곳(dead field)으로 복귀. 증거 로그
`scratchpad/race_elf.log`·`race_human.log`. `W3Party.cs` 한 파일 — 전투 수치 진입점만, AI·온라인·영지 무접촉.
**남은 관찰(이번 결함 아님)**: RaceDef의 다른 0곳 필드 — `고유발동확률`(엘프 회피·드워프 불굴 25%)·`전투당발동`은
사망 처리(LifeSystem) 개입이 커 신중해야 하고, JobDef 이동기쿨·이동기거리, CombatStyleDef 정예우선타겟·
소모품자동사용도 아직 0곳(다음 코드칸 후보). **다음 이터는 폴리싱 칸(마지막 트랙=코드) — 폴리싱 다음이
「타이틀」.**


폴리싱(전투HUD) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20): 마지막 트랙 코드라 폴리싱 칸.
`폴리싱 다음`이 「전투HUD」라 W3Party 전투 HUD를 `GAME_START=hunt QA_PARTY5=1`로 meas 배치 빌드
(오너가 `unity/` 에디터 6000.5.6f1 `-useHub`+임포트워커 점유 중 — `sync_meas.sh` 후
`GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1` 우회)로 프레임 260에 캡처. **결함**: 상단 요약 패널
(좌 `1-1 · 편성 파티 / 처치 N · Ns`, 우 `스킬 자동`)·보상 레일·HP 숫자·카드 번호가 전부 얇은
비볼드 16px 한글이라, 특히 좌측 요약이 밝은 흙길 위에 뜨면 안티에일리어싱만으로 획이 뭉개져
「안 읽히는 글씨」였다. **판별로 원인 좁힘(2단계 오답 후 확정)**: ① 배경 배어듦 가설 → 바탕
불투명도 0.88→1.0으로 올려 패널 내부를 근흑 남색으로 만들어도(나무 배어듦 제거됨) 글자 고스트
그대로 → 배경 아님, ② 글자 크기 가설 → 16→19px + 패널 확대해도 커지기만 하고 여전히 흐림 →
크기 아님, ③ **획 두께가 지렛대**: 같은 16px인데 HP 숫자(378/378)는 숫자라 선명, 필드 위
콜아웃(도발의 함성 등)은 `FontStyle.Bold`+그림자라 선명 — 저장소가 이미 쓰는 방법이다. 수리:
`_cmdLabel`(전투 HUD 크롬 공용 스타일)에 `fontStyle = FontStyle.Bold` 한 줄 추가 →
요약·보상 레일·HP 숫자·카드 번호가 일괄 굵어져 선명. **통과 기준·증거**: 굵힘 후 좌측 요약
2줄이 밝은 배경 위에서도 읽힘, `스킬 자동` 선명, HP 숫자·카드 번호 바 안에서 잘림/넘침 없음,
error CS 0(`grep -c "error CS" build_game.log` = 0). 증거
`output/qa/ashes-to-stars/hud_polish_shots/after_bold.png`(after)·`qa_hunt.png`(동일). **네거티브**:
`fontStyle = FontStyle.Bold`를 제거하면 상단 요약 얇은 한글이 다시 뭉개짐(굵힘 전 첫 캡처와 동일).
전투 수치·AI(W3Party 전투 로직)·온라인·영지 무접촉 — `W3Party.cs` 한 파일, `DrawCombatSummary`
스타일 초기화 한 줄. **남은 관찰(이번 결함 아님)**: 보상 레일 발판 칩(연속·처치율)은 바탕
알파가 칩 페이드 알파(0.92)와 곱해져 나무·폐가 위에서 배경이 여전히 비침 — 굵힘으로 개선됐으나
완전 불투명은 아님, 향후 폴리싱 후보. **다음 이터는 코드칸(마지막 트랙=폴리싱).**

코드칸(직업 스킬 목록) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20, 커밋 `d120ee06`): 직전 트랙
폴리싱이라 이번은 코드칸(기획서 ✅ · 소비처 0곳). 서브에이전트 딥서치로 **새 0곳 갭**을 찾았다:
`JobDef.스킬`(SkillDef[], §4:79-83 ✅ 「전직 단계와 스킬 개수」)이 `ProjectSetup.cs:155-209`에서 직업마다
authored(수호기사=도발의 함성·성채 방패·최후의 보루 …)되고도 **어떤 런타임 코드도 안 읽음** — `grep -rn '\.스킬'`
전체 유일 히트가 ProjectSetup의 **쓰기** 한 곳(`:209 a.스킬 = …`). JobDef 소비처는 지난 세션(12ec2a60)이
JobInfo.ConceptLine(컨셉·고유메커니즘)만 배선했고 스킬 배열은 여전히 죽어 있었다(CombatStyleDef·RaceDef·
prop_scale.json이 겪은 「정의는 있고 부르는 곳이 없다」 재발). 수리: ① `JobInfo.SkillLine(job)` 추가 —
`d.스킬`의 이름을 「보유 스킬 — {이름} · …」로 읽음(못 읽으면 "" 반환, 지어내지 않음 = StyleScreen 독트린),
② `CharacterScreen.cs:363` 트라이트 줄 바로 뒤에 소비처 추가. **부수 근본수리**: 속성 탭 행이 절대 인덱스로
그려져 회복(row2)·부활(row3) 슬롯이 안 쓰이는 건강한 캐릭터에서 **빈 2행 갭**이 생겨 특성 줄이 패널
하단으로 밀렸고(지난 세션 STATUS가 「향후 폴리싱 후보」로 기록), 스킬 줄은 `r.yMax`를 넘겨 **아예 안 그려졌다**
(`GameScreen.Info:485`는 넘치면 조용히 건너뜀). `statusMax`(실제 쓴 최상단 행) 도입해 전직 블록이 항상 그
다음부터 시작 — 겹침 불가(advancementRow는 status행보다 항상 큼), 삭제+추모 4행일 때 옛 코드가 row4에서
「1차 전직」 잠금과 추모 TimeLine이 겹치던 잠재 결함도 같이 닫힘(now row5). **통과 기준·증거**: meas 배치
빌드(오너가 `unity/` 6000.5.6f1 `-useHub`+임포트워커 점유 중 — `sync_meas.sh` 후 UNITY_BIN 6000.5.6f1로
`unity_meas` 빌드, 출력은 BuildGame 하드코딩 경로 `build_game/`)로 프레임 200 캡처. `QA_JOB_TRAIT=1` 수호기사
속성 탭에 「보유 스킬 — 도발의 함성 · 성채 방패 · 최후의 보루」가 트라이트 줄 아래 겹침·잘림·하단 넘침 없이
렌더(에셋 실데이터와 일치), error CS·Exception 0. **네거티브**: ① 수정 전 동일 env 첫 캡처는 스킬 줄이
r.yMax 넘겨 **안 뜸**(statusMax 갭 제거 전) → 갭 제거가 필수임을 증명, ② 기본직(탱/딜/힐/버퍼)은 JobDef.직업명
매칭이 없어 SkillLine "" → 줄 없음(negctrl 샷: 기본 탱커, 스킬 줄 없음 — ConceptLine과 동일 For() 조회,
지난 세션에 기본직 특성 줄 미표시 검증됨). 증거 `output/qa/ashes-to-stars/job_skill_shots/char_seed.png`(양성)·
`char_negctrl.png`(음성). 전투/AI(W3Party)/온라인/영지 무접촉 — `JobInfo.cs`·`CharacterScreen.cs` 두 파일.
**다음 이터는 폴리싱 칸(마지막 트랙=코드) — 폴리싱 다음이 「전투HUD」라 W3Party 전투 HUD 크롬(글씨·여백만,
전투 수치·AI 무접촉).** 남은 관찰: 각 SkillDef의 설명·쿨다운·위력배율은 아직 화면에 안 뜸(이름만) — 스킬
상세 툴팁/2차 전직 스킬 개수 표시는 향후 후보(이번 결함 아님).

폴리싱(결과) 이번 이터 결과 — 결함 1건 수정 (루프, 2026-08-20, 커밋 `fd6ea920`): 마지막 트랙 코드라
폴리싱 칸. `폴리싱 다음`이 「결과」라 `go:Result`를 `QA_GEAR_DROP=1 QA_HUNT_GOLD=1`로 meas 배치 빌드
(오너가 `unity/` 에디터 6000.5.6f1 점유 중 — `sync_meas.sh` 후 `unity_meas`·UNITY_BIN 6000.5.6f1 우회,
qa_shot 래퍼가 권한 게이트에 걸려 Unity 바이너리 직접 호출로 빌드·실행)로 프레임 200에 캡처. **결함
1건 발견·수정**: 보상 줄 사이의 「빈 줄」 스페이서(`ResultScreen.cs:151·194`의 `Info(r, _rowIndex++, "")`)가
빈 텍스트인데도 `GameScreen.InfoAt`이 `panel` 9-slice를 **무조건** 그려, 「생존…」과 「획득 골드」 사이에
테두리만 있는 고아 빈 패널 박스가 떴다(빈 종이 계열). 두 소비처(골드 앞·경험치 앞)를 `_rowIndex++`
세로 간격 예약만으로 교체 — 프레임 미생성. **통과 기준**: 결과 화면에서 보상 줄 사이 빈 프레임 박스
없이 깔끔한 세로 간격 + 하단 선택 카드와 겹침 없음 + error CS 0. **네거티브**: `_rowIndex++`를
`Info(r, _rowIndex++, "")`로 되돌리면 빈 프레임 박스 재발(before 샷과 동일). 증거
`output/qa/ashes-to-stars/result_polish_shots/qa_go:Result.png`(after, row2 빈 프레임 사라짐). before는
직전 동일 env 캡처. **남은 관찰(이번 결함 아님)**: 「획득 골드」 코인 아이콘이 첫 글자 「획」 좌측을
살짝 덮음(`RewardInfo` 아이콘 x=r.x+4 vs 텍스트 들여쓰기) — 향후 폴리싱 후보. `W3Party`/Resources/
`EstateBuild.cs`는 안 건드림 — `ResultScreen.cs` 한 파일. **다음 이터는 코드칸(마지막 트랙=폴리싱).**

코드칸(직업 특성 상세) 이번 이터 결과 — 배선 완료 (루프, 2026-08-20, 커밋 `12ec2a60`): 직전 트랙
폴리싱이라 이번은 코드칸(기획서 ✅ · 소비처 0곳). 최근 3세션이 남은 코드 갭 3종(정예우선타겟·
동맹/전장의안개·접근성옵션)을 봉인/온라인미구현/시기상조로 소진하고 폴리싱으로 대체해왔으나,
서브에이전트 딥서치로 **새 0곳 갭**을 찾았다: JobDef(직업 컨셉·고유메커니즘·스킬 설명, §4 직업
특성 상세 설계 ✅)가 어느 런타임 코드에서도 안 읽힘. **두 겹의 결함**이었다 — ① 에셋이 Resources
밖(`_Game/Data/Jobs/`)이라 런타임 로드 불가(races·styles가 겪은 「Resources 밖 자산」 함정 재발),
② 옮겨도 `LoadAll<JobDef>("jobs")`가 0 반환 — 11개 에셋 전부 `m_Script:{fileID:0}`으로 스크립트
참조가 끊겨(m_EditorClassIdentifier는 에디터 전용) 런타임이 JobDef 타입 인식 못 함.
수리: 에셋을 `Assets/Resources/jobs/`로 이관(git mv, GUID 보존) + m_Script를 JobDef GUID로 11개 복구
+ ProjectSetup 생성 경로·SandboxNote 갱신 + `JobInfo.ConceptLine()` 소비처를 CharacterScreen 속성 탭에
추가(StyleScreen 독트린 — 못 읽으면 지어내지 않고 생략). 통과 기준·증거: meas 배치 빌드로 캡처 —
네거티브 컨트롤 m_Script 수리 전 `0 defs`(줄 안 뜸)→수리 후 `11 defs`, `QA_JOB_TRAIT=1`(env 게이트
상설 시드) 수호기사 세워 속성 탭에 「직업 특성 — 정통 방패탱 · 수호 게이지…」 렌더·읽힘·겹침 없음,
기본직 기본 화면 회귀 없음, error CS·Exception 0. 증거 `output/qa/ashes-to-stars/char_polish_shots/
qa_go:Character.png`. 전투/AI/온라인/영지 무접촉. **다음 이터는 폴리싱 칸(마지막 트랙=코드) — 폴리싱
다음이 「결과」라 결과 화면.** 남은 관찰: 속성 탭 특성 줄이 빈 회복/부활 슬롯(row 2·3) 뒤라 패널
하단에 뜬다 — 읽히지만 상단과 간격이 큼(기존 고정 행 인덱스 특성, 이번 결함 아님). 향후 폴리싱 후보.

폴리싱(던전) 이번 이터 결과 — 결함 0 클린 (루프, 2026-08-20): 마지막 트랙 폴리싱이라 코드칸이어야
했으나 직전 두 세션(`de691764` 등)이 남은 ✅ 코드 갭 3종(정예우선타겟=W3Party 전투 AI 가드레일 봉인 ·
동맹/전장의안개=온라인 미구현 죽은컨트롤 · 접근성옵션=로드맵 4단계 시기상조)을 소진 확인하고 「재감사 말고
폴리싱 돌리라」 인계라, 선례대로 폴리싱을 돌렸다. `폴리싱 다음`이 「던전」이라 `go:dungeon`을 meas 배치
빌드(오너가 `unity/` 에디터 6000.5.6f1 `-useHub` + 임포트 워커 2종 점유 중 — `sync_meas.sh` 후
`GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`으로 우회)로 프레임 200에 캡처. 전수 육안(헤더 제목·부제·
현재위치 바·2노드 카드·아이콘·9-slice 코너·ESC 힌트) + 원해상도 크롭 4종: 헤더 「던전 · 야수 ·
마법사·정령사 ×1.3」 우측 끝 안 잘림, 부제 「시드 20260814 · T4 · 노드 1/9 · 일반 · 보유 2511골드…」
읽힘, 「1. 전투」 카드 아이콘=십자검(NodeIconKey default→`damage`, 소스 `DungeonScreen.cs:142` 교차확인)·
「던전 포기」 카드 아이콘=깨진 하트(`heart_broken`, `DungeonScreen.cs:119` 의도된 상실 은유 키 — 오배정
아님) 모두 정상 렌더, 우측 카드 골드 9-slice 코너 정상 닫힘(안 잘림). 잘림/겹침/흰 종이/낡은 텍스처/
안 읽히는 글씨 **없음**. 던전 노드 아이콘은 `fb7cd614`(등대→종류별)에서 이미 수리 완료라 재발 없음.
증거 `output/qa/ashes-to-stars/dungeon_polish_shots/qa_dungeon.png` + 크롭 `/tmp/dg_{header,swords,heart,corner}.png`.
코드 변경 없음(폴리싱=한 화면 한 결함, 던전은 클린). 폴리싱 다음을 결과로 넘김.
`W3Party`/Resources/`DungeonScreen.cs`는 안 건드림.

코드칸 재시도 결과 — 남은 ✅ 갭 3종 전부 봉인, 폴리싱 월드맵으로 대체 (루프, 2026-08-20): 직전 트랙
폴리싱이라 이번은 코드칸(기획서 ✅ · 소비처 0곳)이어야 했다. 서브에이전트 딥서치로 원장을 훑어 후보 3종을
찾았고 셋 다 안전하게 배선 불가로 판정했다. ① **정예우선타겟**(CombatStyleDef.cs:24) — 직전 세션과 동일,
W3Party 전투 AI를 고쳐야 해 가드레일 봉인. ② **동맹 24h 쿨다운·상호 침략 불가**(§15, 원장 L881)와
**전장의 안개**(§14, L841) — 정의조차 없으나 **둘 다 온라인(다른 유저 별) 전제**다. 그런데 `InvasionState.cs:7,9`가
「다른 유저 서버가 아니라 로컬 별 수비대와 싸운다 · 랭킹·동맹·경로 전투는 여기서 열지 않는다」로 못박혀
있고, 로드맵(원장 L1800대)이 **온라인 기반을 알파 뒤 3단계로 미룸(네트워크 코드 0줄)**이라, 지금 배선하면
가둘 상대가 없는 **죽은 컨트롤**(정예우선타겟과 같은 안티패턴 재생산)이 된다. ③ **접근성·옵션 화면**(G17,
원장 L1791) — 로컬로는 성립하나 로드맵이 **폴리시·베타 4단계**로 못박은 시기상조 신규 화면(관문 순서 위반).
→ 남은 코드 갭이 전부 봉인/시기상조/온라인미구현이라, 직전 세션 선례대로 폴리싱을 돌렸다. `마지막 트랙`은
폴리싱 유지. 새 온라인/접근성 착수는 오너·미래 세션의 로드맵 단계 판단이 필요(사람 관문).

폴리싱(월드맵) 이번 이터 결과 — 결함 0 클린 (루프, 2026-08-20): `폴리싱 다음`이 「월드맵」이라
`go:WorldMap`을 meas 배치 빌드(오너가 `unity/` 에디터 6000.5.6f1 `-useHub` 점유 중 — 임포트 워커 2종 확인,
`sync_meas.sh` 후 `GAME_PROJ=unity_meas`·`UNITY_BIN=6000.5.6f1`·positional `go:WorldMap`으로 우회. positional
$1이 GAME_START env를 덮으므로 mode는 positional로 넘겨야 함 — 첫 시도에 `worldmap`을 넘겨 던전 폴백을
찍은 함정 확인)로 프레임 200에 캡처. 전수 육안(헤더 제목·부제·성계 정보 패널·별/행성 배경·4카드·하단
5칸 나브 도크·우측 끝 크롭·활성탭·카드 크롭): 헤더 「월드맵 … 내 별 1층·별 ×1.02·영공 1.1·침략은 탑
30층(§14·§15)」 우측 끝 안 잘림, 정보 패널 골드 9-slice 프레임 우측 코너 정상 닫힘(안 잘림), 4카드
(성계 이동·침략·랭킹·수비대 0/5) 모두 잠김 사유 라벨·아이콘 정상 렌더, 별·행성·궤도 링 우주 배경 정상,
나브 라벨(영지·필드·탑·월드맵·캐릭터) 칸 안. **활성탭(월드맵) 위 융기 「크라운」은 결함 아님** —
`GameScreen.cs:283 if(here) DrawTexture(...r.y-4f...,_accent)`의 의도된 현재탭 강조선(소스 교차 확인).
잘림/겹침/흰 종이/낡은 텍스처/안 읽히는 글씨 **없음**. 헤더 부제와 정보 패널의 「내 별 1층·별 ×1.02·영공
1.1」 부분 중복은 콘텐츠 판단(§21-3, 정보 패널은 「— 층을 오를수록 커진다」 설명이 추가라 순수 중복 아님)
이라 손 안 댐 — 필드 지갑 중복·캐릭터 6칸 링과 동일 판정. 증거
`output/qa/ashes-to-stars/worldmap_polish_shots/qa_go_WorldMap.png`. 코드 변경 없음(폴리싱=한 화면 한 결함,
월드맵은 클린). 폴리싱 다음을 던전으로 넘김. `W3Party`/Resources/`WorldMapScreen.cs`는 안 건드림.
