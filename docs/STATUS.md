...
최종 갱신: 2026-08-16 · 대화(프로토 80% 집계 수리)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **UI 퀄리티 남은 것**(INBOX 16:46 · 21:45 공간 활용 · 21:50 늘어남 금지) — 전체 「다른 게임만큼」은 사람 육안.
2. **처음 기본직업 idle 포즈**(INBOX 21:52) — 선택 카드가 walk면 고친다.
3. **V4 외부 테스터 70%** — 10세션 삭제 실측은 90%. 사람 70% 계속·24h 재실행은 완료로 내리지 마라.

> **이번 이터 결과(코드/실행): 프로토 80% 정체 — 집계가 실측을 버렸다.**
...
> - **코드** 커밋 직후 해시. 루프가 「80% 유지」로 시작 5종만 닫던 경로를 끊었다.
