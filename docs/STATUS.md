...
최종 갱신: 2026-08-16 · 루프(시작 기본직업 4종 선택)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **시작 기본직업 5종**(INBOX 21:38) — 탱커·물리딜러·마법딜러·서포터·힐러. 지금 기본은 4종이고 마법딜러가 없다(마법사는 1차). 없는 몸통만 힉스필드. 완료된 4종 실루엣은 재생성 금지. 생성 전 `art/.generating`·`higgsfield generate list` waiting. 통과: 타이틀 5장이 서로 다른 전신. 네거티브: 마법딜 폴더를 빼면 카드가 폴백/빈다.
2. **프로토타입 80%**(INBOX 21:27) — 사람 관문만 보고 대기하지 마라. 이번 이터는 시작 선택 구멍을 닫음. 다음 후보는 바로 위 1번.
3. **UI 퀄리티 남은 것**(INBOX 16:46) — 전체 「다른 게임만큼」은 사람 육안.
4. **V4 외부 테스터 70%** — 사람 70% 계속·24h 재실행은 완료로 내리지 마라.
