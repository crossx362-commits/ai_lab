...
최종 갱신: 2026-08-16 · 루프(보스 HP 프레임)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **UI 퀄리티 남은 것**(INBOX 16:46) — 아이템 아이콘·상태 아이콘 HUD·등급 프레임·보스 HP 프레임은 닫힘. 파티 전용 헤더 조각은 없고 폴백도 되돌리지 않는다. 전체 「다른 게임만큼」은 사람 육안.
2. **V4 외부 테스터 70%** — 10세션 삭제 실측 끝. 사람 70% 계속·24h 재실행은 완료로 내리지 마라.

> **이번 이터 결과(코드/실행): UI 퀄리티 — `boss_hp_frame`을 보스전 상단 중앙에 연결.**
...
| 항목 | 근거 | 커밋 |
|---|---|---|
| **보스 HP 프레임 소비처** | `boss_hp_frame` RequiredKeys+DrawBossHp+Overlay. SelfCheck PASS. PNG: 탑 견본 3칸 193585B, 실전 7500/7500 페이즈4 648352B. API 제거 시 컴파일 RED 11건 | 이 커밋 |
