...
최종 갱신: 2026-08-16 · 루프(캐릭터창 목록+대형 모습)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **프로토타입이 80%에서 멈춘 원인**(INBOX 21:27) — 사람 관문(V4 70%·육안 UI)만 보고 대기하지 말고, 원장 ✅인데 소비처 0곳이거나 화면이 비어 있는 구멍을 하나 닫는다. 이번 이터는 캐릭터창을 닫았고(`d1de9f64`) 다음 후보는 바로 아래 2번.
2. **기본직업 4종 이미지+스프라이트 애니**(INBOX 21:25) — 시작 기본직업(탱/딜/힐/버퍼) 전용 그림과 idle/walk 등 프레임을 힉스필드로 만들고 Resources에 반입한 뒤 테스트. 지금 4종은 1차 직업 실루엣을 빌려 쓴다. 생성 전 `art/.generating`·`higgsfield generate list`의 waiting을 본다. 통과: `qa_shot.sh` 캐릭터·hunt에서 4종이 서로 다른 전신으로 읽힌다. 네거티브: 옛 폴더로 되돌리면 4종이 다시 1차 실루엣이다.
3. **UI 퀄리티 남은 것**(INBOX 16:46) — 타이틀·영지·파티·캐릭터(명부/합성)는 페이지+카드. 캐릭터창은 목록 왼쪽·모습 오른쪽(`d1de9f64`). 속성 탭은 아직 줄 목록. 파티 전용 헤더 조각은 없고 폴백도 되돌리지 않는다. 전체 「다른 게임만큼」은 사람 육안.
4. **V4 외부 테스터 70%** — 10세션 삭제 실측 끝. 사람 70% 계속·24h 재실행은 완료로 내리지 마라.

> **이번 이터 결과(코드/실행): 캐릭터창 — 목록 왼쪽 · 대형 모습·장비 오른쪽.**
