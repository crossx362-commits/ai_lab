...
최종 갱신: 2026-08-16 · 루프(층 클리어 기본직업 선택)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **UI 퀄리티 남은 것**(INBOX 16:46) — 타이틀·영지·파티·캐릭터(명부/합성)는 페이지+카드. 캐릭터 상세 장비는 초상 둘레 6칸+가방. 속성 탭은 아직 줄 목록. 파티 전용 헤더 조각은 없고 폴백도 되돌리지 않는다. 전체 「다른 게임만큼」은 사람 육안.
2. **V4 외부 테스터 70%** — 10세션 삭제 실측 끝. 사람 70% 계속·24h 재실행은 완료로 내리지 마라.
3. **캐릭터창 목록+대형 초상**(INBOX 20:39) — 대화 세션 장비 스튜디오(`fe5b7204`)가 초상 둘레 6칸+가방을 넣었다. 목록 왼쪽·모습 오른쪽은 아직 사람 육안.

> **이번 이터 결과(코드/실행): 층 클리어마다 기본직업 1종 선택 · 레이드 증표 2장 확률.**
...
> - **코드** (커밋 직후 해시를 이 줄에 적는다). `W3Party`는 안 건드렸다.
