...
최종 갱신: 2026-08-16 · 이터레이션(코드 — §3 Lv20 1차 전직 선택 슬라이스)
...
| 8 | **§3 전직 시스템** — 오너 확정 경로 중 **기본직업 생성·구저장 호환(`ab20a005`) + Lv20 역할별 1차 선택 2/4/2/3·직업/단계 저장·Character 소비처(`76c6a80b`) 완료**. 다음은 1차 재료5+비살상 시험 성공 시 소비, 이후 스킬 2→4. 2차는 Lv50·재료20·같은 직업 각성(§18-6). 전직 재료는 아직 `LifeItem` enum에 없음 | Lv20 역할별 11종 선택·저장 ✅. 남은 통과 기준: 시험 성공 때만 재료5 소비, 실패/중단은 0 소비, 1차 스킬 4개 실제 반영 | Lv19·타 역할·삭제·반복 전직은 거부(SelfCheck). 시험 실패 때 재료가 줄거나 전직되면 FAIL |
