...
최종 갱신: 2026-08-20 · 월드맵 폴리싱 전수 육안 = 결함 0 클린. 코드칸 재시도했으나 남은 ✅ 갭 3종이 전부 봉인(정예우선타겟=가드레일 · 동맹/전장의안개=온라인 미구현 죽은컨트롤 · 접근성옵션=로드맵 4단계 시기상조)이라 폴리싱으로 대체
마지막 트랙: 폴리싱
폴리싱 다음: **던전**. 이어 결과 → 전투HUD → 타이틀 → 영지 → 필드 → 캐릭터 → 파티 → 월드맵.
