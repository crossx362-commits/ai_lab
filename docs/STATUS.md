...
최종 갱신: 2026-08-16 · 루프 실행기 Grok (UI 아이콘 소비처)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)

1. **UI 호버/눌림 육안 + 필드·탑 제목 아이콘** (오너 UI 지시 다음 슬라이스) — 이번 이터가 영지 건물·캐릭터 초상/목숨을 연결했다. 남은 빈 칸은 ①호버·눌림 3상태가 화면에서 갈리는지 ②필드/탑 제목이 아직 나침반인 것. 통과 기준: `qa_shot`에서 호버 또는 필드/탑 헤더가 옛 나침반·단색과 갈린다. V4 70% 전 OUT은 열지 마라.
...
최종 갱신: 2026-08-16 · 이터레이션(UI 아이콘 소비처)

> **이번 이터 결과(코드/실행): UI 퀄리티 첫 슬라이스 — 정의만 있던 아틀라스 조각을 화면에 연결.**
...
다음 자동 슬라이스는 **남은 UI 크롬**(호버 육안·필드/탑 헤더). V4 **70% 사람 판정**은
자동검사로 완료 선언 금지.
