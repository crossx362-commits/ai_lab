...
| # | 항목 | 통과 기준 | 네거티브 컨트롤 |
|---|---|---|---|
| 1 | **P2 몹 4계열 Play 육안 확인** | `./tools/qa_shot.sh hunt`로 4계열이 서로 다른 그림으로 나오는지 | `MOB_DIRS`를 1종으로 되돌리면 전부 같은 그림 |
| 2 | **몹 화풍 불일치 조사** | 현재 몹(분홍 강아지·흰 양·보라 늑대)이 §0-A 픽셀아트와 안 맞음. 어느 에셋이 쓰이는지 특정 | — (조사 항목) |
| 3 | **캐릭터 52장 Play 육안 확인** | 4직업이 새 아트로, 실루엣이 서로 구분되는지 | 옛 임시본으로 되돌리면 구분이 나빠짐 |
| 4 | **§10-5 보스 쫄 소환** | `BossBattle.cs:317`이 빈 GameObject만 만든다. 실제 몹으로 연결 | 소환 비활성 시 그 페이즈 파티 피해가 사라짐 |
| 5 | **§3 RaceDef 배선** | 종족 4종이 전투에 영향 0. 런타임 소비처 ≥1 | 값 변경이 결과에 반영되는지 |
| 6 | **§16 영지 하위 건물 4종** | 대장간·경매장·영묘·수비대가 전부 "아직 내용이 없다" | — |
| 7 | **§10-9 잡몹 상한** | 코드 `MAXM=200`, 기획서 300~500. 상향 후 60fps 유지 | 그리드를 끄면 fps가 무너져야 함 |
| 8 | **전투 스타일 UI** | 데이터는 살아났으나 플레이어가 바꿀 화면이 없다 | — |
