...
최종 갱신: 2026-08-16 · 이터레이션(코드 — §3 전직 단계별 전투 스킬 슬라이스)
...
| 8 | **§3 전직 시스템** — 기본직업/구저장(`ab20a005`) + Lv20 11종 선택(`76c6a80b`) + 재료/시험(`93b9fe39`) + **기본2/1차4 전투 단계 계약(`42315c7b`) 완료**. 다음은 W3 미지원 1차 6종(광전사·궁수·소환사·드루이드·주술사·정령사)의 고유 전투 소비처. 그 뒤 Lv50 2차 | 기본 5인은 전투 경계 SkillCount=2, 1차는 4. 남은 6종이 검사 폴백이 아닌 자기 메커니즘/스킬로 전투 | SelfCheck가 Basic=2/First=4를 단언. 단계 전달을 제거하면 기본이 1차 능력(최후의 보루 포함)을 쓰고 FAIL. 각 폴백을 되돌리면 해당 직업 고유 스킬 계측이 0이어야 함 |
