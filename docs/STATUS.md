...
최종 갱신: 2026-08-23 · keeper-warn-chip 17차 재검증(BASE `0e2ea98c`)
마지막 트랙: 문서(keeper-warn-chip 17차 재검증 — kChip warns→hold 톤·실패 칩 경고 병기는 `0ec68f11` 선반영(HEAD 조상 확인)으로 현재 board.html(922~929행) 존속 · `ops-box`가 `usage-line` 클래스(494행)라 `.usage-line .u.hold { color: var(--hold) }`(145행) = 노랑 `#e0a050` 렌더 경로까지 확인 · 재구현 없음 · loop/board.html 무변경)
