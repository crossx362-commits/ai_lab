...
최종 갱신: 2026-08-16 · 이터레이션(코드 — §3 1차 전직 재료·비살상 시험 슬라이스)
...
| 8 | **§3 전직 시스템** — 기본직업/구저장(`ab20a005`) + Lv20 11종 선택(`76c6a80b`) + **재료5·비살상 역할 시험·성공 원자 커밋(`93b9fe39`) 완료**. 다음은 기본 스킬 2개→1차 4개 실제 반영. 2차는 Lv50·재료20·같은 직업 각성(§18-6) | 시험 성공만 재료5 소비 ✅, 실패/중단/저장실패는 0 소비·목숨 불변 ✅. 남은 통과 기준: 1차 스킬 4개 실제 전투 반영 | SelfCheck가 오답 행동·실패·중단·저장실패 주입을 탐지. 스킬 소비처를 기본 2개로 되돌리면 1차 4개 통과 기준 FAIL |
