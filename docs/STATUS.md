...
최종 갱신: 2026-08-16 · 이터레이션(코드 — §3 Lv50 같은 직업 2차 각성 흐름)
...
| 8 | **§3 전직 시스템** — 기본/구저장(`ab20a005`) + Lv20 선택(`76c6a80b`) + 재료/시험(`93b9fe39`) + 2→4스킬(`42315c7b`) + 6종 고유 분기(`8128fbf7`) + **6종 실전 계측 하네스(`ab5fe3d0`)**. 다음은 정상 Unity에서 수치·화면 증거 실행. 그 뒤 Lv50 2차 | 6종이 검사 폴백 없이 자기 전투 분기를 소비하고 슬롯별 효과 계측 >0, 직업별 PNG 6장 | `QA_ADV_NO_SLOT=1|2`로 해당 슬롯을 끄면 계측 0·FAIL. Unity 패키지 정상화 대기 |
