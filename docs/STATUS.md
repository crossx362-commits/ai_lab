...
최종 갱신: 2026-08-26 (바퀴) · **루프 생존성 수리**(`d2410dcf`, INBOX STOP 「루프가 안 돈다」 2026-08-26 19:25) — 실측 원인 체인: ①19:29 바퀴(캐릭터창 폴리싱)가 Unity MCP `Endpoint is unavailable`로 code=1 중간사망→STATUS 미갱신 ②loop.sh 설계대로 미갱신 3회 「정상 종료(exit 0)」→ launchd KeepAlive(`SuccessfulExit=false`)는 재실행 안 함 ③유일 회복 수단 `loop_watch.sh`가 pgrep 의존+무흔적 침묵 설계(watch 로그 파일 부재 실측)라 **12:11~19:27 약 7시간 공백**. 수리: loop_watch를 **lap 로그 mtime 단일 축**(fresh ≤90분이면 pgrep 오판과 무관 통과 · stale 시 kickstart→bootstrap 폴백 · 매 실행 판정 근거 로그 · `DRY_RUN`/`LOOP_WATCH_ROOT` 격리 실측 지원)으로 재설계 + PROMPT.md에 「유니티 MCP 불능 시 그 검증만 건너뛰고 배치 SelfCheck로 대체 검증 후 커밋·STATUS 완주」 규칙 추가. macOS pgrep -f procargs 누락 실측(PID 12651 살아있는데 rc=1·speed_lane은 매칭 OK). 다음 바퀴는 STATUS 큐 1위(UI·아트 폴리싱 — INBOX 캐릭터창 관찰, 19:29 바퀴의 접힘한계 스크롤 작업은 code=1로 미커밋 상태였으나 워킹트리 잔여 확인 후 인수 필요). 루프 가동 중(opencode).
...
1. **UI·아트 상시 폴리싱** — 직전 바퀴가 코드·검증 트랙이라 교대 규칙으로 1위. **선반영 인수 먼저**: 19:29 바퀴(캐릭터창 속성 탭 스크롤·`InfoFoldLimit`)가 Unity MCP 사망으로 code=1 미커밋 — 워킹트리 잔여(`CharHudSelfCheck.cs`·`CharacterScreen.cs` 등 `charhud_infofold_inherit.log` 배치 PASS 실측 존재)를 원상 대조 후 이어 마무리. INBOX 캐릭터창 「정보보기 불편·선택 안 됨」은 오너 관찰이라 우선 검토. 「다른 게임만큼」완료 금지. 기존 닫음 나열은 아카이브 참조.
