...
최종 갱신: 2026-08-16 · 루프(저체력 자동 귀환)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **UI 퀄리티 남은 것**(INBOX 16:46) — 타이틀·영지·파티·캐릭터는 페이지+카드. 파티 전용 헤더 조각은 없고 폴백도 되돌리지 않는다. 전체 「다른 게임만큼」은 사람 육안.
2. **V4 외부 테스터 70%** — 10세션 삭제 실측 끝. 사람 70% 계속·24h 재실행은 완료로 내리지 마라.

> **이번 이터 결과(코드/실행): §4·§6 저체력 자동 귀환 — 필드 자동화 일정의 첫 슬라이스.**
...
> - **코드** (이 커밋). `W3Party`는 HOLD 후 비율 getter만.
...
| 항목 | 근거 | 커밋 |
|---|---|---|
| **§4·§6 저체력 자동 귀환** | 필드만 HP≤30%→3초 이탈→영지. 기본 ON. 사망·보상 없음. SelfCheck 전항 PASS. PNG 421774B `저체력 귀환 켜짐`. API 제거 시 컴파일 RED 2건 | 이 커밋 |
