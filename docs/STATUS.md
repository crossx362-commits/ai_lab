...
최종 갱신: 2026-08-16 · 이터레이션(코드 — §3 1차 전직 6종 전투 아키타입 슬라이스)
...
| 8 | **§3 전직 시스템** — 기본직업/구저장(`ab20a005`) + Lv20 11종 선택(`76c6a80b`) + 재료/시험(`93b9fe39`) + 기본2/1차4 계약(`42315c7b`) + **1차 6종 고유 분기(`8128fbf7`) 연결**. 다음은 정상 Unity에서 6종 슬롯1/2 실전 계측·화면 확인. 그 뒤 Lv50 2차 | 기본 5인은 SkillCount=2, 1차는 4. 6종이 검사 폴백 없이 자기 역할·사거리·메커니즘·스킬로 전투하며 슬롯별 효과 계측 >0 | SelfCheck가 Basic=2/First=4와 6종 계약을 단언. 각 전투 분기를 되돌리면 해당 직업 슬롯 계측이 0이어야 함(실행 계측 하네스는 다음 슬라이스) |
