...
최종 갱신: 2026-08-26 (바퀴) · **§18-14 소환수 재소환 소비처 완료**(`996e98ef`) — BalanceConfig.재소환비용실버(50)·재소환쿨다운초(30) → `Resummon` Reader → 캐릭터 속성 탭 줄(원장 수치 그대로 0.5G/h T1=50실버·쿨다운 30초). QA_NO_RESUMMON 네거티브로 옛 화면(줄 없음) 유지. W3Party 무접촉 — 소환수 개체·죽음 개념 도입은 별도 심의 과제. ResummonSelfCheck 배치 PASS exit 0 · GameSweep 65/65 PASS exit 0(unity_meas) · 오너 에디터 컴파일 에러 0 · 플레이모드 A/B 실측 `output/qa/ashes-to-stars/resummon_shots/` 육안(포지티브 줄 있음·네거 줄 없음). 직전 랩이 커밋 전 중단해 남긴 작업트리 수렴분을 원장 수치 대조·전면 재실측 후 인수(c619b9fe 선례). 루프 가동 중(opencode).
...
1. **검증 탐지 장치 패키지** — 회의 20260826-095813 채택 1·다음 바퀴 권고: ①Editor *SelfCheck 전수↔GameSweep 등록 목록 대조 검사 신설(EliteDropSelfCheck 8/18 잠복·GameSweep 등록 누락 재발을 컴파일 단계에서 포착) ②GameFullCheck 전수 1회 의무 실측(전수 194종 스냅샷 흔적을 `last_test_report`에 확보). §18-14 소환수 재소환은 닫음(`996e98ef`).
...
3. **UI·아트 상시 폴리싱** — 최신 정상 샷에서 절단·겹침 한 곳만(다음 후보: GameScreen Body 절단 · INBOX 캐릭터창 정보보기·선택 불편은 오너 관찰이라 우선 검토). 「다른 게임만큼」완료 금지. 기존 닫음 나열은 아카이브 참조.
4. **W2 손맛(회피 기회)** — 더미 판정 FAIL 유지(`loop/w2_dummy_sim.py`, 2026-08-25). 오너 직접 플레이 후 위협밀도·대시 지시 필요.
