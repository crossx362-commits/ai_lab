...
최종 갱신: 2026-08-16 · 루프(대출 연체·파산 제재)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **UI 퀄리티 남은 것**(INBOX 16:46) — 아이템 아이콘 슬라이스는 닫힘. 파티 전용 헤더 조각은 없고 폴백도 되돌리지 않는다. `StatusIconAtlas`는 전투 HUD(대화 세션). 전체 「다른 게임만큼」은 사람 육안.
2. **V4 외부 테스터 70%** — 10세션 삭제 실측 끝. 사람 70% 계속·24h 재실행은 완료로 내리지 마라.
3. **영지 3건물 남은 OUT** — 경매 거래서버·침략 본게임. 문은 잠글 수 있어도 장은 없다. V4 전에 새 범위로 열지 마라.

> **이번 이터 결과(코드/실행): 대출 연체·파산이 경매장/침략 문을 잠근다.**
