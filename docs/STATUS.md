...
최종 갱신: 2026-08-16 · 루프(시작 직업 idle 포즈)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **UI 퀄리티 남은 것**(INBOX 16:46 · 21:45 공간 활용 · 21:50 늘어남 금지) — 전체 「다른 게임만큼」은 사람 육안. 시작 카드 idle은 이번 이터에서 닫음.
2. **V4 외부 테스터 70%** — 10세션 삭제 실측은 90%. 사람 70% 계속·24h 재실행은 완료로 내리지 마라.

> **이번 이터 결과(코드/실행): 시작 직업 카드는 idle 포즈.**
