...
1. **기본직업 4종 이미지+스프라이트 애니**(INBOX 21:25) — 시작 기본직업(탱/딜/힐/버퍼) 전용 그림과 idle/walk 등 프레임을 힉스필드로 만들고 Resources에 반입한 뒤 테스트. 지금 4종은 1차 직업 실루엣을 빌려 쓴다. 생성 전 `art/.generating`·`higgsfield generate list`의 waiting을 본다. 통과: `qa_shot.sh` 캐릭터·hunt에서 4종이 서로 다른 전신으로 읽힌다. 네거티브: 옛 폴더로 되돌리면 4종이 다시 1차 실루엣이다.
2. **UI 퀄리티 남은 것**(INBOX 16:46) — 타이틀·영지·파티·캐릭터(명부/합성)는 페이지+카드. 캐릭터창은 목록 왼쪽·모습 오른쪽(`d1de9f64`). 속성 탭은 아직 줄 목록. 파티 전용 헤더 조각은 없고 폴백도 되돌리지 않는다. 전체 「다른 게임만큼」은 사람 육안.
3. **V4 외부 테스터 70%** — 10세션 삭제 실측 끝. 사람 70% 계속·24h 재실행은 완료로 내리지 마라.
