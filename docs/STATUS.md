...
1. **기획서 ✅ · 소비처 0곳** — 원장 `GAME_DESIGN_ASHES_TO_STARS.md`를 훑어 ✅인데 grep 소비처가 0인 칸 **하나만** 닫는다. §11 드랍 옵션 1~4·§10-8 정예 일반·보스 고급 장비·가방 60칸·무기 직업 계열·§13-3 진입 면 선택은 닫음. 시각 UI「다른 게임만큼」·V2 손맛·V4 70%는 사람 관문이라 닫지 않는다. `W3Party`·오너 Unity는 건드리지 않는다. **중복 리소스 재생성 금지.** 통과 기준: 소비처 ≥1 + SelfCheck + 네거티브. 증거 없는 완료 금지. 직전 트랙이 코드면 이번은 하지 말고 **UI·아트 상시 폴리싱**을 한다.
2. **UI·아트 상시 폴리싱** — 코드 구멍 다음 이터에 화면 하나·결함 하나. 「다른 게임만큼」완료 금지. 할로우 화풍 강제 취소. 이미 있는 장을 쓰고, 있는 대상은 다시 뽑지 않는다. 샷+한 결함이 완료. 지금 화면: 필드. **영지 화면·EstateYard는 클로드 설계 문서가 생기기 전 금지.** 문서가 생기면 읽고 그 문서대로 같이 구현한다.
3. **INBOX 09:57 전체 그래픽 남은 것** — 필드 허브 HUD·전용 7건물·경매장 전용 그림·제목판 52·경매 전폭 막대·캐릭터창 3열·장비 라벨·현황 도크·도크 부제·마을 끌어 보기·굴려 확대는 닫음. 중복 리소스 재생성 금지. 캐릭터/몹 화질은 사람 육안. 새 생성 전 `ARTIFACT_INDEX`·대기 작업 확인.
