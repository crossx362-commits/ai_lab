...
최종 갱신: 2026-08-17 · 루프(수인 약탈량 +20%)

## 다음 할 일 (원장 §22 — 위에서부터 하나만)
1. **UI 퀄리티 남은 것**(INBOX 16:46 · 21:45) — 하단 도크·격자 8×8·시작 2명·침략 보호막·수비대 회복·인간 PvE 18h·경매 수수료 7%·영지생산·드랍률·전직재료배율·약탈량은 닫음. 허브 카드 여백은 다른 세션이 작업 중일 수 있다. 전체 「다른 게임만큼」은 사람 육안.
2. **기획서 ✅ · 소비처 0곳** — 대기하지 말고 원장을 훑어 다음 구멍을 큐에 올린다. `RaceDef` 런타임 칸은 약탈량까지 닫힘. 이속·체력은 W3Party가 이미 읽고, 방어배율·불굴·야성 감각은 W3Party라 대화 세션. 다음 비전투 구멍: **엘프 별 인식 범위 +20%**(§18-9). `WorldStar.Sense`가 층만 쓰고 종족을 안 본다. 통과 기준: 같은 층에서 엘프 영공이 인간/드워프/수인의 120%. `QA_NO_RACE_SENSE=1`이면 엘프도 100. 네거티브: `Sense`에서 종족 배율을 빼면 엘프=인간. 탐험 범위 +30%는 안개 시스템이 없어 안 넣음. 드워프 방어 내구는 건물 HP가 없어 안 넣음. 16×16 부지·배치 프리셋·경로 전투 시뮬·무료 영입 3회·일과표는 💡라 안 연다. 동일 상대 24h -80%·인간 PvP 9h는 보호막과 어긋나 안 넣음.

V4 외부 테스터 70% → 넘김. 사람 70% 계속·24h 재실행은 측정하지 않았다. 테스터 통과가 아니다.

> **이번 이터 결과(코드/실행): 수인 약탈량 +20% — RaceDef.약탈량배율을 읽는다.**
