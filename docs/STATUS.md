...
1. **V3 한 판 종단** — 파티 공격→보스 HP→페이즈→장판/소환/힐체크→처치→탑 층 돌파가 **같은 실행**에서 닫힐 것. 통과: SelfCheck 수치 + `qa_shot.sh boss` PNG. 네거티브: `BOSS_NO_DPS=1`이면 층이 안 오른다. (HP·장판·힐 보고 조각은 이미 있음 — 다시 만들지 마라)
2. **V2 사람 판정** — 실제 창에서 대시 무적 0.3초/쿨 6초. 자동검사로 통과 선언 금지.
3. **V4 삭제 루프 준비** — 보스 패배→3회 사망→삭제→계속 플레이. 영지/침략/경매/합성 새 범위는 V4 이후(§21-3).

아이콘·호버 육안은 위 세 관문 사이 빈 칸을 채울 때만. 기획서 OUT을 새 범위로 열지 마라.
