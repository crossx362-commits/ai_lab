...
최종 갱신: 2026-08-16 · 루프 실행기 Codex (클로드 주간 한도 ~08-17 23:00)

## 다음 할 일 (위에서부터 하나만)

1. **아이콘 최소 세트** — 화면에 이미 뜨는 스킬·하단탭·아이템만. 아틀라스 조각이 있으면 소비처부터. 통과: 소비처 ≥1 + SelfCheck + PNG. 네거티브: 키를 빼면 폴백.
2. **V3 한 판 종단** — 힐체크 판정 → 보스 처치 → 탑 층 돌파. 통과: SelfCheck 수치 + PNG. 네거티브: `BOSS_NO_DPS=1`이면 층이 안 오른다.
3. 호버/눌림 버튼 육안 캡처 — 원본 에디터 락이면 `unity_meas` + `qa_shot.sh`.

> **루프 세팅(2026-08-16):** 클로드 `weekly limit resets Aug 17 23:00` 실측. 실행기는 `loop/agent=codex`. 빈 `loop/HOLD`가 git 추적돼 재개마다 30분 대기하던 함정은 추적 해제.

최종 갱신: 2026-08-16 · 이터레이션(UI 프레임 소비처 — 버튼 3상태·9-slice·체력바)
...
| 2b | **보스 나머지 통합(HP·장판·힐체크)** — HP 코드 `e48aee09`; 이번 이터 장판/힐 피해 경계 실행 PASS. **남은 것은 한 판 종단 검증** | HP SelfCheck `9000→4500→0`, 페이즈1·처치1. 장판은 실파티 위치 2곳 예고 뒤 실제 HP `60+30=90`, 활성 힐체크 보고90. PNG에 보스·파티·주황 장판 동시 렌더. 다음은 힐 판정→처치→탑 층 돌파까지 한 실행에서 닫기 | `BOSS_NO_DPS=1` → HP `9000→9000`; `BOSS_NO_AOE=1` 신규/재진입 → 장판 HP피해0·힐보고0 |
