...
최종 갱신: 2026-08-16 · 이터레이션(코드 — §3 기본직업 생성·구저장 호환 슬라이스)
...
| 8 | **§3 전직 시스템** — **오너 확정(2026-08-16): 기본직업 4종으로 시작 → Lv20 1차 전직 11종 → Lv50 2차 전직.** 현재 코드가 1차 직업으로 바로 생성되는 불일치를 먼저 복구한다. 1차는 재료5+시험, 2차는 재료20(§18-6). ⚠️ **전직 재료는 Economy `LifeItem` enum에도 없다**(주석·설계뿐, 이번 이터 실측) — enum·드랍표·소비처를 전부 새로 배선해야 함. 스킬 2→4는 combat(W3Party). **한 이터에 깨끗이 못 끝나는 다-시스템 과제** — 슬라이스로 쪼개거나 combat 세션과 조율 필요 | 전직 시 기본직업에서 1차 직업을 선택하고 스킬 2→4·직업 세분화가 실제로 반영. CharacterScreen 잠김 해제 | 기본직업 선택/전직 결과를 제거하면 스킬 수·직업명 원복 |
