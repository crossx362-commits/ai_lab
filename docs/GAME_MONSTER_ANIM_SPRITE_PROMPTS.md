# 재와 별 — 몬스터 애니메이션 리스트 · 스프라이트 생성 프롬프트 v0.2
...
> 파이프라인: `art/aigen.py` → `art/split_ai_sheet.py` → `art/align_frames.py` → `unity/Assets/Resources/sprites/`
> 작성: 2026-08-13 / 현행 계약 갱신: 2026-08-16
>
...
| §0-B 규칙 | 이 문서에서의 의미 |
|---|---|
| **전 유닛 1방향 + 런타임 `flipX`** | 방향 이미지는 **1벌만** 생성. `S`·`E`·`N`·8방향 프롬프트 금지 |
...
- 📌 로스터의 "**종** 95개"는 기획상의 종류이지 작화 단위가 아니다. 현재 작화 단위는 잡몹 행동형 4개와 보스 실루엣 4개이며, 완료 자산은 `ARTIFACT_INDEX.md` 확인 없이 다시 생성하지 않는다
...
```
<유닛ID>_<상태>_<프레임번호 2자리>.png
```

- ✅ 현재 몬스터 런타임 상태는 `idle` · `walk` · `attack` · `hurt` · `death` 다섯 가지다. 파일명에 방향 번호·접미사를 넣지 않는다
- ⚠️ `special`과 역할별 추가 상태는 코드 소비처가 생기기 전에는 생성하지 않는다. 행동 예고는 우선 기존 22프레임과 코드 이펙트·색 플래시·스케일 변화로 표현한다

### 1-2. 현재 잡몹 공통 5상태 — 22프레임 고정

| 상태 | 프레임 | fps | 루프 | 이벤트 프레임 |
|---|---:|---:|---|---|
| `idle` | 4 | 4 | ✅ | — |
| `walk` | 6 | 8 | ✅ | — |
| `attack` | 4 | 10 | ✗ | **피해 판정 = 마지막 직전 프레임** |
| `hurt` | 4 | 12 | ✗ | 흰색 피격 플래시 병행 |
| `death` | 4 | 8 | ✗ | 드랍 생성 = 마지막 프레임 |
| **합계** | **22** | | | `SpriteBank.MOB_FRAMES`와 일치 |

- ⚠️ **피해 판정 프레임을 마지막에 두지 마라** — 마지막 프레임에서 판정하면 유저 눈에는 "때리는 모션이 끝난 뒤에 맞는" 것으로 보인다. 타격은 팔이 뻗은 순간(마지막 직전)이다
- ⚠️ 프레임 수를 줄이거나 늘리면 `SpriteBank.MOB_FRAMES`·로더·기존 반입물 4종이 함께 깨진다. 계약 변경은 코드와 자산을 같은 커밋에서 마이그레이션할 때만 허용한다

### 1-3. 향후 역할·계열별 추가 상태 — **현재 생성 금지**

> 아래 상태는 기획 타이밍 명세다. 현재 런타임 슬롯이 없으므로 스프라이트로 만들지 않는다. 소비처가 구현되면 코드 이펙트로 충분한지 먼저 검증하고, 전용 작화가 꼭 필요한 상태만 승인한다.
...
| 실루엣 | 현행 상태 | 프레임 | 제작 방향 | 이미지 |
|---|---|---|---|---|
| 잡몹 **추적형** | 공통5 | 22 | **1** | **22** |
| 잡몹 **포위형** | 공통5 | 22 | 1 | 22 |
| 잡몹 **돌진형** | 공통5, 예고·돌진은 코드 연출 | 22 | 1 | 22 |
| 잡몹 **원거리형** | 공통5, 조준선·발사는 코드 연출 | 22 | 1 | 22 |
| `revive`·`explode` | 소비처 구현 전 생성 금지 | — | — | 0 |
| 정예 6유형 | 오버레이 | — | — | **0** (§7) |
| 보스 실루엣 4종 | 정적 1장 | 1 | 1 | **4** — 반입 완료 |

**현재 계약은 잡몹 88장 + 정적 보스 4장**이며 모두 반입 완료다. 새 생성은 `ARTIFACT_INDEX.md`의 미완 항목에만 허용한다.

- 💡 `revive`·`explode`가 **계열 공용 1벌**인 이유: 계열이 색조뿐이라 실루엣이 같다 → 언데드 4종이 각자 부활 모션을 가질 필요가 없다. 같은 모션에 색조만 얹는다
- ✅ **미러링 규칙**: 원본 1방향을 이동 X축에 따라 `flipX`한다. 방향별 파일을 만들지 않는다
- ⚠️ **비대칭 디자인 금지** — 한쪽 어깨에만 견장, 한 손에만 무기 같은 디자인은 `flipX`하면 반대편으로 옮겨간다. 잡몹·정예·보스 전부 좌우 대칭 실루엣을 원칙으로 한다
- 📌 아틀라스 구성은 Unity Sprite Atlas가 담당한다. 생성 단계에서 4096px 단일 시트로 합치지 않는다

---

## 2. 시트 규격 — `split_ai_sheet.py`가 읽는 현행 형태

> 📌 힉스필드 원본은 마젠타 크로마키 배경으로 생성하고, 실제 셀 수를 확인한 뒤 분할·정렬한다. 요청한 격자와 실제 출력이 다르면 자동으로 이름을 밀어 넣지 말고 수동 매핑으로 검수한다.

| 항목 | 값 | 이유 |
|---|---|---|
| 배경 | **마젠타 크로마키** | 배경 제거·셀 경계 검출이 안정적 |
| 시트 크기 | 생성 결과 원본 유지 | 요청 격자와 실제 격자를 픽셀로 실측 |
| 분할 | `split_ai_sheet.py` 자동 검출, 실패 시 행·열 수동 지정 | 이름 밀림 방지 |
| 정렬 | `align_frames.py` | 22장을 공통 캔버스·발밑 기준으로 통일 |
| 출력 | 투명 PNG, Point 필터 | Unity `Resources/sprites/<유닛ID>/` 반입 |

- ⚠️ 생성기가 요청과 다른 4×4·5×2 격자를 내는 사례가 있다. 셀 수와 자세를 눈으로 확인하기 전에는 자동 반입하지 않는다
...
- ✅ `mob_chaser`·`mob_swarmer`·`mob_charger`·`mob_ranged`는 현재 각각 22프레임이 반입 완료됐다. 같은 ID로 다시 생성·덮어쓰기 전에 `ARTIFACT_INDEX.md`의 원본·반입물·소비처를 확인한다
...
> 보스도 잡몹과 같이 **1방향 + `flipX`**이므로 좌우 대칭을 유지한다. 정적 실루엣 4종은 이미 반입됐고, 애니메이션은 런타임 소비처가 생기기 전에는 만들지 않는다.
...
- 💡 **20보스 → 실루엣 4개 매핑이 위 표다.** 개별 보스는 **색조 + 크기 + 부착 장식(뿔·왕관·날개·후광) 오버레이**로 구분한다. 방향 이미지는 추가하지 않는다
...
4. **배경** — 마젠타 크로마키가 균일한가? 그라디언트가 남아 있으면 `split_ai_sheet.py`의 셀 검출과 배경 제거가 흔들린다
...
| 순서 | 제작물 | 장수 | 왜 지금 |
|---|---|---|---|
| 1 | 잡몹 실루엣 4종 × 22프레임 × 1방향 | **88** | ✅ 반입 완료 |
| 2 | 정적 보스 실루엣 4종 × 1방향 | **4** | ✅ 반입 완료 |
| 3 | `revive`·`explode`·전용 돌진 프레임 | **0 현재** | 코드 소비처와 VFX 대체 여부 검증 뒤 결정 |
| 4 | 보스 애니메이션 | **0 현재** | 보스 애니메이션 로더·재생 경로 구현 뒤 결정 |
| 5 | 정예 오버레이 | 필요한 공용 오버레이만 | 신규 몸통 스프라이트 금지 |

**W4 핵심 몸통 자산은 이미 반입 완료다.** 이후 작업은 새 몸통을 만드는 것이 아니라 기존 자산의 소비처·행동 예고·화면 가독성을 검증하는 일이다.

- ⚠️ 완료 자산을 같은 ID로 재생성하지 않는다. 교체가 필요하면 기존 화면과 새 후보를 같은 캡처에서 비교하고 채택 근거를 `ARTIFACT_INDEX.md`에 남긴다
