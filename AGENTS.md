...
- **에이전트 8명**: 예원(CEO·워치독)·영숙(비서·텔레그램)·봄이(QA)·수리(Dev)·테오(Test)·백호(Backend)·미오(Design)·나무(PM). 로스터·역할·도구·자동 개발 루프는 [CLAUDE.md](CLAUDE.md) 「Agent Roster」가 원천, 각 봇 헌장은 `projects/ai-team/skills/<봇>/SKILL.md`.
