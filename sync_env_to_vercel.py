...
# Vercel에 환경 변수 추가
def add_env_to_vercel(project_id, token, key, value, env_type="production"):
...
def main():
    ...
    # Vercel 프로젝트 정보
    project_id = env_vars.get("VERCEL_PROJECT_ID", "prj_R3ATj3VblwMxJswmsXzwDvMPkCcj")
    ...
    print(f"\n프로젝트 ID: {project_id}")
    print(f"업로드할 변수: {len(upload_vars)}개\n")
    ...
    for key in upload_vars:
        ...
        print(f"📤 {key}...", end=" ")
        success, result = add_env_to_vercel(project_id, vercel_token, key, value)
