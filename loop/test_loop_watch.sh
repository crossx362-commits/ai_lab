...
count_start() {
  ...
  grep -c ':start\b' "$TEST_ROOT/control.calls" || true
}

assert_no_start() {
  if [ -f "$TEST_ROOT/control.calls" ] && grep -q ':start\b' "$TEST_ROOT/control.calls"; then
    echo "FAIL: $1 상태에서 감시자가 start를 호출했다"
    exit 1
  fi
}
...
run_watch
assert_no_start quota_wait

rm -f "$TEST_ROOT/control.calls"
set_phase recovering claude
run_watch
assert_no_start recovering

rm -f "$TEST_ROOT/control.calls"
set_phase owner_stopped claude
run_watch
assert_no_start owner_stopped

rm -f "$TEST_ROOT/control.calls"
set_phase running claude
make_heartbeat_stale
run_watch
if [ "$(count_start)" -ne 1 ] || ! grep -q '^1:start claude$' "$TEST_ROOT/control.calls"; then
...
if [ "$(count_start)" -ne 1 ] || ! grep -q '^0:start codex$' "$TEST_ROOT/control.calls"; then
...
echo "PASS: stale running과 사라진 서비스만 단일 control start로 재개한다"
