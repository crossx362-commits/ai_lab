...
CONTROL_LOCK="$LOOP_DIR/.control-lock"
LOCK_ATTEMPTS="${LOOP_CONTROL_LOCK_ATTEMPTS:-120}"
LOCK_WAIT_SECONDS="${LOOP_CONTROL_LOCK_WAIT_SECONDS:-0.25}"
LOCK_HELD=0

if ! [[ "$WAIT_ATTEMPTS" =~ ^[0-9]+$ && "$STOP_WAIT_ATTEMPTS" =~ ^[0-9]+$ \
    && "$LOCK_ATTEMPTS" =~ ^[1-9][0-9]*$ \
...
acquire_control_lock() {
  ...
  while ! mkdir "$CONTROL_LOCK" 2>/dev/null; do
    if [ -f "$CONTROL_LOCK/pid" ]; then
      owner="$(tr -d '[:space:]' < "$CONTROL_LOCK/pid" 2>/dev/null || true)"
      case "$owner" in
        ''|*[!0-9]*) ;;
        *)
    ...
    if [ "$attempt" -ge "$LOCK_ATTEMPTS" ]; then
      echo "제어 실패: 다른 시작/중단 명령이 끝나지 않았습니다." >&2
      return 1
    fi
...
service_uses_repo_source() {
  service_info | grep -Fq "$ROOT/loop/loop.sh"
}
...
wait_service_unloaded() {
  ...
  while service_loaded; do
    if [ "$attempt" -ge "$STOP_WAIT_ATTEMPTS" ]; then
      return 1
    fi
    attempt=$((attempt + 1))
    [ "$attempt" -le "$STOP_WAIT_ATTEMPTS" ] && sleep "$WAIT_SECONDS"
  done
  return 0
}

unload_main_service() {
  local provider="$1" reason="$2" bootout_rc=0
  "$LAUNCHCTL" bootout "$SERVICE" >/dev/null 2>&1 || bootout_rc=$?
  if wait_service_unloaded; then
    return 0
  fi
  state_set recovering "$provider" "$reason: bootout=$bootout_rc, 서비스 소멸 미확인" || true
  echo "시작 실패: 기존 launchd 서비스가 사라지지 않았습니다." >&2
  return 1
}
...
start_loop() {
  local provider before_heartbeat pid clear_stop="${2:-1}"
  provider="$(configured_agent "${1:-}")" || return $?
  if [ "$clear_stop" = "0" ]; then
    if [ -f "$STOP_FILE" ] || [ "$(state_get phase)" = "owner_stopped" ]; then
      echo "중단 유지: owner_stopped"
      return 0
    fi
  fi
  ...
  if service_loaded && ! service_uses_repo_source; then
    state_set recovering "$provider" "예전 실행 경로를 저장소 원본으로 교체" || true
    unload_main_service "$provider" "예전 실행 경로 교체 실패" || return 1
  fi

  if [ "$FORCE_RESTART" = "1" ]; then
    state_set recovering "$provider" "heartbeat 정체로 원본 재기동" || true
    if service_loaded; then
      unload_main_service "$provider" "heartbeat 정체 서비스 종료 실패" || return 1
    fi
  fi
...
case "${1:-}" in
  start) with_control_lock start_loop "${2:-}" ;;
  ensure-running) with_control_lock start_loop "${2:-}" 0 ;;
  stop) with_control_lock stop_loop ;;
  status) show_status ;;
  *)
    echo "사용법: bash loop/control.sh <start|ensure-running|stop|status> [claude|codex|grok|opencode]" >&2
    exit 2
    ;;
esac
