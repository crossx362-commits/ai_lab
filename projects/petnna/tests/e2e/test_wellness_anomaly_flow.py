...
올바르게 노출/억제되는지 회귀 검증한다. 세 시나리오를 다룬다:
  ...
  2) 정상 시드   → 경고 미노출('특이사항 없음' 안정 카드, 2026-08-12 문구 통일)
...
def run(page, base_url):
    ...
    # 2026-08-12: "이상 없음"(판정) → "특이사항 없음"(관찰)으로 문구 통일.
    assert "특이사항 없음" in normal_card, f"정상 시드에서 안정 카드 문구가 없음: {normal_card[:120]!r}"
    ...
    # 표본 부족 안내는 '표본 수/목표치'를 보여준다. 예전엔 리터럴 '현재'로 잡았는데,
    # 그건 표기의 일부지 기능이 아니다 — 배지를 진행 표시('10/17일')로 다듬는 개선이
    # 이 단언 하나에 막혀 3회 실패하고 보류됐다(2026-07-28 확인·완화).
    assert "예측 웰니스" in few_card and "10" in few_card, \
