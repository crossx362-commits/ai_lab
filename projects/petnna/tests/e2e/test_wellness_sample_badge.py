...
여기서는 배지 자체의 구조를 본다:
  ...
  2) 배지가 '현재' + 실제 표본 수를 표시한다(형식은 '현재 N일' 또는 '현재 N/17일' 모두 허용
     — 미래 표기 변경에도 견디도록 '현재'와 표본 수만 단언).
...
def run(page, base_url):
    ...
    # 배지: '현재' + 실제 표본 수(형식 무관 — '현재 10일' / '현재 10/17일' 모두 허용)
    assert "현재" in card10, f"배지에 '현재' 표기가 없음: {card10[:150]!r}"
    ...
    assert "예측 웰니스" in card16 and "현재" in card16, \
