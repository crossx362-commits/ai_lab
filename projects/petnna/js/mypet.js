...
function renderMyPets() {
    ...
    if (graphicContainer) {
        if (current.type === 'custom') {
            graphicContainer.innerHTML = `<img loading="lazy" src="${escapeHtml(current.imageUrl)}" class="w-full h-full object-cover rounded-xl animate-pet-body" onerror="avatarFallback(this)">`;
        } else {
            let svgMarkup = '';
            if (current.type === 'cat') {
