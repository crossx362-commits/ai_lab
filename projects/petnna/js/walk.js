...
let drawingRouteWaypoints = WalkModule.drawingRouteWaypoints;
...
const WALKMATE_SAMPLE_MATES = [
    { name: '초코 아빠', pet: '초코 (푸들)', dong: '역삼동', time: '아침', emoji: '🐩' },
    { name: '보리 언니', pet: '보리 (믹스견)', dong: '역삼동', time: '저녁', emoji: '🐕' },
    { name: '나비 집사', pet: '나비 (코숏)', dong: '삼성동', time: '저녁', emoji: '🐈' },
    { name: '몽이 누나', pet: '몽이 (말티즈)', dong: '삼성동', time: '아침', emoji: '🐶' },
    { name: '두부 아빠', pet: '두부 (시바)', dong: '연남동', time: '오후', emoji: '🐕‍🦺' },
    { name: '루비 집사', pet: '루비 (포메)', dong: '연남동', time: '저녁', emoji: '🦮' },
];

function getWalkMateState() {
    try {
        return JSON.parse(localStorage.getItem(WALKMATE_STORAGE_KEY)) || {};
    } catch (e) {
        return {};
    }
}

function saveWalkMateState(state) {
    localStorage.setItem(WALKMATE_STORAGE_KEY, JSON.stringify(state));
}

function renderWalkMate() {
    const box = document.getElementById('walkmate-section');
    if (!box) return;
    ...
    const matesHtml = mates.length
        ...
        : `<div class="text-center py-5 space-y-1">
                ...
                <p class="text-[10px] text-gray-300">이웃이 옵트인하면 여기에 나타나요!</p>
           </div>`;
    ...
    box.innerHTML = `
        <div class="space-y-2.5">
            ...
            <p class="text-[9px] text-gray-300 text-center">베타 샘플 프로필입니다. 실제 이웃 매칭은 순차 오픈돼요.</p>
        </div>`;
}

function optInWalkMate() {
    const input = document.getElementById('walkmate-dong-input');
    const dong = input ? input.value.trim() : '';
    if (!dong) {
...
function greetWalkMate(name) {
    if (typeof showToast === 'function') showToast(`${name}님에게 산책 인사를 보냈어요! 🐾`);
}
...
function startRouteDrawingMode() {
    ...
    // 배열만 비우면 이전에 그린 마커·선이 지도에 그대로 남는다 — 빌더를 다시 열 때마다
    // 번호 마커가 겹겹이 쌓여 "점이 너무 많이 찍힌" 것처럼 보였다(2026-07-25).
    ...
    if (WalkModule.myLocationMarker) {
        ...
        drawingRouteWaypoints.push([pos.lat, pos.lng]);
        drawingRoutePoints = [[pos.lat, pos.lng]];
...
// 사용자가 찍은 지점(drawingRouteWaypoints)과 화면에 그리는 도로 경로(drawingRoutePoints)를
// 분리한다. 예전에는 OSRM이 돌려준 도로 꼭짓점을 전부 drawingRoutePoints에 밀어넣고
// 그 배열로 마커를 그려서, 두 번만 클릭해도 번호 마커가 수십 개씩 찍혔다(2026-07-25 오너 신고).
// 마커·드래그·삭제는 waypoints 기준, 선과 저장은 도로 경로 기준.
function _pathLengthKm(points) {
    let d = 0;
    for (let i = 1; i < (points || []).length; i++) {
        d += getDistanceKm(points[i - 1][0], points[i - 1][1], points[i][0], points[i][1]);
    }
    return d;
}

// 도로 꼭짓점 배열에서 앵커로 쓸 지점만 균등 추출(양끝은 항상 포함).
function _thinToAnchors(points, max) {
    if (!Array.isArray(points) || points.length <= max) {
        return (points || []).map(p => p.slice());
    }
    const step = (points.length - 1) / (max - 1);
    const out = [];
    for (let i = 0; i < max; i++) {
        out.push(points[Math.round(i * step)].slice());
    }
    return out;
}

async function rebuildDrawnRoutePath() {
    if (drawingRouteWaypoints.length === 0) {
        drawingRoutePoints = [];
        return true;
    }
    if (drawingRouteWaypoints.length === 1) {
        drawingRoutePoints = [drawingRouteWaypoints[0].slice()];
        return true;
    }
    const road = await routeViaOSRM(drawingRouteWaypoints);
    const ok = !!(road && road.length > 1);
    drawingRoutePoints = ok ? road : drawingRouteWaypoints.map(p => p.slice());
    return ok;
}
...
async function addPointToDrawnRoute(lat, lng) {
    ...
    drawingRouteWaypoints.push([lat, lng]);

    if (drawingRouteWaypoints.length === 1) {
        drawingRoutePoints = [[lat, lng]];
        reRenderDrawingRoute();
    ...
    let onRoad;
    try {
        onRoad = await rebuildDrawnRoutePath();
    } finally {
        // finally로 풀지 않으면 예외 한 번에 빌더가 "계산 중"으로 영구히 잠긴다.
        _routingBusy = false;
    }

    reRenderDrawingRoute();
    showToast(onRoad
        ? `🛣️ 지점 ${drawingRouteWaypoints.length} 추가 (도로 따라 연결)`
        : '📍 지점 추가 (오프라인: 직선 연결)');
}

// 루트 자동 생성 — 현재 위치(없으면 지도 중심)를 기점으로 목표 거리에 맞는
// 순환 코스를 만들어 '빌더 안에' 올린다. 바로 저장하는 랜덤 코스(generateRandomRoute)와
// 달리, 지점을 드래그·삭제해 다듬은 뒤 저장할 수 있다.
async function autoGenerateDrawnRoute(targetKm) {
    if (!mapInstance) { showToast('⚠️ 지도를 먼저 로드해주세요.'); return; }
    if (_routingBusy) { showToast('⏳ 경로 계산 중입니다...'); return; }

    const km = Number(targetKm) || 2;
    let lat, lng;
    if (WalkModule.myLocationMarker) {
        const pos = WalkModule.myLocationMarker.getLatLng();
    ...
    // 경유지가 많을수록 각 지점을 거치느라 우회가 쌓여 짧은 코스를 만들 수 없다.
    // 목표가 짧으면 지점 수를 줄인다(마커도 덜 지저분해진다).
    const legs = km <= 1.5 ? 3 : km <= 3 ? 4 : 6;
    const latPerM = 1 / 111000;
    const lngPerM = 1 / (111000 * Math.cos(lat * Math.PI / 180));
    const spin = Math.random() * 2 * Math.PI;   // 매번 다른 방향으로 돌게
    // 경유지별 반경 흔들림은 한 번만 뽑아 재시도 내내 재사용한다 — 매번 새로 뽑으면
    // 반경을 조정해도 모양이 통째로 바뀌어 목표 거리로 수렴하지 않는다.
    const jitter = Array.from({ length: legs }, () => 0.8 + Math.random() * 0.4);
    const start = [lat, lng];

    const buildWaypoints = (radiusM) => {
        const wps = [start.slice()];
        for (let i = 1; i <= legs; i++) {
            const angle = spin + (i / (legs + 1)) * 2 * Math.PI;
            const r = radiusM * jitter[i - 1];
            wps.push([
                lat + Math.sin(angle) * r * latPerM,
                lng + Math.cos(angle) * r * lngPerM
            ]);
        }
        wps.push(start.slice());   // 출발지로 복귀 = 순환 코스
        return wps;
    };

    _routingBusy = true;
    showToast(`🪄 ${km}km 순환 코스를 도로에 맞춰 생성 중...`);

    // 도로를 따라가면 직선 둘레보다 얼마나 늘어나는지는 동네 도로망마다 크게 다르다
    // (도심은 3배 가까이 늘기도 한다). 고정 보정계수 대신 실제 거리를 재서 반경을
    // 비례 보정하는 걸 몇 번 반복해 목표 거리에 맞춘다.
    let radiusM = Math.max(120, (km * 1000) / (2 * Math.PI));
    let best = null;
    let onRoad = false;

    try {
        for (let attempt = 0; attempt < 4; attempt++) {
            drawingRouteWaypoints = buildWaypoints(radiusM);
            onRoad = await rebuildDrawnRoutePath();
            const d = _pathLengthKm(drawingRoutePoints);

            if (!best || Math.abs(d - km) < Math.abs(best.km - km)) {
                best = { km: d, wps: drawingRouteWaypoints, pts: drawingRoutePoints, onRoad };
            }
            // 오차 15% 이내면 충분. 도로 경로를 못 얻었으면 반복해도 같은 직선이라 의미 없다.
            if (!onRoad || d <= 0 || Math.abs(d - km) / km <= 0.15) break;

            // 한 번에 너무 크게 흔들면 발산하므로 배율을 제한한다.
            radiusM = Math.max(80, radiusM * Math.max(0.4, Math.min(2.2, km / d)));
        }

        drawingRouteWaypoints = best.wps;
        drawingRoutePoints = best.pts;
        onRoad = best.onRoad;
    } finally {
        // finally로 풀지 않으면 예외 한 번에 빌더가 "계산 중"으로 영구히 잠긴다.
        _routingBusy = false;
    ...
    const desc = document.getElementById('route-draw-banner-desc');
    if (desc) desc.innerText = '자동 생성된 코스예요. 지점을 드래그하거나 더블클릭해 다듬은 뒤 저장하세요.';

    // 동네 도로망에 따라 목표보다 훨씬 길게밖에 못 만드는 곳이 있다(도보 경로가 성긴 지역).
    // 그럴 때 목표 거리를 그대로 부르면 거짓말이 되므로, 실제 거리와 함께 사정을 알린다.
    if (!onRoad) {
        showToast(`🪄 ${dist.toFixed(2)}km 코스 생성 (오프라인: 직선 연결)`);
    } else if (Math.abs(dist - km) / km > 0.25) {
        showToast(`🪄 ${dist.toFixed(2)}km 코스 생성 — 이 근처 도보 길 사정상 ${km}km에 맞추기 어려웠어요. 지점을 옮겨 조절해보세요.`);
    } else {
        showToast(`🪄 ${dist.toFixed(2)}km 코스 생성! 마음에 안 들면 다시 눌러보세요.`);
    }
}

// 배너의 거리 셀렉트를 읽어 자동 생성
function autoGenerateDrawnRouteFromUI() {
    const sel = document.getElementById('route-auto-distance');
    return autoGenerateDrawnRoute(sel ? sel.value : 2);
}
...
function reRenderDrawingRoute() {
    ...
    // Redraw markers — 사용자가 찍은 지점만. 도로 꼭짓점까지 마커를 달면
    // 클릭 두 번에 번호 마커가 수십 개 찍힌다(2026-07-25 수정).
    drawingRouteWaypoints.forEach((pt, i) => {
        const index = i + 1;
        const lat = pt[0];
        const lng = pt[1];
        ...
        marker.on('dragend', async function (event) {
            L.DomEvent.stopPropagation(event);
            lastMapInteractionTime = Date.now();
            // 계산 중에 옮기면 계산이 끝나며 waypoints를 덮어써 이동이 사라진다 —
            // 마커를 원위치로 되돌리고 거절한다(삭제·취소와 같은 이유).
            if (_routingBusy) {
                event.target.setLatLng(L.latLng(pt[0], pt[1]));
                showToast('⏳ 도로 경로 계산 중입니다...');
                return;
            }
            const newLatLng = event.target.getLatLng();
            drawingRouteWaypoints[i] = [newLatLng.lat, newLatLng.lng];
            // 지점을 옮겼으면 그 사이 도로 경로도 다시 계산해야 선이 실제 길을 따라간다.
            _routingBusy = true;
            try {
                await rebuildDrawnRoutePath();
            } finally {
                _routingBusy = false;
            }
            reRenderDrawingRoute();
...
async function removePointFromRoute(idx) {
    // 경로 계산 중에 지점을 지우면, 계산이 끝나며 waypoints를 통째로 덮어써
    // 삭제가 조용히 되돌아간다(2026-07-25 검수에서 재현).
    if (_routingBusy) { showToast('⏳ 도로 경로 계산 중입니다...'); return; }
    drawingRouteWaypoints.splice(idx, 1);
    _routingBusy = true;
    try {
        await rebuildDrawnRoutePath();
    } finally {
...
async function undoLastRoutePoint() {
    if (_routingBusy) { showToast('⏳ 도로 경로 계산 중입니다...'); return; }
    if (drawingRouteWaypoints.length === 0) {
        showToast("⚠️ 취소할 지점이 없습니다.");
        return;
    }
    drawingRouteWaypoints.pop();
    _routingBusy = true;
    try {
        await rebuildDrawnRoutePath();
    } finally {
        ...
        if (rIndex !== -1) {
            ...
            showCustomDialog({
                ...
                onConfirm: (name) => {
                    ...
                    customRoutes[rIndex].waypoints = drawingRouteWaypoints;
        ...
        showCustomDialog({
            ...
            onConfirm: (name) => {
                ...
                const newRoute = {
                    ...
                    waypoints: drawingRouteWaypoints,
                    distance: distStr,
                    favorite: false,
                    createdAt: Date.now()
                };
...
function startRouteEditingMode(routeId) {
    ...
    // waypoints 필드는 2026-07-25부터 저장된다. 그 이전 경로는 도로 꼭짓점만 남아 있어
    // 전부 마커로 띄우면 다시 지저분해지므로, 최대 8개만 균등 추출해 앵커로 쓴다.
    drawingRouteWaypoints = Array.isArray(r.waypoints) && r.waypoints.length
        ? JSON.parse(JSON.stringify(r.waypoints))
        : _thinToAnchors(drawingRoutePoints, 8);
