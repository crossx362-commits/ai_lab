...
<div class="space-y-4 animate-fade-in">
    ...
    <!-- items-start는 flex-col(모바일)에서 교차축=가로가 되어 지도 카드가 내용 너비로
         쪼그라들었다(오른쪽 32px 여백) — 사이드바는 w-full이라 멀쩡해 카드만 좁아 보였다.
         기본값 stretch로 두어 모바일에서도 가로 폭을 꽉 채운다(2026-07-25). -->
    ...
    <div class="flex-1 min-w-0 bg-white rounded-2xl p-4 border border-amber-50 shadow-sm relative overflow-hidden sm:flex sm:flex-col">
        ...
        <!-- ── 지도 (Critical: 최소 260px 확보) ──────────────────────────────────
             모바일 높이 45vh→38vh(2026-07-29): 45vh는 지도가 뷰포트 세로 절반 이상을
             차지해 하단 컨트롤·리스트가 스크롤 없이는 안 보였다. 38vh(~253~308px)면
             핵심 CTA가 첫 화면에 함께 잡힌다. 데스크톱(sm+)은 무변경. -->
        <div class="relative w-full bg-gray-100 rounded-2xl overflow-hidden border border-amber-100/50 shadow-inner z-10 h-[clamp(260px,38vh,520px)] sm:h-auto sm:flex-1 sm:min-h-[360px]">
        ...
        <div id="route-draw-banner" class="hidden mt-3 p-4 bg-brand-50 border border-brand-200 rounded-2xl animate-fade-in flex flex-col gap-2.5"
            ...
            <!-- 자동 생성: 현재 위치 기준 순환 코스를 빌더에 올린다(저장 전에 다듬을 수 있음) -->
            <div class="flex items-center gap-2 bg-white border border-brand-200 rounded-xl p-2">
                <label for="route-auto-distance" class="text-[11px] font-bold text-gray-600 shrink-0 pl-1">목표 거리</label>
                <select id="route-auto-distance"
                    class="flex-1 min-w-0 text-xs font-bold text-gray-700 bg-transparent outline-none cursor-pointer">
                    <option value="1">1km · 가볍게</option>
                    <option value="2" selected>2km · 보통</option>
                    <option value="3">3km · 넉넉히</option>
                    <option value="5">5km · 길게</option>
                </select>
                <button onclick="autoGenerateDrawnRouteFromUI()"
                    class="bg-brand-100 hover:bg-brand-200 text-brand-800 font-bold text-xs py-2 px-3 rounded-lg flex items-center gap-1.5 transition-all shrink-0 min-h-[36px]">
                    <i class="fa-solid fa-wand-magic-sparkles text-sm"></i> 자동 생성
                </button>
            </div>
