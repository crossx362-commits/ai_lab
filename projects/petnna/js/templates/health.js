...
<div class="space-y-4 animate-fade-in">
    ...
    <div class="glass rounded-2xl shadow-soft-lg border border-brand-100/50 overflow-hidden">
        ...
        <details id="health-top-analysis" class="health-top-analysis group/topfold border-t border-gray-100">
            ...
            <div class="health-top-analysis-body lg:grid lg:grid-cols-2 lg:gap-x-4 lg:items-start">
                ...
                <!-- 🗓️ 이번 주 컨디션·급여·활동 요약 (weekly-condition-report.js가 채움) -->
                <div id="weekly-condition-report-card"></div>
    ...
    <!-- '오늘 기록' 그룹 이브로우는 제거(미오 P3, 2026-08-06) — 이 그룹은 카드 한 장뿐이라
         이브로우 "오늘 기록"이 바로 아래 카드 제목 "오늘의 기록"과 동일 문구를 이중 노출했다.
         카드 자체 제목으로 통합한다(다른 두 그룹은 카드가 여러 장이라 이브로우 유지). -->
    ...
    <div id="health-today-record-card" class="card-modern shadow-soft-lg p-5">
        ...
        <!-- 🐾 행동 큐 감정 디코더 (behavior-decoder.js가 채움) — 백로그 나무 P2.
             꼬리·귀·자세·발성을 골라 규칙 기반으로 감정·대응법을 리포트하고
             감정 로그를 누적. 컨디션(몸)과 감정(마음)을 같은 카드에 나란히 남긴다. -->
        <div id="behavior-decoder-widget" class="mt-4"></div>
