...
-- 블로그 글 댓글 (post_id는 posts.js의 id 필드와 매칭)
...
create index if not exists homepage_blog_comments_post_id_idx on public.homepage_blog_comments (post_id);
...
create policy "bc anon read" on public.homepage_blog_comments for select using (true);
drop policy if exists "bc anon insert" on public.homepage_blog_comments;
create policy "bc anon insert" on public.homepage_blog_comments for insert with check (true);
