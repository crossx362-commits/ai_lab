...
namespace Ulon.Editor
{
    public static partial class VisualSliceBuilder
    {
        ...
        /// 반경은 **돌 깔린 네모와 같은 자리**다(`WorldSplat.PlazaCoverAt`의 square, 모듈 4칸).
        /// 처음엔 「가로지른 여섯(3.4~4.3m)과 결백한 것(5.2m) 사이」로 4.75m를 잡았는데,
        /// **화면에서는 그대로 담이 마당을 갈랐다**(검수 반려 2026-09-09) — 내가 고른 속이
        /// 돌포장보다 훨씬 작아서 포장 위 담이 전부 밖으로 빠졌다. 자는 내가 정한 속이 아니라
        /// **보는 사람이 마당으로 읽는 넓이**를 재야 한다: 돌이 깔린 데까지가 마당이다.
        /// 값은 여기 하나뿐이고, 자(`SliceSelfCheck.CollectPlazaCrossers`)와 세는 도구가 이것을 읽는다.
        /// </summary>
        public const float PlazaYard = 4f * KitScale;
