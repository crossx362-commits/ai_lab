...
namespace Ulon.Editor
{
    public static partial class SliceSelfCheck
    {
        ...
        ///
        /// 하한은 **살아 있는 샷들을 재서** 잡았다(실측 2026-09-09, 18장):
        /// 근접 5.8~9.0 · 마을 6.3~21.1 · 지역 7.4~25.6 · 입구 14.7~24.8 · 실내 60.5~100.
        /// 살아 있는 최저가 5.8%(`22_mob_closeup`, 화면에서 몹이 한가운데 크게 선다)이므로
        /// 하한은 그 절반인 **3%**다 — 살아 있는 샷에 여유를 주고 죽은 장(0.1%)만 문다.
        /// 여유를 안 주면 산포가 조금 흔들릴 때마다 자가 울고, 우는 자는 곧 꺼진다.
        ///
        /// NC 둘: ①주인공을 숨긴다 ②카메라를 90° 돌린다 — 둘 다 하한 아래로 떨어져야 한다.
        /// </summary>
        const float SubjectShareMin = 0.03f;
        ...
        static void AssertShotSubjectFramed()
        {
            ...
            for (int i = 0; i < shots.Length; i++)
            {
                ...
                float share = SubjectShare(shots[i], objects, out string absent);
                if (absent.Length > 0)
                ...
                measured++;
                if (share < worst) { worst = share; worstName = name; }
                if (share < SubjectShareMin)
                    throw new InvalidOperationException("샷 " + name + "의 주인공이 화면의 " +
                        (share * 100f).ToString("0.0") + "%뿐입니다(하한 " + (SubjectShareMin * 100f).ToString("0") +
                        "%) — 이 장은 아무것도 안 보고 있습니다. 카메라 자리를 **대상에서 유도**하십시오.");
            }
            if (measured == 0)
                throw new InvalidOperationException("주인공을 잰 샷이 0장입니다 — 원장이 비었거나 샷 이름이 바뀐 것입니다(자가 무력합니다).");
            report = " · 잰 샷 " + measured + " · 최악 " + worstName + " " + (worst * 100f).ToString("0.0") +
                     "% · 못 잼(지형) " + unmeasurable;
            Debug.Log("[Ulon] 샷 주인공 프레임 통과 — 하한 " + (SubjectShareMin * 100f).ToString("0") + "%" + report);
        ...
        static void AssertShotSubjectNegativeControl(QaShots.Shot[] shots)
        {
            ...
            if (idx < 0 || !ShotSubject.Table.TryGetValue(target, out string[] objects) || objects.Length == 0)
