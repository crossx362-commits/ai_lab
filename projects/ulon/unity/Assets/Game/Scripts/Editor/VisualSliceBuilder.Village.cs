...
namespace Ulon.Editor
{
    public static partial class VisualSliceBuilder
    {
        ...
        static void EnsureHuntMob(string goName, string mobId, string fbx, Vector3 pos)
        {
            ...
            {
                BindMob(go, mobId);
                // **찾았다고 그대로 두면 옛 자리에 묻힌 채 남는다.** 이 자리들은 지형이 생기기 전
                // 평지(y=0) 시절에 놓인 것이고, 그 뒤 아무도 높이를 다시 유도하지 않았다 —
                // 실측(2026-09-09): 기사·도적의 몸이 지표 10.0m 아래 y=0.0에 묻혀 있었고,
                // 화면에는 그 밑에 매달린 사본이 서 있었다. 높이는 **매 판 지표에서 다시 유도한다**.
                SnapRootToGround(go);
                return;
            }
