...
namespace Ulon.Editor
{
    public static partial class SliceSelfCheck
    {
        ...
        ///   ①**밝기 차**(얕은 띠 − 열린 물) = 깊이 페이드. 실측 `64` 48.8 · `15` 78.0.
        ///     거품만 꺼도 43.4로 거의 안 떨어진다(대부분 깊이 색의 몫) — 이 수 하나로는
        ///     **거품이 통째로 사라져도 초록불**이다. 깊이 색까지 끄면 6.1로 무너진다.
        ///   ②**얕은 띠의 흰 몫**(RGB 모두 215↑) = 거품. 실측 `64` 6.2% · `15` 27.6%.
        ///     거품을 끄면 0.4%로 무너진다. ①의 구멍을 이 수가 막는다.
        ...
        /// </summary>
        const float ShoreFadeMin = 25.0f;     // 실측 48.8·78.0 대 NC 6.1
        const float ShoreFoamMin = 0.02f;     // 실측 6.2%·27.6% 대 NC 0.4%
