...
namespace Ulon.Editor
{
    public static partial class VisualSliceBuilder
    {
        ...
        public static void EnsureVillageTerrain()
        {
            ...
            // **자갈은 젖은 돌 부스러기지 마른 모래가 아니다**(2026-09-09, `63_pier_cutface`).
            // 호수 절개면(수면 +0~6m, 경사 31~35°)은 셈으로는 자갈 77~94%인데 화면은 크림색 186으로
        ...
        static void EnsureWater()
        {
            ...
            if (mat != null)
            {
                ...
                if (wshader != null)
                {
                    ...
                    // **1.6단계 — 띠를 좁히고 벽에서는 껐다**(검수 반려 2026-09-11).
                    // 셈(`OutdoorCensus.RunFoamExtent`)이 원인을 못박았다: `15` 호수의 가장 깊은
                    // 흰 픽셀이 **0.57m**로 상한(0.60)에 붙어 있었고 그 자리 원장 기울기는 **8°**였다.
                    // 8°의 가로폭 문턱은 0.28m인데 화면이 0.57m까지 하얬다 = **화면 법선이 실제보다
                    // 가파르게 튄다**. 그래서 완경사에서는 늘 상한이 지배하고, 그 상한이 호수를
                    // **흰 도넛**(물 면적의 18.9%)으로 만들었다. 상한을 절반으로 내린다.
                    // 폭을 4m로 키운 것은 **법선 부호를 고친 뒤** 다시 고른 값이다(그 전에는 법선이
                    // 늘 가팔라 상한이 지배했다). 상한 0.45는 이제 **벽 쪽 안전장치**일 뿐이다.
                    mat.SetFloat("_FoamWidthM", 4.0f);
                    // (문턱 하한 `_FoamDepth`는 1.7단계에서 **없앴다** — 0으로 두면 쓰이지 않는
                    //  죽은 손잡이인데 NC 경로만 그걸 만지고 있었다. 벽에서 띠를 끄는 일은
                    //  45° 컷과 화면 깊이 기울기가 한다.)
                    // **물가 순백 선 깨기**(1.7b). 검수 처방은 「폭이 좁아지면 알파도 같이 죽인다」
                    // 였는데 **셈이 그 가설을 부쉈다**: 문제의 `64` 좌안 선은 흰 픽셀 세로런
                    // 중앙값 **3px**이고, 검수가 받은 `15` 호수 띠는 **1px**이다 — 나쁜 쪽이 더
                    // 두껍다. 좁아서가 아니라 **고르게 이어져** 계단 노치를 그대로 그려서 나쁘다.
                    // 그래서 폭이 아니라 이어짐을 끊는다(0 = NC, 그 판에서 선이 되살아나야 한다).
                    mat.SetFloat("_FoamBreak", 0.55f);
                    mat.SetFloat("_FoamBreakScale", 1.6f);   // 60cm 칸 두 겹 — 물가 선을 무리로 쪼갠다
                    // **1.7c — 양이 아니라 자리를 흔든다**(검수 판정 ⓑ). 흰 양을 깎는 길은
                    // 게이트 하한 2%와 정면으로 부딪혔다(0.8이면 1.7%). 거품이 **시작되는
                    // 깊이**를 월드 잡음으로 ±12cm 밀고 당기면 흰 몫은 거의 그대로인 채
                    // 물가가 계단 노치를 따라가지 않는다. 1.25m 칸이라 「우글거림」이 아니라
                    ...
                    // 좁은 쪽에 맞추고 가장자리 페이드를 키워 「가는 선」이 아니라 「번지는 물가」로 읽게 한다.
                    mat.SetFloat("_FoamDepthSteep", 0.30f);
                    mat.SetFloat("_FoamMaxAlpha", 0.75f);     // 거품 안에서도 물색이 비치게(순백 한 겹 금지)
                    mat.SetFloat("_FoamNoiseScale", 0.6f);
                    mat.SetFloat("_FoamJitter", 0.40f);       // 가장자리에 **더한다**(곱하면 폭이 흔들린다)
                    mat.SetFloat("_FoamEdgeSoft", 0.38f);     // 안쪽으로 번지게 — 얇은 띠가 「그린 선」으로 안 읽히게
                ...
                // 0.55는 **흰 구멍만 막았을 뿐 반쪽 수리였다**(검수 판정): 포화가 0이어도 호수가
                // 바다보다 **두 배 밝아** 한 화면에서 같은 물이 두 물감이었다(호수 (180,239,249) 대
                // 바다 (87,151,184)). 눈금을 포화에서 **두 물의 색차**로 바꿔 다시 골랐다
                // (`ExposureCensus.RunWaterTone`) — 밝기 비 0.55→1.59 · 0.40→1.29 · **0.25→1.09** · 0→0.97.
                // 0.25는 비 1.09·채도 차 0.04로 바다와 같은 물로 읽히면서 물결 무늬가 남는다.
