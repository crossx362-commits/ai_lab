        ...
        static void EnsureWater()
        {
            ...
            if (mat != null)
            {
                ...
                if (wshader != null)
                {
                    ...
                    //  45° 컷과 화면 깊이 기울기가 한다.)
                    // **꺼 둔다**(1.7단계). 「띠가 화면에서 몇 px인지 재서 그 아래면 진하기를 죽인다」는
                    // 처방 자체는 셰이더에 들어갔지만 **그 px를 세 번 다 잘못 쟀다**:
                    //   ①`fwidth(diff)` — 완경사에서 「안 좁다」 ②`1/fwidth(foam)` — 띠 안쪽은 값이
                    //   1에 붙어 「무한히 넓다」 ③직접 계산(가로폭×픽셀밀도×기울어짐) — 이번엔
                    //   **어디서나 좁다**고 나와 하한 1.5px에서도 `15` 호수 흰 몫이 4.2% → 0.8%로 무너졌다.
                    // ③은 값이 아니라 **공식이 틀린 것**이다(2m 띠가 40m에서 6px 아래로 나올 수 없다).
                    // 게이트를 완화해 통과시키지 않는다 — 기전은 남기고 **0으로 꺼서** 1.6 화면을
                    // 유지한다. 다음 랩에서 `bandPx`를 화면에 직접 그려 보고(디버그 출력) 고친다.
                    mat.SetFloat("_FoamMinBandPx", 0f);
