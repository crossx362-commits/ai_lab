...
namespace Ulon.Editor
{
    public static partial class VisualSliceBuilder
    {
        ...
        /// **백사장 랩(2026-09-11)에서 512로 올렸다.** 해안 램프를 완만하게 만들자 `64` 근접 샷의
        /// 지면이 카메라 쪽으로 2m 남짓 올라왔고(강 하구 바닥이 SeaFloor 0.4 → 백사장 2.7),
        /// 같은 256에서 중앙값이 **0.89 → 1.02px**로 상한을 넘었다. **상한을 올리지 않는다** —
        /// 상한 1.0px은 「텍셀이 눈에서 사라지는 선」이라는 문장 그대로고, 지면이 가까워졌으면
        /// 갚을 것은 해상도다. 512에서 0.45px·조망 무늬 2.2(하한 2.0)로 둘 다 통과한다.
        internal static int NoiseRes = 512;
        ...
        /// <summary>
        /// 판별 테스트용 **도포(알파맵) 해상도**(−1이면 유니티 기본 512). 빌더는 지금까지 이 값을
        /// **읽기만** 했다 — 높이 격자만 바꿔 가며 「격자 무죄」를 내던 판별이 도포 격자를 한 번도
        /// 만져 본 적 없다는 뜻이다(2026-09-11). 축을 재려면 굽는 쪽을 실제로 바꿔야 한다.
        /// </summary>
        ...
        /// <summary><paramref name="n"/> 자 눈금 위의 흰 잡음(타일 경계에서 되돌아 이어진다).
        /// 텍셀 눈금(`R`)으로 부르면 128칸 계단 없이 낟알을 뿌릴 수 있다.</summary>
        static float HashN(int x, int y, int n)
        {
            x = ((x % n) + n) % n;
            y = ((y % n) + n) % n;
            int h = (x * 374761393) ^ (y * 668265263);
            ...
            for (int y = 0; y < R; y++)
            {
                ...
                for (int x = 0; x < R; x++)
                {
                    ...
                    else if (pattern == 7)
                    {
                        ...
                        // (2026-09-10 이 무늬는 **물러났다** — 128칸 문지름의 두 텍셀 계단이
                        //  근접에서 직조 천으로 읽혔다. 지금 수면은 9이고, 7은 되풀이 자의
                        //  **네거티브 컨트롤**로만 남는다 — 지우면 그 자가 무력해진다.)
                        float sm = 0f;
                    ...
                    {
                        // **물결 — 현 수면 무늬**(검수 판정 2026-09-10, 무늬 7을 대신한다).
                        // 7은 방향 쏠림 0.69로 「도장 아님」 자를 통과했는데도 근접에서 물이
                        // **사선 직조 천**으로 읽혔다 — 쏠림은 생겼으나 **되풀이가 남았다**.
                        //
                        // 범인을 가른 판별 둘: ①수면만 단색으로 구우니 체크가 **사라졌다**
                        // (`builds/qa/water/64_river_bend_flatwater.png`) → 지형이 아니라 무늬다.
                        // ②7의 문지름은 그대로 두고 **낟알만 해시로** 바꾼 판을 지어 보니 되풀이
                        // 봉우리가 0.664로 **거의 그대로**였다 → 범인은 선형 낟알
                        // (`(x*61+y*149)&63`)이 아니라 **128칸 문지름**이다. 텍스처가 256인데
                        // 문지름이 128 눈금 위에서 돌아 한 칸이 두 텍셀 계단으로 깔렸고,
                        // 그 계단이 대각선으로 늘어서 짜임이 됐다. 가설이 틀렸고 셈이 고쳤다.
                        //
                        // 그래서 낟알도 문지름도 **텍셀 눈금**에서 한다. 13텍셀 문지름은
                        // 7칸(=14텍셀)과 같은 세계 길이라 결의 굵기는 그대로다. 표본이 13개로
                        // 늘어 흩어짐이 √(7/13)로 줄므로 대비를 2.2 → 3.0으로 맞춰 세기를 보존한다.
                        // 실측 되풀이 봉우리 **0.705 → 0.073**, 방향 쏠림은 0.69 → 0.60으로 유지.
                        float sm = 0f;
                        for (int k = -6; k <= 6; k++) sm += HashN(x + k, y + k, R);
                        sm /= 13f;
                        float grit = HashN(x + 97, y + 131, R);
                        t = Mathf.Clamp01(0.5f + (sm - 0.5f) * 3.0f + (grit - 0.5f) * 0.22f);
                    }
                    else
                    {
                        // 0 = 잔풀 잡음(기본)
