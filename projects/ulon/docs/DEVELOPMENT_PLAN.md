...
## 오너 우선순위 (2026-09-13) — 플레이 루프 먼저

게임 안에서 바로 쓰는 축을 **그래픽·월드 폴리시보다 앞**에 둔다.

| 순위 | 축 | 그록이 닫을 것 | 완료 증거 |
|---|---|---|---|
| 1 | **전투** | 명중·막기·물리방어·스킬성장·몹/보스 사냥이 플레이에서 이어짐. 2클라에서 HP·피격 동기화 | 호스트/2클라 플레이 샷·로그. 에디터 게이트만으로 완료 금지 |
| 1 | **제작** | 채집 재료→스테이션 제작→내구/수리/Maker Mark→거래 가능 아이템 | 플레이로 광석/나무→철검 등 1사이클 + 가방 반영 |
| 1 | **채집** | 광맥·벌목·아마/자원 고갈·도구 uses | 필드에서 직접 채집 샷·과적/도구 소모 확인 |
| 1 | **소셜** | 안전거래·파티·(있으면) 길드/결투 초대가 화면에 보이고 결과가 맞음 | 가능하면 2클라. 최소 호스트에서 패널+서버 판정 |
| 2 | 받침대 | persist 기동, `two_client_check`, 관심영역·시체/인벤 동기화 | `/ready`·two_client 로그 — 위 1번을 위해 필요할 때만 |
| 3 | 그래픽·월드맵·물가·씬 정리 | Codex/GPT 차선. 그록이 새 카드로 열지 않음 | 기존 차선 유지 |

아래 「이어서 진행할 순서」의 인프라 P0는 **플레이 루프를 막지 않을 때만** 끼워 넣는다. 맵 확대·동화풍 UI는 이 네 축보다 뒤로.
