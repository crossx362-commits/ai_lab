...
> 2026-09-11 현황 대조: 아래 표는 목표 일정이며 달성 판정이 아니다. 현재 구현·검증 차이와
> 실행 우선순위는 [개발계획](DEVELOPMENT_PLAN.md), 기능별 근거는 [구현 현황](DESIGN_COVERAGE.md)을 따른다.
> PostgreSQL·퀵바·행동 SFX/VFX·운영 로그는 이미 코드가 있으므로 재작성보다 검증·복구를 우선한다.
