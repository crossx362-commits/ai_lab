...
## 다음 착수
**입구 근접 샷 과노출**(검수 큐 2): `07` 250↑ 포화 29.2%·RG 동시 21.7%, `11` 20.4/13.1,
`09` 14.9/5.4 대 야외 광각 `02` 2.4/0.1. **원인 이름을 붙이지 말고 세어서 가른다** —
후보(등불 점광 끈 판 / 태양만인 판 / 재질 알베도)를 같은 판에 나란히. 찍기 전 `*_before.png` 사본. 야외·실내 샷을 돌며 새 안건을 세울 것. **인상은 가설이다** — 이번 세션에 화면에서
