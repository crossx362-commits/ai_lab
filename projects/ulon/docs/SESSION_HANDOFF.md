...
## 다음 착수
검수 판정 대기 **셋**: ⓐ 「호수가 만이다」가 결함인가 설계인가 ⓑ 강 폭 15.5m vs 원장 12m 중
어느 쪽이 참인가 ⓒ 강 근접 샷을 세울까(강을 무는 자가 없고 근접 샷도 없다 — 같은 뿌리다).
③은 정정으로 **닫혔다**(전이대는 있다). 판정 전까지 값을 바꾸지 말고, 강 근접 샷 후보 자리를
`15`와 같은 방식(대상에서 유도)으로 **계산만** 해 둔다.
