...
강도 셌다(`Editor/OutdoorCensus.River.cs`, 축 넷): ①끊김 0 ②젖은 폭 평균 22.2m(원장 12m —
하구는 바다와 합쳐진다) ③**강변 모래·자갈 0%**(26곳 전부 풀 — 호수·바다에는 전이대 자가 있는데
강에는 없다) ④조망 넷에만 들고 근접 샷이 없다. 이 자의 구멍을 머리말에 적어 뒀다: ④는 가림을
걷어도 **「보일 자리인가」이지 「읽히나」가 아니다**.
...
## 다음 착수
검수 판정 대기 둘: ⓐ 「호수가 만이다」가 결함인가 설계인가 ⓑ 강 축 셋(③ 강변 전이대 0% →
`AssertShoreBand` 대상 포함 / ② 폭을 원장 12m에 맞출지 지금 폭을 원장으로 삼을지 / 강 근접 샷 신설).
