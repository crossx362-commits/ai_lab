...
8db38fed(원장 레코드 단위 검증 + AssertDataRecordSanity).
...
3. 검수 회신 대기: 방 깊이(`DungeonDepth`) 확대로 실내 전투 시야를 넓힐지 — `LandBase` 상향이 걸려 지형 랩 한 바퀴다.
   회신 없으면 건너뛰고 큐 진행.
4. (검수 관찰, 반려 아님) 지역을 잇는 **길** — 밭·숲·광산이 평지에 떠 있다. 다음 지형 랩에서 고려.
