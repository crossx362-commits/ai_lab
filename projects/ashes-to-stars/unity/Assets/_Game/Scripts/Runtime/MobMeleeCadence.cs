...
{
    /// <summary>§10-2 근접 잡몹 공격 주기. MobDef.공격간격의 런타임 소비처.</summary>
    public static class MobMeleeCadence
    {
        public const string EnvShow = "QA_MOB_MELEE_CADENCE";
        public const string EnvNo = "QA_NO_MOB_MELEE_CADENCE";
        ...
        public static float Seconds()
        {
            ...
            if (ForceDef != null) return Valid(ForceDef.공격간격);
            if (_cached > 0f) return _cached;
            var def = ScriptableObject.CreateInstance<MobDef>();
            _cached = Valid(def != null ? def.공격간격 : Default);
        ...
        public static string Line() => Blocked
            ? ""
            : $"근접 공격 {Seconds().ToString("0.#", System.Globalization.CultureInfo.InvariantCulture)}초(§10-2)";
