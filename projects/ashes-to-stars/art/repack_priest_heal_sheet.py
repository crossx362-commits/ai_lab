#!/usr/bin/env python3
"""사제 치유 4x2 시트를 정수 256px 셀로 의미 보존 재패킹한다."""
...
TARGET = ROOT / "unity/Assets/Resources/FX/priest_heal_sheet.png"
...
def main() -> None:
    source_path = Path(sys.argv[1]) if len(sys.argv) == 2 else TARGET
    source = Image.open(source_path)
    if source_path == TARGET and source.size == (COLS * CELL, ROWS * CELL):
        current = encoded(source.convert("RGBA"))
        assert source.getchannel("A").getbbox() is not None, "가시 픽셀이 없다"
        ...
        return
    assert source.size == (1774, 887), f"예상하지 않은 원본 크기: {source.size}"
    ...
    for row in range(ROWS):
        for col in range(COLS):
            frame_alpha = alpha.crop((col * CELL, row * CELL, (col + 1) * CELL, (row + 1) * CELL))
