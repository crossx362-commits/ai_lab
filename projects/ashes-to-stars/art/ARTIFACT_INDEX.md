...
기획서 `GAME_DESIGN` §3·§0-B·§16 + INBOX 할로우 교체 대조(2026-08-18).

| 기획 | 상태 | 이유 |
|---|---|---|
| 기본 5직업 ×13 | 있음 | 클로드가 할로우 반입 중. 재생성 금지 |
| **1차 전직 10종 전신** | **없음** | `LookDir`이 수호기사=탱, 광전사=탱, 소환사=마법사… 전부 기본 5장. 마법사만 본인 |
| 종족 4 | 안 만듦 | §0-B 팔레트만. 스프라이트 0장 |
...
만들지 않는 것(기획서 §0-B): 8방향, 종족별 실루엣, 보스 20종 전용 모델, 스킬 55종 전용 FX.
