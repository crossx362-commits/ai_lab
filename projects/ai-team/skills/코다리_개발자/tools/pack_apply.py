...
def _load_operator_credentials(brain_root):
    """v7: 운영자(1인 기업)의 자격증명을 두뇌에서 로드. pack_apply 가 키트 HTML/JS
       ...
       자격증명은 외부 연결 패널 (AI Team) 에서 입력. 키트 사용자(고객) 는
       이 키를 볼 일이 없음 — 운영자가 빌드 시점에 박힘. """
    
    # Ollama 활성 모델명 자동 조회 시도
    lm_model = None
    try:
        ...
        if "data" in models_data and isinstance(models_data["data"], list):
            # 임베딩(embedding) 모델 제외한 첫 번째 텍스트 생성 모델 자동 감지
            text_models = [
                m["id"] for m in models_data["data"]
                if "id" in m and "embed" not in m["id"].lower()
            ]
            if text_models:
                # 코다리는 딥시크 전용
                deepseek = [m for m in text_models
                            if any(k in m.lower() for k in ("deepseek", "code", "coder", "codestral"))]
                lm_model = deepseek[0] if deepseek else text_models[0]
                print(f"  [Ollama] 코다리 모델 선택: '{lm_model}'")
    except Exception as e:
        print(f"  [Ollama Info] 로컬 Ollama 모델 조회 생략 (서버 미기동 또는 연결 실패): {e}")

    creds = {
        "__GEMINI_API_KEY__": "",
        "__GEMINI_TEXT_MODEL__": lm_model if lm_model else "gemini-3.1-flash-lite-preview",
    ...
    try:
        ...
        if os.path.exists(gp):
            ...
            if not lm_model and g.get("TEXT_MODEL"): creds["__GEMINI_TEXT_MODEL__"] = g["TEXT_MODEL"]
...
def _inject_credentials(file_path, creds):
    """v7: 텍스트 파일 안의 placeholder 를 운영자 자격증명으로 교체 및 API Endpoint 로컬화.
    ...
    # 구글 API 서버 주소를 로컬 Ollama API 주소로 자동 변환
    google_endpoint_models = "https://generativelanguage.googleapis.com/v1beta/models"
    if google_endpoint_models in content:
        content = content.replace(google_endpoint_models, "http://localhost:1234/v1")
        replaced = True
        
    google_endpoint_base = "https://generativelanguage.googleapis.com"
    if google_endpoint_base in content:
        content = content.replace(google_endpoint_base, "http://localhost:1234")
        replaced = True
...
def _copy_tree(src_dir, dst_dir, creds=None):
    ...
    if missing_placeholders:
        guide = {
            "__GEMINI_API_KEY__": "AI Team → 외부 연결 → ✨ Google Gemini → API Key 입력",
            "__PAYPAL_CLIENT_ID__": "AI Team → 외부 연결 → 💰 PayPal → Client ID 입력",
        }
...
def _find_brain_root():
    ...
    v4: BRAIN_ROOT 환경변수가 가장 강함 (AI Team 익스텐션이 직접 지정).
    이전엔 ~/.ai-team-brain 가 빈 폴더로 존재만 해도 우선 매칭돼서
    ...
    cands = [
        ...
        os.path.expanduser("~/.ai-team-brain"),
        os.path.expanduser("~/.ai-team-brain-imported"),
    ]
    for c in cands:
        if os.path.exists(c):
            return c
