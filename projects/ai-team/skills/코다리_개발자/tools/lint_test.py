...
def main():
    ...
    if not results:
        ...
        if py_files:
            _py = sys.executable  # python3 하드코딩 대신 현재 인터프리터 사용
            errs = []
            for pf in py_files[:30]:  # 30개 cap
                code, out = _run(f'"{_py}" -m py_compile {json.dumps(pf)}', cwd=project, timeout=10)
                if code != 0:
    ...
    if passed == len(results):
        ...
        # Ollama로 실패 원인 분석 및 수정 제안
        failed_summary = "\n".join(
            f"[{label}] exit={code}\n{out.strip()[-500:]}"
            for label, code, out in results if code != 0
        )
        try:
            ...
            suggestion = _gc.text(
                f"다음 코드 검증 실패 로그를 분석해서 원인과 수정 방법을 한국어로 간단히 설명해줘:\n\n{failed_summary}",
                max_tokens=600, task="coding",
            )
            if suggestion:
                print("\n🤖 [Ollama 분석]")
                print(suggestion)
        except Exception as e:
            _log(f"AI 분석 실패: {e}", "warn")
