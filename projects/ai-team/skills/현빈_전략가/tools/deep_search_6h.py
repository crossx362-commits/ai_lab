...
def generate_dynamic_topics(custom_topic: str) -> list:
    ...
    prompt = (
        f"현빈 전략가. 주제 '{custom_topic}'를 6개 하위 리서치로 쪼개라. "
        'JSON 배열만: [{"hour":1,"topic":"짧은 제목","prompt":"핵심 질문 1문장"}]'
    )

    try:
        raw = lm_chat(prompt, json_mode=True, max_tokens=500, temperature=0.3, task="strategy")
...
def run_deep_research(custom_topic=None):
    ...
    for i, topic_info in enumerate(topics):
        ...
        research_prompt = (
            ...
            "한국어 markdown. 섹션 4개 이하: 시장/기회/리스크/즉시 실행. "
            "각 섹션 3줄 이하, 추정은 '추정' 표시."
        )

        try:
            analysis = lm_chat(research_prompt, max_tokens=1200, temperature=0.5, task="strategy")
            if not analysis:
