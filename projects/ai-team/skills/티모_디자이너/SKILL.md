...
- **행동**: 로컬 Ollama와 정규 표현식(Regex) 정적 분석 패턴을 결합하여 `petnna` 서비스의 주요 UI 컴포넌트 및 CSS 파일을 주기적으로 역추적 분석하고 발견된 결함 및 개선 리포트를 텔레그램으로 자동 포워딩합니다.
- **정규 표현식(Regex) 기반 자동 검사**:
  - CSS/JS 정적 코드 스캔 시 정규 표현식을 사용하여 금지된 타이포그래피(예: `font-family:.*(Inter|Roboto|Open Sans|Lato)`) 및 하드코딩된 크기 속성(예: 반응형을 저해하는 `width:\s*\d+px`), 부적절한 터치 타겟 크기 등을 감지하여 자동으로 개선 우선순위를 분류합니다.
