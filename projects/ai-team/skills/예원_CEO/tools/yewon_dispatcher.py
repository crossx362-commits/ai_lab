...
- 현빈(strategist): 코인/주식 시장 동향 수집 및 Ollama 로컬 분석, 공포탐욕지수·김프·KOSPI/KOSDAQ 퀀트 점수 보고
...
def dispatch_and_execute(ceo_message: str) -> str:
    ...
    try:
        ...
        _agent_keywords = {
            ...
            "현빈": ["현빈", "시장 분석", "코인 동향", "주가 동향", "주식 동향", "코인", "주식", "crypto", "stock"],
            "티모": ["티모", "디자인 검토", "UI", "UX"],
        ...
        elif _match(agent, ["현빈", "business", "research", "strategist"], ["시장 분석", "코인 동향", "주가 동향", "주식 동향", "현빈"]):
            import subprocess
            hyunbin_script = os.path.join(
                PROJECT_ROOT, "projects", "ai-team",
                "skills", "현빈_전략가", "tools", "crypto_market_intelligence.py"
            )
            result = subprocess.run(
                [sys.executable, hyunbin_script],
                cwd=os.path.dirname(hyunbin_script),
                capture_output=True, text=True, encoding="utf-8", errors="replace",
                timeout=300
            )
            if result.returncode == 0:
                return f"✅ 현빈 코인·주식 시장 분석 완료\n\n{result.stdout[:1500]}"
            else:
                return f"❌ 현빈 시장 분석 실패\n\n{result.stderr[:500]}"
