...
TUNING_DEFAULTS = {"gate_score": 48, "gate_entry": 48, "observe_minutes": 2, "paper_auto_max": 8}
TUNING_BOUNDS = {"gate_score": (40, 70), "gate_entry": (40, 75),
...
# ── 1) 학습 ──────────────────────────────────────────────
def _recent_trades(days: int = 14) -> list[dict]:
    cutoff = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
    return [t for t in _load(TRADES_FILE, [])
