...
- **업로드 후 모니터링 (가희 담당)**: 매일 오전 5시 가희(`content_inspector.py`)가 채널 전체를 스캔하여 중복 제목/설명/썸네일이 발견되면 Ollama 및 Pollinations를 통해 자동 교체 수정을 감행합니다. 루나의 `youtube_audit.py`는 가희의 오딧 프로세스를 호출하는 래퍼 역할로 동작합니다.
