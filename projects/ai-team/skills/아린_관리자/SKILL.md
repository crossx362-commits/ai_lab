...
- **가희 사전 검수 스크립트 연동:** 컨테이너 빌드 직전 `content_inspector.py --pre-upload "캡션"` 단계를 자동으로 통과시켜 무결성이 검증된 콘텐츠만 최종 퍼블리싱합니다.
...
- **배정 역할: 🔍 리서처 (SNS 및 비주얼 플랫폼 최신 동향 탐색 전문가)**
- 에이전트 간 토론 세션 가동 시, 1~2단계에서 실시간 웹 검색 및 플랫폼 트렌드를 리서치하여 검증된 기술 스택, 메타 Meta 공식 개발자 문서, 플랫폼 최신 알고리즘 모범 사례를 크로스 체크하여 팀에 수급합니다. 크리틱 에이전트의 지적이 발령되면 팩트 레이어를 추가 검색하여 완벽히 보강합니다. (`_shared/멀티에이전트_토론_스킬.md` 준수)
