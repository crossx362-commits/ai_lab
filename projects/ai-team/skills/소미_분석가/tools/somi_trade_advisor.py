...
def _volume_breakout(kis: KISClient, code: str) -> bool:
    """거래량 동반 20일 고가 돌파(turtle/Donchian) — 모의 전용 전략(웹 연구 2026-07-05, 한별 소유).
    당일 종가 > 직전 20거래일 고가 AND 당일 거래량 ≥ 직전 20일 평균 1.5배.
    ⚠️ 재검증 이력(2026-07-05): 최초 검증(진입점수 임계 40, 라이브보다 관대)에선 이 단순형이 약해 보여
    상대강도+RSI(14≤70) 필터를 추가했으나, **실제 라이브 게이트(진입점수 60)로 재검증하니 정반대** —
    필터 추가판은 거래가 4~21건까지 줄어 통계적으로 판단 불가(`backtest.py --validate`, MIN_TRADES=30
    기준 표본부족)였고, 오히려 **이 단순형이 유일하게 두 기간(12/24mo) 모두 표본충분(52~89건)·흑자·
    고샤프(2.5~3.1)로 검증 통과**. 실제 운영 게이트에 맞춰 검증해야 한다는 교훈(단일 임계값 검증 금지 —
    다기간·실게이트 일치 검증은 backtest.py --validate 참고). ⚠️ 백테스트 검증 전이라 실거래 미적용
    (호출부 _is_paper 가드). 25일 일봉은 모의 후보에만 조회 → 실거래 부하 0."""
    try:
