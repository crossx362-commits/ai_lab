...
아래 세 관문을 모두 통과할 때만 기동한다:
  ...
  2. 이 기계가 지정된 운영기일 것      — _shared/fleet_machine_policy.json(primary_platform)을
                                        petnna_single_machine_guard와 공유(2026-07-11 통합).
...
git pull 자체는 절대 실패시키지 않는다(항상 exit 0).
  ...
  python fleet_bootstrap.py --claim-ops-host # 이 기계를 운영기로 지정 (정책 파일 갱신)
  python fleet_bootstrap.py --check          # 관문만 점검, 기동 안 함

2026-07-11 통합: 예전엔 TELEGRAM_POLL_HOST(.env.encrypted, 기계+계정 파생 키로 암호화)로
운영기를 지정했다. 그 키가 machine+user 조합이라 두 기계가 같은 암호문을 못 읽어, 오너가
바꾼 지정이 다른 기계엔 전혀 전달되지 않는 결함이 있었다(펫나 데몬 쪽에서 먼저 발견 —
_shared/fleet_machine_policy.json 참고). 같은 결함을 여기서도 고치는 김에 두 메커니즘을
정책 파일 하나로 합쳤다(오너 지시: "합칠 수 있는 거 합쳐서 줄이자") — 시크릿 파일을
더 이상 안 건드리니 claim도 더 안전해졌다.
"""
import argparse
import platform
import subprocess
...
from _shared.process import read_fleet_policy, write_fleet_policy  # noqa: E402
...
def gates() -> tuple[bool, str]:
    ...
    designated = str(read_fleet_policy().get("primary_platform", "")).strip()
    if designated and designated != sys.platform:
        return False, (f"이 기계({_host()}, {sys.platform})는 지정 운영 플랫폼({designated})이 아니다 — 이중 가동 방지.\n"
    ...
    return True, f"통과 — 운영기 {_host()}({sys.platform}), 인터프리터 {sys.executable}"
...
def claim_ops_host() -> int:
    """이 기계를 운영기로 지정한다 — fleet_machine_policy.json의 primary_platform을 갱신.

    2026-07-11 이전엔 .env.encrypted를 복호화→갱신→재암호화했다(기계별 파생 키라 다른
    기계는 애초에 못 읽는 설정이었다). 이제 평문 git 파일 하나만 고치면 되니 암호화
    시크릿을 건드릴 필요가 없다 — claim이 이전보다 훨씬 안전해졌다."""
    policy = read_fleet_policy()
    policy["primary_platform"] = sys.platform
    policy["updated"] = _host()
    path = write_fleet_policy(policy)
    print(f"✅ 운영 플랫폼을 {sys.platform}({_host()})(으)로 지정했다. "
          f"{path.relative_to(ROOT)}이 변경됐다 — 커밋·푸시하라.\n"
