...
namespace Tankfall.View
{
    public sealed partial class BattleDemo
    {
        ...
        void ShellGalleryStep()
        {
            ...
            //    ✏️ **그런데 이건 포세이돈 교란의 원인이 «아니었다»**(2026-09-20 실측):
            //       고정한 뒤에도 포세이돈 465 → **504** 로 안 내려갔다. **가설 둘째도 틀렸다**
            //       (첫째는 「3단계에서 다시 만든다」였고, 그건 바닥을 더 올렸다).
            //    ⇒ 그래도 남긴다 — **캡처를 실시간에 안 기대게 하는 건 그 자체로 옳다.**
            //       다만 **「이게 그 문제를 고쳤다」고 읽지 마라.** 포세이돈 원인은 **아직 모른다.**
