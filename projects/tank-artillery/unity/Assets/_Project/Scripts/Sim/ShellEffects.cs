...
namespace Tankfall.Sim
{
    ...
    public class StatusEffects
    {
        ...
        /// <summary>
        /// 유닛에 화상. **맞은 유닛에게 불이 옮아붙은 것**이다 (2026-09-19 배선, 오너 허가).
        ///
        /// 🚨 **그 전까지 게임 호출부가 0곳이었다.** 캐터펄트 2번탄은 착탄점 장판(`PlaceFire`)만 깔았는데
        ///    `AiGunner` 는 <see cref="BurnGain"/> 으로 **유닛 화상**을 점수에 넣고 있었다 —
        ///    **AI 가 자기가 못 얻는 값을 계산하고 쐈다.** 이제 그 값이 실제로 일어난다.
        ///
        /// **설계 — ✅ 오너 확정 (2026-09-19): 장판은 «자리», 화상은 «맞은 유닛» 을 맡는다.
        /// 둘은 겹칠 수 있고, 겹치면 둘 다 들어간다.**
        /// 불 한가운데에서 직격당한 탱크가 더 아픈 것이 의도다.
        /// 🛑 **이제 이걸 «버그»로 보고 한쪽을 지우는 것은 금지다.** 오너가 확정한 규칙이다.
        /// ⚠️ 눈이면 안 붙는다(장판과 같은 규칙, 원작 "눈이 오면 캐터펄트의 불이 사라진다").
        ///    거는 쪽(`BattleDemo`·`BattleSimVerify`)이 날씨를 보고 거른다 — 여기서 날씨를 보지 않는 이유는
        ///    `StatusEffects` 가 날씨를 모르는 순수 상태 보관소이기 때문이다.
        /// ⚠️ **덮어쓰기다 — 안 쌓인다**(독과 같다). <see cref="BurnGain"/> 이 그 규칙을 그대로 본다.
        /// </summary>
        ...
        /// <summary>
        /// 몸에 붙은 불을 끈다. 눈이 올 때 부른다 — 원작 "눈이 내리면 캐터펄트의 불이 사라진다".
        ///
        /// 🗑️ **여기 있던 `Cleanse`(독·화상·속박을 통째로 지움)는 2026-09-19 에 삭제했다.**
        ///    그건 **폐기된 게이지형 궁극기(§2-9-12)의 효과 5종 중 하나**였고, 그 설계가 사라지면서
        ///    부를 주체가 없어져 호출부 0곳으로 남아 있었다(명세 §8 은 "✅구현"이라 적어 놨었다).
        ///    지금 설계에 «전부 지우는» 해제가 필요한 자리는 없다. 필요한 것은 **불만 끄는** 이 좁은 해제다.
        ///    ⚠️ 궁극기로 정화를 되살리지 마라 — 궁극기는 핵 하나로 오너가 확정했다.
        ///
        /// ⚠️ 장판 불(`HazardField.ClearFires`)만 끄고 이건 안 끄면 **바닥 불은 꺼지는데 몸에 붙은 불은
        ///    계속 타는 비대칭**이 생긴다. 둘은 같은 자리에서 같이 불려야 한다(`BattleDemo.SetWeather`).
        /// </summary>
        /// <returns>불을 끈 유닛 수 — 0 이면 "꺼짐" 로그를 띄우지 않기 위해.</returns>
        public int ClearBurns()
        {
            int n = _burn.Count;
            _burn.Clear();
            return n;
