...
static class BattleSimVerify
{
    ...
    // ══════════════════════════════════════════════════════════════════════════════════
    //  🎛️ **스위치 표 — «무엇을 바꾸나 / 무엇을 «안» 바꾸나 / 어느 경로를 타나»** (2026-09-19)
    //
    //  🚨 **이 표가 없어서 하루에 «세 번» 물렸다.** 셋 다 같은 고장이다 —
    //     **손잡이의 «사정거리»가 코드에도 문서에도 없었고, 그래서 «전부에 닿는다»고 가정했다.**
    //       ① `TANKFALL_SEED` 가 **미러 게이트에만** 닿았다 → 「12종 전부 0.0%p」를 «재현성»으로 오독할 뻔했다.
    //          (그건 재현성이 아니라 **스위치가 안 닿았다는 증거**였다.)
    //       ② 난이도 사다리 판정에 **네거티브 컨트롤이 없었다** → 빨간불도 못 믿는다.
    //       ③ `TANKFALL_VAR` 이 **`.WithFlight()` 를 안 탔다** → 추진·아에로·모션 없는 탱크를 쟀다(+15%p).
    //
    //  🛑 **「안 바꿈」 칸은 «확인한 것»만 적는다.** 안 본 것은 **「미확인」**이다 —
    //     빈칸을 「안 바꿈」으로 읽는 순간 이 표가 **또 하나의 거짓 초록불**이 된다(「없음은 값이 아니다」).
    //  🛑 **스위치를 새로 만들면 여기에 줄을 «먼저» 추가해라.** 세 칸을 못 채우겠으면 그 스위치는 아직 덜 만든 것이다.
    //
    //  | 스위치            | 바꾸는 것                                   | **안 바꾸는 것**(확인된 것만)                       | 비고 |
    //  |-------------------|---------------------------------------------|-----------------------------------------------------|------|
    //  | TANKFALL_MAP      | 지형(MapHeightFunction.MapKind)             | 미확인                                              | 머리말에 맵 이름이 찍힌다 |
    //  | TANKFALL_SEED     | **8개 RunMatch 전부**(2026-09-19 확장)      | 기후 굴림(BattleSimVerify 가 판마다 따로 굴린다)     | **옛날엔 미러에만 닿았다(사고 ①)** |
    //  | TANKFALL_ROW      | 그 행만 돌린다(셀당 판수 2배)               | **미러 게이트를 건너뛴다**(BurnWiringGate 만 돈다)   | 기능 커밋 전엔 필터 없이 한 번 |
    //  | TANKFALL_VAR      | 그 기종의 «기본 스탯» 한 필드                | —                                                   | **WithShell «전»에 덮는다**(2번탄 배율 순서) |
    //  | └ (사고 ③)       | —                                           | **~~WithFlight~~ 를 «안» 탔다 → 2026-09-19 수리**    | `AssertVarPathIsFaithful()` 이 이제 강제 |
    //  | TANKFALL_VALUES   | VAR 의 값 목록(한 런에 여러 개)             | 런 수를 안 늘린다                                   | 값마다 `--- VAR = v ---` 가 찍힌다 |
    //  | TANKFALL_CRATER   | `VAR=CraterRadius` 의 준말                  | 미확인                                              | — |
    //  | TANKFALL_AERO     | 수평 상수 가속 배율(0~1)                    | 미확인                                              | 머리말에 **대조군 도장**이 찍힌다 |
    //  | TANKFALL_TEAM     | 팀 인원 + **서든데스 턴도 같이**            | 미확인                                              | 위 SuddenDeathTurn 주석 참고 |
    //  | TANKFALL_CLIMATE  | 기후(증폭벽·회오리)                         | 미확인                                              | — |
    //  | TANKFALL_WEATHER  | 날씨(Clear/Snow)                            | 미확인                                              | 눈이면 화상 0 이 **정답**이다 |
    //  | TANKFALL_ITEMS    | 아이템 수                                   | 미확인                                              | — |
    //  | TANKFALL_SUPPLY   | 헬기 보급                                   | 미확인                                              | — |
    //  | TANKFALL_ULT      | 궁극기 on/off                               | 미확인                                              | — |
    //  | TANKFALL_SPREAD   | 다탄두 패턴 on/off                          | 미확인                                              | — |
    //  | TANKFALL_NOWIND   | 바람 0                                      | 미확인                                              | — |
    //  | TANKFALL_NOHEAL   | 회복 0                                      | 미확인                                              | — |
    //  | TANKFALL_CRATERSHAPE | 크레이터 모양                            | 미확인                                              | — |
    //  | TANKFALL_MIRROR_ERR  | 미러 게이트의 조준 오차                   | 미확인                                              | — |
    //  | TANKFALL_ONLY     | 돌릴 단계 고르기                            | 미확인                                              | — |
    //  | TANKFALL_BALANCE_OK | (verify.sh) 이 측정의 «의도 확인»          | 측정 내용은 아무것도 안 바꾼다                      | 금지가 아니라 의도 확인 |
    //
    //  🔑 **「안 바꾼다」가 «의도»인지 «사고»인지도 같이 적어라.**
    //     `TANKFALL_ROW` 가 미러를 건너뛰는 것은 **의도**다(그 행만 돌리니까).
    //     `TANKFALL_VAR` 이 `WithFlight` 를 안 탄 것은 **사고**였다 — 같은 「안 바꿈」인데 성격이 정반대다.
    // ══════════════════════════════════════════════════════════════════════════════════
    ...
    static void Matchup(TankKind? only, uint perCell)
    {
        ...
        // 🚨 **이 진단 열들은 «하네스 정의»다**(2026-09-20 확인). 안 적으면 다음 사람이
        //    게임 전적과 나란히 놓고 「안 맞는다」고 판다 — 실제로 두 정의가 다르다.
        Console.WriteLine("    🚨 낙하·지속·설치물은 **«피해자 팀»** 기준이다(하네스 정의). "
                          + "게임 전적은 **«사수»** 기준이라 **자해(아군 오사)에서 갈린다** — 게임은 세고 여기선 안 센다. "
                          + "기종끼리 비교는 성립한다(여기선 정의가 일관). **게임 전적과 나란히 놓지 마라.**");
