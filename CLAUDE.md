...
- **"문제 찾아 고치기"를 정기 자동 감사로 전환 — `petnna_pipeline_audit.py` 신설(2026-07-11, 오너 "문제는 좀 계속 찾아서 고쳐 지식화 자동화해")** — 바로 위 항목에서 사람이 "검토했어?"라고 물어야만 나온 연쇄 발견 패턴(기능 하나 고치면 그 주변에 잠든 결함이 드러남)을, 매번 사람이 시키지 않아도 정기적으로 재현하는 도구를 만들었다. `skills/예원_CEO/tools/petnna_pipeline_audit.py` — 펫나 파이프라인 핵심 파일 10개 전문을 구독 클로드(plan 모드=읽기전용)에게 통째로 보여주고, 이 저장소에서 실제로 터졌던 사고 5종(배정과제 캡/게이트/슬롯 방치·조용한 예외 삼킴·에이전트 간 안전장치 비대칭·데몬 루프 안 sys.exit 위험·재시도 상태의 재시작 초기화)을 점검 관점으로 명시해 패턴매칭시킨다. 발견은 보고서(`output/qa/petnna/pipeline_audit/`)+텔레그램, 고확신 항목만 백로그에 `보류`+`owner=사람`으로 적재(LLM 추정 결함이라 자동 구현 대상에는 절대 안 넣음 — code_auditor.py의 안전한 `--apply`와 달리 이건 기계적으로 검증 불가능한 로직 판단이라 사람 확인 필수). skill_auditor(월)·code_auditor(토)와 겹치지 않게 일요일 배치, schedules.json+launchd 등록. **첫 실행에서 실제로 5건 발견, 전부 검증해서 그날 즉시 고침**: ①`select_issue()`가 재발 시 attempts만 리셋하고 status는 '완료'로 남겨 재도전이 또 실패해도 MAX_ATTEMPTS·회의소집이 영원히 발동 안 함(→ status도 '대기'로 리셋) ②`_shared/backlog.py`에 `owner_type_mismatch()` 신설 — 테오·미오는 owner뿐 아니라 type까지 맞아야 소비하는데 council.needs_human()은 owner만 봐서 owner=테오+type≠테스트 같은 배정이 '대기'로 적재돼도 아무도 못 집는 좀비가 될 수 있었음(적재 시점 차단 + harness_monitor 디스패치 시점 방어 이중화) ③테오의 배정 테스트 과제에 상한 없이 무한 재시도되던 결함 — `TASK_MAX_ATTEMPTS`(3) 도입, 도달 시 '보류' 전환(수리의 MAX_ATTEMPTS 패턴과 동일 계열) ④나무만 미오·테오·백호와 달리 `advisory_lock` 보호가 없어 데몬과 수동 실행이 겹치면 backlog.json 동시쓰기 가능 — 동일 패턴 적용. 회귀 테스트 `test_pipeline_audit.py`(5개)·기존 테스트 파일들에 추가 12개. **교훈**: 사람이 "검토했어?"라고 재차 묻지 않아도 같은 깊이의 점검이 반복되게 하려면, 발견 자체를 자동화하되 "적용"은 여전히 사람 검토를 거치게 하는 게 안전하다 — LLM이 코드를 읽고 짐작한 결함은 실제로 맞을 확률이 높아도(이번엔 5/5 전부 진짜였다) 기계적으로 검증 가능한 게 아니므로.
