...
- **클라우드 클로드는 구독(`claude -p`) 1선 — API 크레딧 막힘 대응(2026-07-05 오너 지시)** — `llm._claude_code`가 Claude Code headless(`claude -p --append-system-prompt`)를 subprocess로 호출해 **구독 사용량으로 클로드**를 쓴다(API 크레딧 불필요). 체인: Ollama→**ClaudeCode(구독)**→Gemini→Claude API(크레딧 백업). 품질은 로컬 gemma 압도(전문 퀀트 수준 JSON). **주의 ①`--bare` 금지**(API키 인증만 읽어 구독 OAuth 무시) **②구독 rate limit 공유** — 데몬 대량 호출이 한도 넘으면 오너 본인 Claude Code가 제한된다. 그래서 로컬 우선(`AI_TEAM_LLM_PRIMARY=ollama`) 유지로 구독 호출을 최소화하고, 구독은 `lm_first=False`(issue_impact 등) 클라우드 우선 작업의 1선으로만. ③subprocess라 API보다 느림.
