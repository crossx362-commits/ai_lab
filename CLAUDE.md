...
- **게이트 하한은 백테스트 근거(2026-07-02)** — 12개월·40종목 그리드(`backtest.py --grid`): 기준 50↓는 누적 -42~-65%·MDD -75% 손실 엣지, 55↑ 흑자, 60↑ 손익비 1.2+. 모의 게이트 기본 55·튜닝 하한 52. 하락 국면은 차단 대신 +10 선별(`SOMI_BEAR_GATE_BUMP`) — 백테스트상 무차별 매수 +23.6% vs 선별 +62.3%(전량차단과 동성능). RSI·상대강도 추가 필터는 과필터로 성과 악화 — 추가 금지. "체결 늘리려 문턱 인하"는 수익률 파괴, 재검증 없이 52 밑으로 내리지 마.
