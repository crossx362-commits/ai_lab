...
- **게이트 하한은 백테스트 근거(2026-07-02)** — 12개월·40종목 그리드(`backtest.py --grid`): 기준 50↓는 누적 -42~-65%·MDD -75% 손실 엣지, 55↑ 흑자, 60↑ 손익비 1.2+. 모의 게이트 기본 55·튜닝 하한 52. 하락 국면은 차단 대신 +10 선별(`SOMI_BEAR_GATE_BUMP`) — 분기점이 정확히 65(60이면 +17.8%로 붕괴, 65↑ 전량차단과 동성능 +62.3%). RSI·상대강도 추가 필터는 과필터로 성과 악화 — 추가 금지. "체결 늘리려 문턱 인하"는 수익률 파괴, 재검증 없이 52 밑으로 내리지 마.
- **수급확인 게이트는 최대 엣지원 — 끄지 마라(2026-07-02 백테스트)** — 모멘텀 단독 +62%(샤프 2.3) vs 수급확인 추가 +154%(승률 71%·PF 2.06·MDD 절반·샤프 5.0). `SOMI_SOOMGEUP_GATE` 기본 true 유지. 눌림목(PF 1.06)·평균회귀(-94%)는 데이터로 기각 — 재도입 금지. 조용한매집(선취)은 누적 +5192%/800건이나 MDD -84% — 채택하려면 슬롯 상한·하드손절 필수 조건으로 별도 검증부터. 학습 데이터 배관: 청산→`somi_closed_trades.json`→한별 tune(평일 15:45, 성장엔진 16:10 선행)·성과추적(금 16:30)·성장엔진 learn.
- **운영 기계 분담: 장중=Windows PC, 맥은 꺼지는 날 있음(2026-07-02)** — 맥은 아침(~06:45) 종료→저녁(19:00) 부팅하는 날이 있어 장중 매매(소미제안 데몬·발굴·데스크)는 Windows 프로세스가 주인. 맥 launchd 장중 잡은 맥이 켜진 날의 안전망(데몬 가동 시 one-shot 자동 스킵). 새 매매 코드 배포 시 Windows 쪽 소미제안 데몬 재시작 필수 — 가동 중인 옛 데몬엔 새 코드가 반영 안 된다. "장중에 안 돌았다" = 그 시간 어느 기계가 켜져 있었는지부터 확인.
