...
- **`issue_impact` 평가는 클라우드(json_mode) 고정 — 클로드 1선(2026-07-05 GPT 퇴출)** — 로컬 모델은 json_mode 미적용이라 JSON 파싱 실패가 잦고, 실패 시 빈 결과로 덮어써 뉴스 신호가 통째로 유실된다. 체인은 Claude→Gemini→로컬(`llm.text lm_first=False`). GPT는 기본 체인에서 제거(오너 지시) — 영숙 함수호출도 클로드 tool use(`ANTHROPIC_BOT_MODEL`, 기본 haiku). 매 슬롯 재평가는 비파괴(비면 직전값 복원).
