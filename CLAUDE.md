...
| 소미 (Somi) | Analyst — 실시간 감시/집행 코어 | `somi_price_monitor.py`, `somi_position_monitor.py`, `short_covering_analyzer.py`, `watchlist_manager.py` |
| 한별 (Hanbyul) | Quant — 정량 매매 두뇌: 전략 검증·백테스트·매수판단·발굴·튜닝 | `quant_analyzer.py`, `backtest.py`(전략·게이트 검증), `somi_trade_advisor.py`, `somi_screener.py` |
...
- **열지도 타일은 균일 정사각형 격자 — 트리맵 아님(오너 지시 2026-07-07)** — `status_dashboard.py`의 `renderMarket`는 모든 종목을 **같은 크기 정사각형**으로 섹터별 그룹 배치(색=등락률만). 셀 한 변 `s`를 이분탐색으로 세로(H)에 맞게 최대화, 시장/패널마다 s는 달라도 한 뷰 안에선 전부 동일. **크기로 시총/거래대금 인코딩 금지**(과거 squarify/stripLayout 트리맵은 제거). 크기 토글(시총/거래대금)은 섹터 내 정렬 순서로만 작동 → 푸터 라벨 `정렬 =`. 상시 데몬(8890)이 사용자 화면 — 코드 수정 후 `launchctl kickstart -k gui/$(id -u)/com.ailab.dashboard`로 재로드(워치독은 HEAD 변화에만 반응, 미커밋 편집은 수동 재시작 필요). 참고: preview(claude) 헤드리스는 flex-height 단독 `/heatmap`에 뷰포트를 0×0으로 줘 타일이 0개로 보임 — 대시보드 버그 아님, `renderMarket` 직접 호출로 검증하거나 실브라우저(8890)로 확인.
...
- **긴급 시장경보는 시세 실측이 1차 — 웹검색 LLM 의존 금지(2026-07-08 서킷브레이커 지각 사고, 격노)** — 7/7 코스피 장중 -8.2%(서킷브레이커)를 속보감시(breaking_monitor, 시간당 웹검색)가 13:00·15:00 "속보 없음"으로 놓치고 다음날 09:26에야 보고 + 문구가 매번 달라 쿨다운 무력화로 3연발. 원인: 긴급 감지를 LLM 웹검색에 의존(검색/CLI 실패 = 조용한 장님) + 앞40자 dedup 키. 수리: ①`somi_price_monitor.index_crash_watch`(1분 상시) — 코스피/코스닥 프록시(KODEX 069500/229200) 전일대비 -5%(사이드카권)·-8%(서킷권) 실측 즉시 텔레그램, 지수·단계당 하루 1회(파일 상태), 전송 성공만 소진(실패 시 1분 뒤 재시도), 단계는 `SOMI_INDEX_CRASH_STAGES` ②breaking_monitor는 [긴급] 시작 강제(잡문 전송 차단)·지수급락류는 날짜+카테고리 dedup·과거사건 제외 프롬프트. 뉴스 LLM은 맥락 보강용 2선이지 감지 1선이 아니다.
...
- **조립식 스펙 base 선택지 — 눌림형은 `"base":"trend"` 필수(2026-07-07)** — momentum base(`_score_levels`)는 돌파일에만 60+라 pullback 필터와 AND하면 구조적 거래 0건(Pullback_Strong 24·36mo·60종목 0건 실증 — 모멘텀 60+ ↔ 3~10% 눌림 동시 불가). `make_spec_levels`가 `spec.base`(momentum|trend 화이트리스트, 미지값 momentum 폴백) 지원. `_trend_levels` 배점은 **눌림 중에도 유지되는 큰 추세 척도만**: MA20>MA50 정배열 25 + MA20 상승기울기 20 + 종가>MA50 15 + 60일수익 양 10 = 최대 70(게이트 60 = 앞 3요소 동시 충족). **종가>MA20 배점 금지** — 3~10% 눌림이면 MA20 아래로 내려가 눌림형과 재모순(합성 검증: 5.9% 눌림서 45점 탈락 → MA50 기준 70점 통과). ※커밋 2899f63 메시지의 배점 설명(종가>MA20 25)은 폐기 초안 기준 — 코드가 정본. strategy_lab `_valid_spec`은 pullback 포함 스펙에 trend 강제, `_gen_specs` 프롬프트도 base 안내. 이 개선은 채택 아닌 **측정 가능성** 목적(웹 순수 눌림 분석 2026-07-07은 중소형 -24% 열등) — Pullback_Strong은 base:trend로 백로그 재검증 대기. 담당: 한별.
...
- **"쓸데없는거 다 삭제" 정리(오너 지시 2026-07-08) — 산출물 0인 데몬 3개 + 설계상 정지 데몬 1개 완전 삭제** — 실 로그 감사 결과: 미장소미(`somi_us_trader.py`, 5일 가동·체결 0건·스캔 로그도 없음), 크립토소미(`somi_crypto_trader.py`, 1.5일 가동·10분마다 스캔했지만 매번 후보 0건), 전략랩(`strategy_lab.py`, 1일+ 가동·결과 1건 생성했으나 보류·채택 0건) — 셋 다 "실험/미검증 수집단계"로 시작해 실제 산출물 없이 daemon 슬롯·API 호출·로그만 소비. 소미신호(`somi_signal_engine.py`)는 모의 중 설계상 정지 상태로 계속 떠서 매 사이클 "정지" 메시지만 찍음. **넷 다 완전 삭제**(daemon 정지 + agent_controller/notify.py/check_all.py 등록 제거 + 스크립트 파일 삭제 + growth_engine의 US 전용 튜닝 함수·bot_common/somi_bot_tools의 미장 조회 핸들러 등 연쇄 죽은 코드 제거 + 플러그인 문서 참조 정리). 필요해지면 git 이력에서 복구 가능. 소미제안 매수 게이트도 같이 정리 — 오늘 추가했던 역행강세선별(위 항목)부터 제거해 핵심(탐지점수·bear bump·수급확인·손익비·dq/risk상태)만 남김. **교훈: 미검증 실험 데몬을 계속 띄워두려면 최소한 산출물(로그/체결/결과)이 실제로 쌓이는지 정기 확인할 것 — "일단 켜두고 지켜보자"가 방치로 굳으면 아무도 안 보는 죽은 프로세스가 됨.**
