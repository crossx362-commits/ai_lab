...
- **거래량 돌파+상대강도+RSI = 모의 전용 전략(웹 연구+백테스트 개선 2026-07-05, 한별 소유)** — `advisor.analyze_candidate`가 `_is_paper() and _volume_breakout(kis,code)`이면 +10점 가점. **4중 조건**: ①당일 종가 > 직전 20거래일 고가 ②당일 거래량 ≥ 직전 20일 평균 1.5배 ③RSI(14) ≤ 70(블로우오프 회피) ④상대강도(종목 20일 수익률 > 코스닥150 지수 = 주도주). **단순 돌파(초기 +7)는 약했음**(12mo -24.2%·24mo +43.5%·샤프 0.82) → 오너 지시 "10점까지 좋게 개선"으로 상대강도+RSI 필터 결합. **개선 검증(`backtest.py --compare` `_breakout_combo_levels`, 40종목·10일 보유): 24mo 승률 62.9%·PF 1.54·+204%·MDD -35.5%·샤프 2.89 = 전 전략 1위, 12mo도 승률 63.6%·+36.4%·MDD -34.3%·샤프 1.83 흑자.** 두 기간 모두 최상위·최저 낙폭 — 상대강도가 수익, RSI회피가 낙폭 절반. 52주 신고가(장기 축)와 병행 가점. **실거래 미적용**(`_is_paper` 가드 — 지수 포함 조회 전부 모의 후보만, 실거래 부하·영향 0). 담당: 한별. 실거래 승격은 모의 실데이터 축적 후 판단. 눌림목·평균회귀는 데이터 기각(재도입 금지). 백테스트 개선 변형은 `_breakout_rs/combo/52w_levels` 3종 비교로 combo(상대강도+RSI)가 승자.
