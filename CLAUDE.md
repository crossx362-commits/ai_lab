...
- **국면선별(하락장 역행강세 선별) 임시 활성 — 검증된 우위 없음, 오너 지시로 실험 가동 중(2026-07-08)** — `backtest.py --beargate`로 실측: 대형주 40종목(12·24mo)에선 역행강세선별이 전량차단과 **사실상 동일**(거래 54→55/89→90건, PF·MDD·샤프 무변화 — 하락장 통과 종목이 거의 없어 선별이 사실상 전량차단과 같은 효과). 중소형 20종목(**실사냥터**, 12·24mo)에선 역행강세선별이 전량차단보다 **전 구간 열등**(누적 -32.3%/-55.7% vs 전량차단 -29.0%/-37.4%, 샤프도 더 나쁨) — 이 유니버스는 세 옵션(게이트없음·전량차단·역행강세선별) 전부 손실. 즉 **채택 근거 없음**(strategy_lab 3단 판정 기준으로도 중소형 손실이라 기각감). 그럼에도 오너가 "임시로 돌려" 명시 요청해 `SOMI_BEAR_RELSTR`(기본 true, 모의+bear국면 한정, `somi_trade_advisor._bear_relstr_ok`) 로 활성화 — **데이터수집 실험이지 검증된 개선이 아님**. `.env SOMI_BEAR_RELSTR=false`로 즉시 원복 가능(코드 변경 불필요), 원복 시 `_doctrine_audit`엔 안 걸림(이 토글은 독트린 위반 목록 밖 — 오너 명시 요청이라 무단강화 아님). 참고: 이 결과는 앞선 "경쟁력 감사"·"ATR 변동성 손절" 항목과 같은 계열의 패턴 — 대형주에서 괜찮아 보이는 필터가 중소형(실사냥터)에선 열등하게 나오는 사례가 반복됨.
