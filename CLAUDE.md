...
- **분할매수는 '돌파 증액'만 유효 — 눌림 분할은 역선택(2026-07-03 연구)** — 40종목 12/24개월: 돌파분할(50%+50%@+2%, 3일 유효)×현행청산 = PF 2.43·24mo +568%·MDD -19%·샤프 5.26 (일괄 대비 샤프 2.2→5.3). 눌림 분할(-2% 추가)은 약한 종목일수록 풀포지션이 되는 역선택으로 성과 반토막 — 도입 금지. 분할매도는 현행(+5% 절반+트레일)이 최적 근처, 3단 세분화는 승자 이익만 깎음. 트레일온리는 누적 최고나 승률 37%로 기각. 상세: `docs/SPLIT_ENTRY_EXIT_STUDY_2026-07-03.md`.
