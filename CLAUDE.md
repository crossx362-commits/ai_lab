...
- **거래량 동반 20일 고가 돌파(turtle/Donchian) = 모의 전용 실험(웹 연구 2026-07-05, 한별 소유)** — `advisor.analyze_candidate`가 `_is_paper() and _volume_breakout(kis,code)`이면 +7점 가점: 당일 종가가 직전 20거래일 고가 상향 돌파 + 당일 거래량 ≥ 직전 20일 평균 1.5배(웹: 검증 돌파 승률 60~70%·R:R 3:1+). **52주 신고가(장기 신고가 축)와 다른 단기 돌파+거래량 확인 축** — 두 전략 병행 가점. **실거래 미적용**(`_is_paper` 가드 — 25일 조회·가점 전부 모의 후보만, 실거래 부하·영향 0). 백테스트로 기존 전략 대비 우위 검증 후에만 실거래 도입. 담당: 한별. 눌림목·평균회귀는 이미 데이터로 기각됨(재도입 금지)이니 신규 실험은 돌파·모멘텀 축으로 한정. **백테스트 검증 결과(2026-07-05, `backtest.py --compare` `_breakout_levels`, 40종목): 약하고 기간 의존적 — 12mo 누적 -24.2%(기존 모멘텀 +20.7%보다 열위·샤프 0.4), 24mo +43.5%(기존 -27.4%보다 우위·샤프 0.82). 52주 신고가(12mo +74.6%/24mo +203%·샤프 1.6~1.8)에 크게 못 미침. → 모의 데이터 수집은 유지하되 실거래 승격 금지(우위 불명확). 52주 신고가만 실거래 후보로 유효.**
