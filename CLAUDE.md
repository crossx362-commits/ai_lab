...
- **백로그 owner는 '그 에이전트가 백로그를 읽는가'로만 배정 — 소비 경로 없는 배정은 조용한 영구 방치(2026-07-10 사고+수리)** — 회의(`petnna_council.py`)가 액션아이템을 백호·미오·나무에게 배정했으나 그 셋은 `add_backlog_items`로 **적재만 하고 읽지 않는다**. 수리는 `owner in ("", "수리")` 필터로 정상 스킵하고, 신선도 감사(`petnna_fleet_health.py`)는 산출물만 보므로 경보도 없어 4건이 `대기`로 1.5일 방치됐다. 수리 로그의 `처리 가능한 이슈/과제 없음 — 대기`가 백로그에 `대기` 잔량이 있는데도 뜨면 = **owner 불일치**(유령 PR대기와 증상 동일, 원인 다름 — ③번으로 추가 점검). 수리: `council.needs_human()`이 `AUTO_OWNERS=("", "수리", "테오", "미오")` 밖 owner를 사람 검토 트랙(보류)으로 라우팅 + 테오(`owner=테오·type=테스트`)·미오(`owner=미오·type=디자인`)가 자기 과제를 소비(산출물을 남긴 뒤에만 완료 처리 — 실패 시 대기로 남아 재시도). **새 에이전트에 과제를 배정하려면 먼저 그 도구에 백로그 소비 코드를 넣고 `AUTO_OWNERS`에 추가하라.** 회귀 테스트: `tests/test_backlog_routing.py`.
