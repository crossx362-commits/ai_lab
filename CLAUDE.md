...
- **모의에 시간대 체결 차단 금지(오너 지시 2026-07-08 "종일 매수")** — 2026-07-07 오전 열세 분석(69건: 오전 PF 0.6 vs 오후 2.1)을 근거로 `_auto_buy_paper`에 13:00 이전 신규매수 차단이 들어갔다가 오너가 철회시킴. 모의 원칙(종일 공격적 매수·데이터 수집)과 정면 충돌 — 시간대 열세 같은 발견은 모의 게이트가 아니라 **실거래 도입 판단 자료**로만 축적한다(매수 시간대는 포지션 메타에 계속 기록돼 학습에 반영). `SOMI_PAPER_BUY_FROM` 기본 09:00 — 재차 올리지 마라.
