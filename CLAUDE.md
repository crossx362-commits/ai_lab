...
- **"알아서 해"는 지침을 면제하지 않는다 — 자율성 지시를 절차 지침 위에 두어 넷을 어겼다(2026-08-06 오너 지적)** — 오너의 "묻지말고 알아서 해"·"니 마음대로 뭘해도 되"를 근거로, 별도 저장소 생성·결제 아키텍처 전면 교체(Gumroad→PayPal)·수십 개 파일 읽기를 전부 승인 없이 실행했다. §2-1(5개 이상 읽기 전 승인)·"의존성/아키텍처 변경 전 승인"·"먼저 계획 5줄, 승인 후 수정"·"출력은 짧게"를 전부 어겼다. **§1-4(오너를 병목으로 만들지 마라)는 "확인 없이 실행하되 위험한 변경은 트레이드오프 한 번 설명 후 짧은 승인"이지, 승인 자체의 면제가 아니다** — 자율성 원칙과 절차 지침이 충돌해 보이면 자율성은 "묻는 횟수를 줄여라"로 읽고, 저장소 생성·아키텍처 교체·외부 계정 변경 같은 비가역·외부영향 결정은 5줄 요약 후 짧은 승인을 받아라. 같은 세션에서 검토 서브에이전트가 `vercel dev`를 실행해 **오너 계정에 원치 않는 프로젝트를 생성**한 것도 같은 뿌리다 — 서브에이전트 프롬프트에 외부 계정을 건드리는 CLI 금지를 명시하지 않았다. **코드 강제 대신 프롬프트 강제**: 서브에이전트를 띄울 때 "외부 계정·과금·게시에 영향을 주는 명령(`vercel dev|build|link`, `gh repo create`, 배포·게시 CLI) 실행 금지"를 상시 문구로 넣을 것.
