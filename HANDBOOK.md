...
- `docs/` 4: `AI_MODEL_STRATEGY.md` ⭐(2026-07-22 낡은 아키텍처 기술로 CLAUDE.md 포인터로 축소) ·
  `AI_QUALITY_OPTIMIZATION.md` · `GEMINI_TOKEN_OPTIMIZATION.md` · `progress.md`
  (`AGENT_COMPATIBILITY.md`·`AGENT_PIPELINE_REVIEW.md`는 삭제된 루나/아린/경수/케빈 세대의
  1회성 점검 보고서라 2026-07-22 삭제)
...
### 2-8. 추측·예측 금지 (오너 지시 2026-08-04)
모르는 것을 아는 것처럼 답하거나 구현하지 않는다. 모든 작업·보고·제안은
**확인된 사실**(코드 직접 열람·실측·실행 결과) 또는 **검색으로 확인한 정보**(웹서치·문서)에
근거한다. 확인하지 못한 것은 "추정"·"추가 확인 필요"로 명시하고, 필요하면 검색 후 작업한다.
(2-1 거짓말 금지의 사전 단계 — 회의·LLM의 결론도 가설이므로 코드를 직접 열어 확인 후 실행.)

### 2-9. 항상 한국어로 (오너 지시 2026-08-04)
사람에게 보내는 모든 응답·보고·텔레그램 메시지·커밋 메시지·회의록은 한국어로 작성한다
(코드·식별자·로그 원문은 예외). 오너가 여러 세션에 걸쳐 반복 확인한 지침이다.
